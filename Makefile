...
#   summer     → editions/summer/    (short interactive edition: exercises, word searches, answers — published site book)
#   summer/en  → editions/summer/translations/en/ (full English handbook, translation base; build with `make en`)
...
#   translations/XX → editions/translations/   (7 language editions)
#
# Quick reference:
...
LANG_CODES := ru fr de zh es fa tr
...
help:
	...
	@printf "  check            Verify all citation keys resolve in every bib\n"
	...
	@printf "  push             Push to GitHub (origin)\n\n"

# ── Citation gate ───────────────────────────────────────────────────────────
check:
	python3 build/scripts/check_citations.py
