...
**Date of this document:** 2026-06-21

> **June 2026 update.** Since the April baseline the Winter edition went through
> an extended editorial pass (now 517 pp): citation-integrity fixes replacing
> fabricated/illustrative examples with verified real sources, a second expert
> panel (all items resolved), four native TikZ/pgfplots figures, a chapter-letter
> appendix numbering scheme (`NA.x`) with a hyperref-anchor fix, cross-appendix
> math harmonization, and a full reconciliation of the teaching materials and the
> interactive reader with the corrected book. Repo hygiene also landed: deploy
> infra (with server IPs) purged from history, limen-os reduced to author links,
> the unused `editions/winter/chapters/` markdown source tree retired to a
> gitignored `.local/` archive, and a redundant npm lockfile / one-off script
> removed. Mirrored to GitHub and GitLab.
...
- **Teaching** — `editions/winter/latex/teaching/` (`ch*_teacher.tex` +
  `ch*_solutions.tex`)
- **Reader** — `web/reader/public/winter/chapters/ch01.md`–`ch18.md` (chapter
...
| Winter PDF build | ✅ Verified | `make winter` clean as of June 2026: 517 pp, 0 undefined refs, 0 duplicate destinations, 0 biber warnings; output `editions/winter/latex/main.pdf` |
...
| Winter "mentality" review | ☐ Pending | Holistic pass over thesis coherence, argument flow, and voice across all 18 chapters (queued) |
