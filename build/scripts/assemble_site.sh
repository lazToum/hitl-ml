...
cp "$ROOT/site/favicon.svg" "$DIST_SITE/favicon.svg"
...
if [ -f "$ROOT/editions/autumn/index.html" ]; then
  ...
  [ -f "$ROOT/editions/autumn/morty.png" ] && cp "$ROOT/editions/autumn/morty.png" "$DIST_SITE/autumn/morty.png"
fi
...
# companion/web/index.html fetches /companion/<name>.md, so md goes at the root
cp "$ROOT"/companion/*.md "$DIST_SITE/companion/"

# Spring *handbook* (jupyter-book HTML) + landing page. The Spring *platform*
# (FastAPI/Rust backend, Flutter mobile) is not static, so only the book ships here.
if [ -d "$ROOT/editions/spring/_build/html" ]; then
  mkdir -p "$DIST_SITE/spring/book"
  cp -R "$ROOT/editions/spring/_build/html/." "$DIST_SITE/spring/book/"
fi
if [ -f "$ROOT/editions/spring/web/landing.html" ]; then
  mkdir -p "$DIST_SITE/spring"
  cp "$ROOT/editions/spring/web/landing.html" "$DIST_SITE/spring/index.html"
fi
...
[ -f "$ROOT/editions/summer/hitl_summer_edition.pdf" ] && cp "$ROOT/editions/summer/hitl_summer_edition.pdf" "$DIST_SITE/downloads/summer/hitl_summer_edition.pdf"
...
# GitHub Pages: custom domain + skip Jekyll (preserve _static/ underscore dirs).
printf 'what-if.io\n' > "$DIST_SITE/CNAME"
touch "$DIST_SITE/.nojekyll"
