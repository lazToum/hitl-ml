...
| Compressed toward zero | P(routed) is very low — humans rarely see anything, so HCI stays near zero no matter how much value they add, and the accuracy gap is estimated on a tiny, unreliable sample |
...
- Always report the routed-subset size alongside HCI, with confidence intervals on the accuracy gap. When P(routed) is tiny, HCI is near zero by construction and the gap estimate is noise — a small routed set tells you nothing until it grows.
...
One convention, used throughout this essay: the escalation threshold is the reported confidence below which cases go to a human. Raising the threshold routes more cases to humans; lowering it routes fewer. Every threshold discussion below assumes this direction.
...
- Human agreement rate with model on routed cases
- HCI over time
