...
*Note: This is a study guide, not an academic text. The case studies (Netflix, Nest, Alexa, Super Bowl, GPS incidents, Air Canada) are well-known public incidents used here as teaching examples. For primary sources and formal citations, see the Winter Edition's `references.bib`.*
...
- Work through **Part III** for the case studies you should be able to retell
- Use **Part IV** for the technical layer (active learning, alignment methods, vocabulary)
- Use **Part V** for practice questions (cover the answers first)
- Use **Part VI** as the night-before review sheet
- Use **Part VII** to identify what's still shaky
...
*Part I asked the questions. Now let's build the mental models to answer them. The frameworks below are the analytical backbone of the field — they connect the case studies in Part III to the concepts you'll be tested on.*
...
The mathematical foundations underlying each dimension (predictive entropy and mutual information for uncertainty detection, Bayesian decision theory for stakes calibration, online learning for feedback integration) are covered in the Winter Edition's appendices.

**The simple model behind every threshold.** At any decision point, a HITL system faces a binary choice: act autonomously or query a human. Let $c$ be the system's confidence (0–1), $C_{\text{error}}$ the cost of getting it wrong, and $C_{\text{query}}$ the cost of asking. The expected cost of acting blindly is $(1-c) \cdot C_{\text{error}}$; the cost of asking is $C_{\text{query}}$. The optimal policy: **query when $c < \tau$, where $\tau = 1 - C_{\text{query}}/C_{\text{error}}$**. High-stakes errors pull $\tau$ toward 1 (ask almost always); expensive queries pull it toward 0 (act almost always). Every threshold in this book — from Netflix's 90-minute pause to the Nest thermostat's motion timeout — instantiates this trade-off.

:::{note}
**Relationship to the Spring Edition.** The Spring Edition (Build the Loop) works from a complementary five-dimension set — Control, Transparency, Feedback, Autonomy, and Trust — oriented around what a builder decides. Both frameworks describe the same HITL design space from different vantage points: Winter analyses systems from the outside, Spring builds them from the inside.
:::
...
**Multiple incidents worldwide (Charlton, MA 2021; Kailua-Kona, HI 2023; and others)**
...
**What happened:** A 6-year-old asked Alexa to get a dollhouse. Alexa ordered a $170 one. When a news anchor reported the story on TV, home viewers' Echo devices woke up and tried to order dollhouses too.
**Why it happened:**
...
**Numbers:** Hallucination rates (2024 studies, 2023-era models) — 58–88% for general-purpose LLMs on verifiable questions about specific federal cases (Dahl et al.); ~17–33% for RAG-based legal research tools (Magesh et al.)
...
| **BADGE** | Uses gradient embeddings; selects diverse AND uncertain batch | Landmark batch method (ICLR 2020); recent benchmarks find uncertainty sampling often competitive |
...
- Eliminates critic/value network (40–60% memory reduction vs. PPO; illustrative estimate)
...
- Reduces human annotation cost from ~$1–10/sample to <$0.01 (illustrative)
- Human labor shifted from labeling to writing the constitution

**Comparison at a glance:**

| Method | Reward source | Needs RL? | Memory | Best for |
|--------|---------------|-----------|--------|----------|
| PPO-RLHF | Learned RM from human prefs | Yes | High | General alignment |
| DPO | Implicit (no RM) | No | Low | Preference alignment |
| GRPO | Learned RM or verifiable rewards | Yes (simplified) | Medium | Reasoning/math |
| Constitutional AI | Learned RM from AI-labeled prefs (RLAIF) | Yes (RLAIF) | Medium | Harmlessness at scale |
...
**Ownership gap** (Ch. 18) — training data contributed by nearly everyone, models owned by very few, with opacity (undisclosed corpora, unverifiable inclusion) in between

**Ghost work** (Ch. 18) — the invisible human labor (labeling, moderating, verifying) that makes automated systems appear automatic; term from Gray & Suri
...
| Annotation cost | ~$1–10/sample (illustrative) | ~$0.01/sample (AI does labeling; illustrative) |
...
**Paragraph 2 — GPS case:** Algorithm confident in directions; zero mechanism to express spatial uncertainty or sanity-check "this is a boat ramp, not a road." Driver deferred to expressed confidence. Multiple vehicle losses and rescues.

**Paragraph 3 — Air Canada chatbot:** System presented hallucinated policy information with the same confidence as factual information. No pathway to say "I'm not sure — ask a human." Result: $800+ legal liability; the tribunal held that a company is responsible for the information its AI provides.
...
**BADGE = landmark batch method for deep active learning (ICLR 2020).** Uses gradient embeddings. But: uncertainty sampling still competitive when implemented properly.
...
```
              ...
              ASKS WELL                     DOESN'T ASK WELL
       +------------------------------+------------------------------+
HIGH   | Medical AI radiology         | GPS into lake                |
STAKES |                              | Air Canada chatbot           |
       |                              |   (confident hallucination)  |
       +------------------------------+------------------------------+
LOW–   | Netflix "still watching?"    | Nest thermostat              |
MEDIUM | Fraud detection (SMS text)   |                              |
STAKES | Voice assistant: "I don't    |                              |
       |   understand"                |                              |
       +------------------------------+------------------------------+
```
...
*Previous editions: Misunderstood | Summer Workbook Edition | Before You Ship*
