...
Calibration matters for HITL because escalation thresholds are set on confidence scores. Throughout this manual, the escalation threshold means the reported confidence below which cases go to a human — raising the threshold sends more cases to review, lowering it sends fewer. If a "95% confidence" score actually predicts 70% accuracy, the threshold at which human review is triggered is meaningless — the model will pass cases to auto-processing that should be reviewed.
...
**Model answer:**
Direct Preference Optimization (DPO) is an LLM alignment method that converts human preference comparisons (which of two outputs is better?) directly into a fine-tuning loss, without needing an intermediate reward model or reinforcement learning. Technically: it reparameterizes the RLHF objective so that the reward can be expressed in closed form in terms of the policy and the reference (SFT) model, allowing preference data to optimize the model directly via supervised learning.
...
*Grade-band examples are given for four of the eight case studies (Netflix, Nest, Air Canada, GPS) as representative. Apply the same bands by analogy when grading Echo, Google Home, Medical AI, and Fraud Detection.*
...
**Grade A response:**
The Air Canada case (Moffatt v. Air Canada, 2024 BCCRT 149) illustrates three principles beyond the immediate legal outcome. First: companies cannot disclaim liability for AI-generated content by treating the system as a separate entity. The "chatbot is a separate legal entity responsible for its own actions" defense was explicitly rejected — all information on a company's website is the company's responsibility. Second: the specific failure demonstrated is "confident wrongness" — the chatbot presented incorrect policy information with the same certainty it used for correct information, with no differentiation. The failure was not in being wrong (all systems err) but in lacking any mechanism to express uncertainty or escalate to a human. Third: this highlights a design expectation for HITL in customer-facing information systems. A single sentence — "I believe the policy is X, but please confirm with our support team before booking" — would have changed the outcome. For HITL designers, the lesson is that Uncertainty Detection and the escalation pathway (Intervention Design) are not optional quality improvements in high-stakes information delivery — they are a growing liability exposure.
...
1. **Threshold calibration:** Raise the flagging bar so only alerts with high probability of clinical significance escalate. If 40% of records are being flagged, the flagging criterion is far too loose.
...
The chatbot's specific error was not that it was wrong — all systems produce errors. The error was that the wrong answer was delivered with exactly the same confidence, tone, and format as a correct answer. A customer has no way to distinguish "this chatbot is certain" from "this chatbot is guessing." The hallucination rate for general-purpose AI systems on legal queries is documented at 58–88% (Dahl et al. 2024) — far above any threshold at which confident delivery of information is appropriate. The chatbot's design did not reflect this limitation.
...
GRPO eliminates the critic/value network entirely. For each training prompt, GRPO generates a *group* of K responses (DeepSeekMath used K = 64; implementations vary widely, typically 8–64). The advantage for each response is computed as:
...
- Requires no second network (significant memory reduction; DeepSeekMath states only that dropping the critic "significantly reduces training resources")
...
| **Cost per training signal** | ~$1–10/comparison (human time + annotator management; illustrative) | ~$0.01/comparison (AI self-generates and evaluates against principles; illustrative estimate — primary sources do not publish per-sample dollar figures) |
...
**Key synthesis point:**
Both methods are HITL. The human is not eliminated from either pipeline. DPO shifts human effort to individual evaluation; Constitutional AI shifts it to normative design. Constitutional AI is not "less human involvement" — it is different human involvement. Writing a good constitution is a high-skill, high-stakes task: a poorly constructed constitution will produce a model that perfectly satisfies bad principles at scale. The cost reduction claimed for Constitutional AI (from human annotation costs to near-zero AI self-evaluation costs) is illustrative rather than sourced; the human responsibility is not reduced, it is concentrated earlier and at a more influential point.
...
- Result: Beta is less accurate overall, but you can set an escalation threshold at "confidence < 85%" and know that at least 15% of the flagged emails are misclassified. You can reason about the system's behavior.
