...
The QR code encodes the URL `{base_url}/scan/{token}`. When scanned, the phone follows this URL to the API's scan endpoint. The base URL must be publicly reachable from the player's phone — `localhost` will not work in production.
...
From the app's perspective, NFC and QR produce identical results: a URL containing a token. The only difference is how the URL is obtained (tap vs. scan) and the `source` parameter sent to the scan endpoint (`"nfc"` vs. `"qr"`). This distinction is logged in the event table.
...
The universal scan endpoint handles both QR and NFC:
...
The endpoint does not validate that the scan is for the player's current clue. It just returns which clue the token belongs to. The app decides what to do with that information — confirm the player is in the right place, show an error if not.

This design allows the scan endpoint to be used by spectators, developers, and future features without requiring session context.
