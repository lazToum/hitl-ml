...
The dashboard communicates with the Rust API through `web/src/services/api.ts`. Authentication uses `oidc-client-ts` — the creator logs in via Dex and the browser stores the token.
...
The `CreateHuntRequest` matches the Rust API's `CreateHunt` model: title, description, optional start/end times. The status starts as `draft` — the creator cannot activate a hunt with no clues.
...
- The clue's content (title, body, media URL)
    ...
    const generateWithAI = async () => {
        ...
        try {
            ...
            setForm(f => ({
                ...
                answerValue:     result.data.answer_value,
                answerTolerance: result.data.answer_tolerance,
...
The AI generation fills the form but does not save. The creator sees the AI output in the same form fields they would use to write a clue manually. They can edit any field before saving. The "AI was here" is not stored — a generated clue and a hand-written clue look identical in the database.
