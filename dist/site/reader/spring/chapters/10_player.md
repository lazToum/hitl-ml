...
The `scopes` list includes `groups` — this tells Dex to include group membership in the token. Without this scope, the `groups` claim would be absent and all logins would default to the `player` role.
