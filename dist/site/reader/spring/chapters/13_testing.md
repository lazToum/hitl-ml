...
This edition ships without tests. That is honest: tests are the first casualty of time-constrained implementation, and adding them after the fact reveals gaps that were invisible during development.

What should exist:

- **Unit tests** for the validators (`clue_validator.rs`) — fuzzy match edge cases, GPS boundary conditions
- **Unit tests** for the hint engine (`hint_engine.rs`) — unlock condition logic
...
Adding these tests to `clue_validator.rs` would catch regressions if the normalization or similarity function changes. They would not catch the "mt. everest" case if someone forgot to write that test case. The tests are only as good as the edge cases the writer imagined.
...
def test_generate_clue_response_shape():
    ...
    result = asyncio.run(generate_clue(
        theme="history",
        location_description="a town square",
        difficulty="medium",
        answer_type="text",
    ))
    assert "title" in result
    assert "body" in result
    assert "answer_value" in result
    assert isinstance(result.get("answer_tolerance"), float)
...
async fn generate_clue_deserializes() {
    ...
    let result = client.generate_clue(GenerateClueRequest {
        theme: "history".into(),
        location_description: "a town square".into(),
        difficulty: Some("medium".into()),
        answer_type: Some("text".into()),
    }).await;
...
The system has no tests. You have been asked to add some. Before writing a single test, make a list of the five most important behaviors to test in this system — the five things that, if broken, would cause the most harm to users or the most loss of trust in the system.
