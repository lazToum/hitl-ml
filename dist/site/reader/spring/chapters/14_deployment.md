...
**1.** The docker-compose.yml starts six services: postgres, dex, rust-api, python-ai, redis, and the web assets server. What is the minimum set needed to run a hunt with one player? What could you cut?

**2.** The Dex configuration uses static passwords for dev accounts. What does "production" Dex configuration look like? What changes, and what stays the same?
...
**Internet-facing deployment.** A cloud server with a proper domain, HTTPS, and Dex configured with a real identity provider (Google, Apple, Microsoft, university SSO). This is what a long-term or high-stakes deployment looks like.
...
**`OIDC_ISSUER` is localhost.** For production, this must be the server's public hostname or IP. The issuer URL is baked into every token Dex issues — tokens issued with `localhost` as the issuer will fail validation on any other machine.
...
For a classroom deployment with a small number of known users, static passwords (as in `dex/config.yaml`) may be sufficient. Each participant gets an account created by the organizer.

For a larger or more open deployment, configure Dex to use an upstream identity provider:

```yaml
# dex/config.yaml additions for Google login
connectors:
  - type: google
    id: google
    name: Google
    config:
      ...
      redirectURI: https://your-server.com/dex/callback

# Disable static passwords when using real identity providers
enablePasswordDB: false
```

With this configuration, players authenticate with their Google accounts. Dex handles the OAuth flow and issues its own tokens to the Rust API. The Rust API does not know or care that the identity ultimately came from Google.

Groups (for role assignment) can be configured via Dex's group claim mapper or by creating a custom connector that adds group membership based on email domain or other criteria.
...
- **Redis integration.** Redis is in the compose file but the Rust API does not use it. Planned uses: session caching, rate limiting on the answer submission endpoint (to prevent brute-forcing QR tokens). Not implemented.
...
- **Health endpoints.** The API has no `/health` endpoint. Docker compose uses `pg_isready` for postgres and `wget` for Dex, but the Rust API has no healthcheck. Adding one is a one-line change.
