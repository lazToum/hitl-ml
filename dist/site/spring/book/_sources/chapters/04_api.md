...
Most application routes require authentication because their handlers take `auth: AuthUser`. The `/public/hunts` endpoints are the exception: they are intentionally unauthenticated so the discovery page can list active hunts before login. The scan route is encoded into QR/NFC tags, but the current handler still takes `AuthUser`; scanned URLs therefore work from the authenticated app flow and will not behave as an anonymous spectator endpoint without an authorization token.
