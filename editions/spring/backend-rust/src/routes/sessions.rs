...
pub async fn submit_answer(
    ...
    let result   = clue_validator::validate(&state.db, &clue, &req, Some(&state.ai)).await?;

    let hunt_id = fetch_hunt_id(&state, id, auth.id).await?;
    let logger  = EventLogger::new(&state.db, &state.broadcaster);
    // Deliberately omit the raw submitted value: the event log records the
    // outcome (score, passed, attempts), never what the player guessed.
    logger.log(id, hunt_id, Some(clue.id), types::ANSWER_ATTEMPTED,
        json!({ "score": result.score, "passed": result.passed, "attempts": attempts }),
    ).await?;
