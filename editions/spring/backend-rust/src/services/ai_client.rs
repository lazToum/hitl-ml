...
impl AiClient {
    ...
    /// POST builder with the shared-secret header attached when configured.
    fn post(&self, path: &str) -> reqwest::RequestBuilder {
        let req = self.client.post(format!("{}{}", self.base_url, path));
        match &self.token {
            Some(token) => req.header("X-Sidecar-Token", token),
            None => req,
        }
    }
    ...
    pub async fn semantic_match(&self, expected: &str, submitted: &str) -> anyhow::Result<f64> {
        ...
        let resp: Resp = self.post("/semantic-match")
            .json(&Req {
                expected,
                submitted,
            })
    ...
    pub async fn verify_photo(&self, description: &str, photo_b64: &str) -> anyhow::Result<f64> {
        ...
        let resp: Resp = self.post("/verify-photo")
            .json(&Req {
                description,
                photo_b64,
            })
    ...
    pub async fn adaptive_hint(
        ...
        let resp: Resp = self.post("/adaptive-hint")
            .json(&Req {
                clue_body,
                answer,
                attempts,
    ...
    pub async fn analyze_session(
        ...
        let resp: Resp = self.post("/analyze-session")
