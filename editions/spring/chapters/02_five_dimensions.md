...
The Winter edition develops a five-dimension analytical framework for evaluating HITL systems (Uncertainty Detection, Intervention Design, Timing, Stakes Calibration, and Feedback Integration — see Winter Part II and its study companion for the full treatment). This edition works from a complementary set of five dimensions — Control, Transparency, Feedback, Autonomy, and Trust — oriented around what a builder decides when constructing a loop. The two frameworks describe the same design space from different vantage points: Winter asks "what properties should a well-designed system have?" from the outside; Spring asks "what decisions do you make when you build one?" from the inside. They are not competing frameworks — they are the same concerns, refracted through different roles.

:::{admonition} The decision at every threshold
:class: note
Beneath every design dimension is a simple decision-theoretic tradeoff. Let $c$ be the system's confidence (0–1) that acting autonomously would be correct, $C_{\text{error}}$ the cost of a wrong autonomous action, and $C_{\text{query}}$ the cost of asking a human (latency, friction, attention). Expected cost of acting: $(1-c) \cdot C_{\text{error}}$. Cost of querying: $C_{\text{query}}$. The optimal policy: **query when $c < \tau$, where $\tau = 1 - C_{\text{query}}/C_{\text{error}}$**. A fuzzy matching threshold of 0.85, a hint unlock timer of 5 minutes, a photo verification confidence cutoff — every threshold you will set in this system is an instance of this tradeoff. The five dimensions are how you refine the question "what should $C_{\text{error}}$ and $C_{\text{query}}$ be?" into something concrete.
:::
...
:class: tip
The current `PATCH /hunts/:id` endpoint already lets creators change status and hunt metadata. The unresolved design question is versioning: should editing a clue on an active hunt be allowed? If yes, what should happen to sessions already past that clue? If no, what does the UI show when a creator tries?
:::
...
**For the creator:** The observer dashboard (`routes/observer.rs`) streams live session events over WebSocket. A creator watching a live hunt can see which clue each player is on and can inspect attempts, hints, completions, and nudges as events arrive. This is high transparency — arguably more than a creator designing a physical hunt would have.
...
- **Player creation**: On first login, the middleware (`middleware/auth.rs`) creates a new player row automatically, inferring their role from Zitadel project-role claims or the email fallback. No human approves the account.
...
:class: note
Every default in the system encodes an assumption about trust. The default fuzzy threshold of 0.85 implies the system trusts that answers scoring 0.85 or higher on the similarity scale are probably correct. The default hint unlock of 5 minutes implies the system trusts that 5 minutes is long enough for a genuine attempt. These defaults will be wrong for some hunts. The question is: who do you trust to change them?
:::
