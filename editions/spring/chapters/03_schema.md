...
Let us walk through `db/schema.sql` and read it as a design document rather than a technical artifact. The listings below are abridged for readability: they omit the `hints`, `clue_tokens`, `teams`, and `team_members` tables, some referential-action clauses, and several indexes. The file itself is the source of truth.
...
**`email` is nullable.** The schema leaves room for anonymous or non-email identities through `anon_token`, but the current app flow uses OIDC login and just-in-time player creation through Zitadel. Anonymous play is a schema affordance, not a wired product flow.
...
A hunt is owned by exactly one player (who must have `role = 'creator'`, though the schema does not enforce this — the API does). The status lifecycle is represented by `draft`, `active`, and `archived`. The current API accepts status updates between these values, and the dashboard includes a Reactivate action for archived hunts; there is no irreversible archive rule yet.
...
The `events` table is a catch-all audit log. Significant actions — clue view, answer attempt, correct answer, hint request, observer nudge, session start, session complete, and scan resolution when the scan endpoint is used with a session — generate event rows. The `payload` is a JSONB blob that can hold arbitrary data specific to the event type.
...
A few indexes to keep the most common queries fast (abridged — the schema defines nine). Reads of `events` are per-session and served by `idx_events_session`; the observer dashboard's live feed is push-based over an in-memory broadcast channel and never queries the table. The `ts` index is there for the time-range queries the analytics pipelines above will need — retention sweeps and "all events in the last N minutes" reports — once someone builds them.

---

## Running the schema

Start Postgres and apply the migrations with `sqlx-cli` (install it once with `cargo install sqlx-cli --no-default-features --features postgres`). From the edition root:
...
This applies both migrations: the initial schema and the `external_id` column added for OIDC identities. If you start the full compose stack instead, you can skip this step — the Rust API runs `sqlx::migrate!()` on every startup (`backend-rust/src/db/mod.rs`) and applies any pending migrations itself. The manual command is for working against the database without the API running.
