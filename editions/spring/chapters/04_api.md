...
**3.** Compile-time SQL query checking sounds appealing. But it means your code can only build if a database is running — or you have pre-cached the schema. What does that tradeoff look like in CI? On a laptop without internet?
...
**Memory safety without garbage collection.** The Axum server can handle thousands of concurrent WebSocket connections without pausing for GC — though in practice, database connection pooling and broadcast channel contention are more likely bottlenecks for this system than garbage collection. For latency-sensitive workloads, the absence of GC pauses is a genuine advantage.
...
The cost is real: longer compile times, a steeper learning curve, and more ceremony around ownership. For an edition like this one, that is a reasonable tradeoff — readers who work through the Rust chapters learn something they cannot learn from a FastAPI tutorial.
...
Redis is optional in `AppState`. Startup attempts to connect to it; if the connection fails, the API logs a warning and continues with `redis: None`. When Redis is available, answer submissions use it for a per-player, per-clue rate limit of 10 attempts per minute. It is not yet used for session caching or background jobs.
...
Router::new()
    ...
    // Session analysis (creator/observer)
...
Most application routes require authentication because their handlers take `auth: AuthUser`. The `/public/hunts` endpoints are the exception: they are intentionally unauthenticated so the discovery page can list active hunts before login. The scan route is encoded into QR/NFC tags. The current handler still takes `AuthUser`, so scanned URLs only work inside the authenticated app flow — there is no anonymous spectator access without a token.

Route-level authorization — checking that the authenticated user has the right role — happens in the handler or service, not at the router level. A player who calls `POST /hunts` will get through authentication but be rejected by the hunt creation handler once it checks their role.
