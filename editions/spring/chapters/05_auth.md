...
We replaced the homebrew JWT system with [Zitadel](https://zitadel.com/): a modern, self-hosted identity platform built in Go (v4.x; the compose stack in Ch. 14 pins `ghcr.io/zitadel/zitadel:v4.14.0`). It ships a polished login UI, project-level role management, and a full admin console — without requiring a JVM or a heavyweight runtime.
...
# Admin: admin@treasurehunt.localhost / Admin1234! (or ZITADEL_ADMIN_PASSWORD)
```
...
- Three dev users: `creator@example.com`, `observer@example.com`, `player@example.com` (password: `Password1!`)
...
**`validate_aud = false`.** The resource server does not check the audience claim. In a single-service setup, checking it adds friction without security benefit. If the system grows to multiple services, re-enabling audience validation should be the first security hardening step — token reuse across services is a common attack vector in microservice architectures.
...
(Zitadel's current docs recommend the project-ID-scoped form `urn:zitadel:iam:org:project:{projectId}:roles`; the code relies on the legacy alias shown above for simplicity. The `orgDomain` value is illustrative, not literal.)

The `infer_role()` function checks this first, then falls back to a generic `groups` array used by some OIDC providers, then to email-prefix convention — which handles the three dev accounts out of the box without any role grant setup:
...
rust-api:
  environment:
    ...
    OIDC_JWKS_URL: http://nginx:80/oauth/v2/keys      # internal URL (for key fetching)
```
...
The solution in this compose stack is to fetch keys through the internal nginx service (`nginx:80`) while validating the issuer against the public URL (`localhost:8180`). The nginx service preserves the public host header that Zitadel expects, so the JWKS request works from inside Docker without changing the issuer claim. These two environment variables exist precisely to support this split.
...
Is this a problem? What would you change if this system were used in a real classroom with real students? What would you keep? (Under GDPR, JIT account creation may fall under "contractual necessity" if the service cannot function without an identity, or "legitimate interest" — but the data subject should be informed. For educational deployments, check whether your institution's data protection agreement covers automatically created accounts.)
