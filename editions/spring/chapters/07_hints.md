...
Note that `num_minutes()` truncates to whole minutes — a player at 4:59 since unlock sees `elapsed = 4`, so the default 5-minute threshold means "after 5 full minutes have elapsed."
...
Note what the exhausted-hints path does *not* do: `request_hint` returns an error when the sequence runs out, and the fallback from that error to an AI-generated hint lives in the route handler (`routes/sessions.rs`), not in this function — it is described in "What the AI adds" below.
...
When a player returns to the view of the clue they are currently on — common in the mobile app — they should see all hints they have already unlocked, not just the last one:
