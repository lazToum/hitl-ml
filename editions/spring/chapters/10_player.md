...
class AuthService {
  ...
  static Future<String?> accessToken() => _storage.read(key: 'access_token');
}
```

Note what `accessToken()` does not do: there is no expiry check and no refresh-on-read. The service has a `refresh()` method that exchanges the stored refresh token for a new pair, but nothing calls it automatically — an expired access token is sent to the API until a request comes back with a 401. Automatic refresh is a known gap in the current app.
...
class _ScanScreenState extends State<ScanScreen> {
  ...
  void initState() {
    super.initState();
    _tryNfc(); // also attempt an NFC read when the screen opens
  }
  ...
  Widget build(BuildContext context) {
    return MobileScanner(
      onDetect: (capture) {
        ...
        final token = NfcService.tokenFromUrl(barcode!.rawValue!);
...
The scanner extracts the token from the scanned URL via `NfcService.tokenFromUrl` and returns it to `ClueScreen`; the `_tryNfc()` call on init is what makes the "Tap NFC" half of the button label real. `ClueScreen` places that token into the answer field and submits it to `POST /sessions/:id/answer`, where QR and NFC answers are validated by exact match against the clue's token in `clue_tokens`.
...
`LocationAccuracy.high` uses GPS hardware when available, cellular/WiFi triangulation as fallback. In practice on mobile, this gives roughly 3–20 metre accuracy outdoors depending on conditions, device, and satellite visibility. The creator needs to account for this variability when setting the clue radius.
...
The React web player includes a proximity map for GPS clues. Rather than showing the player exactly where the target is, the map renders an 80-metre teal circle centered on the target location. The player can see whether they are inside or outside the zone, but not where within it they should go.

This is a deliberate game-design choice, not a privacy control. The zone circle avoids spoiling the exact spot at a glance and keeps the physical challenge on the map, while still giving the player useful directional feedback. But it does not protect the target location: the exact target coordinates are shipped to the web client, and the page renders a live "~Xm away" distance to that exact point — a desk-bound player could read the coordinates from the network response or triangulate from the distance readout. If privacy were the goal, the server would have to return only the zone, not the target.

In the web app, the player's current position is tracked via `watchPosition` (the browser Geolocation API's continuous mode, not a one-shot reading). The haversine distance between the player's current coordinates and the target is computed client-side and displayed in real time as they walk — the slight floating-point differences between JavaScript and the Rust server's haversine are negligible for an 80-metre zone display. The actual answer submission still sends a fresh point-in-time coordinate reading to the server. In Flutter, `LocationService.stream()` exists, but the current clue screen uses a one-shot `LocationService.current()` call when the player taps **I am here!**.
...
When a web player submits the final correct answer, a full-screen Babylon.js particle scene fires over the completion screen. The scene uses a burst emitter — on the order of a thousand particles alive at once (the emitter caps at 2,000, with 400 emitted per second and 2–4 second lifetimes) with randomized initial velocity, in a hardcoded gold/teal/indigo palette.

The celebration is implemented as a React component that mounts a `<canvas>` over the full viewport and runs the Babylon.js engine until the player navigates away — there is no fixed duration or timed unmount. It adds on the order of a few hundred KB (gzipped) to the player bundle, depending on how well the bundler tree-shakes the Babylon import graph — a deliberate choice: the 3D particle engine signals that this is a game, not a form, and the emotional beat of a completion celebration is worth the bundle cost for the intended use case.
...
At what point does this become a problem? Is it always a problem, or only when the data is used in specific ways? What disclosure, if any, would you include in the app's onboarding? (In jurisdictions governed by GDPR, passive collection of behavioral data for system improvement requires a lawful basis — typically legitimate interest or consent. For classroom deployments involving students, FERPA or COPPA may apply.)
