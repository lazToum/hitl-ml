...
- A game master, annotating struggling players' sessions with nudges (player-facing delivery is discussed below)
...
The channel capacity is 128 events. If a subscriber falls behind (slow network, browser tab backgrounded), it will miss events beyond the buffer. The drop is not silent, though: the next `recv()` on a lagged `tokio::broadcast` receiver returns `Err(RecvError::Lagged(n))` with the exact number of missed events, and the receiver stays connected. The current handler's `while let Ok(event) = rx.recv().await` loop treats any error — including `Lagged` — as end-of-stream, so the dashboard never learns it skipped events; surfacing a "missed N events" marker would be a one-match-arm change. Observers see a live stream, not a guaranteed delivery queue.
...
The endpoint checks that the caller is staff, checks hunt access, upgrades the HTTP connection to WebSocket, subscribes to the hunt's broadcast channel, and forwards events as they arrive. As noted above, an observer who falls behind can miss events beyond the channel buffer — and because the handler does not act on the lag signal, they might see incomplete data without knowing it.
...
The `models/event.rs` module defines the event taxonomy:
