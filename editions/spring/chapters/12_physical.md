...
**1.** QR codes and NFC tags are physical objects placed in physical locations. What happens when the QR code is damaged, the NFC tag stops responding, or the location becomes inaccessible? Who is responsible?

**2.** The QR code encodes a URL that encodes a token that maps to a clue. There are three layers of indirection. Why is each layer there? What would break if you removed one?
...
The minimum dimension is 200×200 pixels, suitable for printing at 2–3 cm per side. QR codes are redundant by design — at error-correction level M (the default used by `QrCode::new` above) they can decode correctly with roughly 15% of the code damaged; level H tolerates up to 30% but produces a denser symbol for the same data. Printing at a larger size further improves reliability.
...
class NfcService {
  static Future<String?> readTag() async {
    return await FlutterNfcKit.poll().then((tag) async {
      final records =
          tag.ndefAvailable ? await FlutterNfcKit.readNDEFRecords() : null;
...
Some starting points: a text-only mode that replaces QR/NFC/photo with typed answers for players without cameras or mobility; audio clues for visually impaired players; web-based participation for those without smartphones; and a placement guideline that QR codes should be reachable from a seated position. None of these are implemented here — but naming the gaps is the first step toward designing for a wider range of humans.
