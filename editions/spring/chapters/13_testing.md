...
The fuzzy text validator is the best candidate for unit tests. Its core is a pure comparison (no database, no network), it has edge cases that are easy to enumerate, and failures have direct user impact. The tests below are a simplified sketch — the real `validate_text` takes the clue, the submission, and an optional AI client — but the cases are the ones that matter:
...
Integration tests for the API require a running database. The `axum-test` crate provides a `TestServer` abstraction for in-process testing without binding a real port (add `axum-test` under `[dev-dependencies]` in `Cargo.toml`):
...
The `<creator_token>` and `<player_token>` placeholders stand in for a JWT the test server will actually accept: mint one signed with a throwaway key the test `Config` trusts, or mock the `AuthUser` extractor. Without one of those, the requests fail with 401 before the interesting assertion runs.
