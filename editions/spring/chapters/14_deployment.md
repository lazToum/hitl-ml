...
**1.** The docker-compose.yml starts the core services: postgres, Zitadel (plus its `login` UI), nginx, rust-api, redis, and python-ai. It can also start the handbook server under the `book` profile. What is the minimum set needed to run a hunt with one player? What could you cut?
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_USER:     hunt
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-hunt_secret}
    volumes:
  ...
  rust-api:
    ...
    environment:
      DATABASE_URL:   postgres://hunt:${POSTGRES_PASSWORD:-hunt_secret}@postgres:5432/treasure_hunt
...
The full `docker-compose.yml` also sets Zitadel's database credentials (`ZITADEL_DATABASE_POSTGRES_*`) so it can create and reach its own database — omitted here for brevity; without them the stack fails at Zitadel startup.

A few things to note for production:

**Passwords are in docker-compose.yml.** The `${POSTGRES_PASSWORD:-hunt_secret}` form reads the password from the environment (or a `.env` file), falling back to `hunt_secret` when nothing is set. For local development this is fine; the committed default is not a secret. For anything shared, move the password out of the committed file — a `.env` file excluded from version control, environment-specific override files (`docker-compose.prod.yml`), or Docker secrets.
...
The two-stage build means the final image contains only the compiled binary and its runtime dependencies — no Rust toolchain, no build artifacts. With the `debian:bookworm-slim` base shown above, the image lands on the order of 100–150 MB uncompressed (roughly a third of that compressed for registry push/pull).
...
```bash
# On first deploy, run migrations from the host against the mapped
# postgres port. The production image contains only the server binary —
# no cargo, no sqlx-cli, and no migrations/ directory — so
# `docker compose exec rust-api ...` cannot run them:
DATABASE_URL=postgres://hunt:hunt_secret@localhost:5432/treasure_hunt \
  cargo sqlx migrate run --manifest-path backend-rust/Cargo.toml
```
...
Let's Encrypt provides free TLS certificates via `certbot`. For a classroom event, a self-signed certificate might work, but the Flutter app will refuse the connection outright (no bypass prompt), and browser sessions will show a warning — both break trust at a critical moment.
...
- **Health endpoints.** Postgres and Zitadel both have compose healthchecks (Zitadel's runs its `ready` subcommand), but the Rust API has no `/health` endpoint. Adding one is a one-line change.
- **Credential management.** The `docker-compose.yml` contains hardcoded passwords suitable for local development. For any shared-server deployment (classroom, event), move secrets to a `.env` file excluded from version control, or use Docker secrets.
...
- **Legal compliance.** Three areas deserve attention before any deployment beyond localhost: (1) *Child privacy* — if minors are among your players, COPPA (US) and GDPR age-appropriate consent rules apply; (2) *Data residency* — personal data stored on your server may be subject to cross-border transfer restrictions under GDPR, Brazil's LGPD, or China's PIPL; (3) *Export control* — software using standard cryptography (JWT validation, OIDC token handling) falls under US EAR Category 5 Part 2, though published open-source code using standard algorithms is broadly exempt; LLM API providers may also impose their own geographic restrictions.
