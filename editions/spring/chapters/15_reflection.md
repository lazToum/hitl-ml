...
**1.** You have spent a semester — or a stretch of self-paced weeks — building this system. What is the most important thing you learned that you could not have learned from a textbook?
...
An observer watches the session stream. They see that three players are simultaneously stuck on clue 2. They send a nudge to the most-stuck player. Today that nudge becomes an `observer_nudge` event in the observer feed; a player-facing delivery channel would be the next step.
...
Built:
- A Rust API that compiles and handles the core game state machine
- A Python AI sidecar that generates clues, estimates difficulty, checks semantic matches, verifies photo answers, generates adaptive hints, and analyzes completed sessions
...
Left out (honestly, in rough priority order):
- **Wrong-answer capture** — the schema does not store what players guessed, only that they guessed. This is the highest-impact gap: without wrong answers, clue improvement is guesswork. See Chapter 3.
- **The calibration loop** — session data accumulates but nothing reads it to improve estimates
- **Broader tests** — Rust validator and hint unit tests exist, but API integration, sidecar contract, and behavioral HITL tests are still missing
- **Adaptive hints with real attempt history** — fallback hints are exposed, but wrong-answer strings are not stored and the request currently passes an empty attempts array
- **Redis beyond answer-attempt rate limiting** — no session cache or queue yet
...
**HITL systems and design**

- Robert Monarch, *Human-in-the-Loop Machine Learning* (Manning, 2021) — comprehensive guide to active learning, annotation quality, and HITL architecture.
- Eduardo Mosqueira-Rey et al., "Human-in-the-loop machine learning: a state of the art" (*Artificial Intelligence Review*, 2023) — survey of HITL methods and applications.
- Saleema Amershi et al., "Power to the People: The Role of Humans in Interactive Machine Learning" (*AI Magazine*, 2014) — foundational paper on interactive ML and human teachability.
- Thomas B. Sheridan, *Telerobotics, Automation, and Human Supervisory Control* (MIT Press, 1992) — the classic text on human supervisory control, still relevant to observer dashboard design.

**Automation and human factors**

- Lisanne Bainbridge, "Ironies of Automation" (*Automatica*, 1983) — the paper that established why automation doesn't eliminate the need for human skill.
- Burr Settles, *Active Learning Literature Survey* (UW-Madison TR 1648, 2009) — the definitive survey of query strategies, uncertainty sampling, and query-by-committee.

**The technologies we used**

- *The Rust Programming Language* (rust-lang.org/book) — the official Rust book, covering the ownership model, async, and the type system we relied on.
- *Axum* (docs.rs/axum) — the web framework; its extractor pattern and type-safe routing shaped the API design.
- *sqlx* (github.com/launchbadge/sqlx) — compile-time checked SQL queries; the `.sqlx/` offline cache is documented there.
- *Zitadel* (zitadel.com/docs) — the OIDC platform; their guide to project-level role management covers the setup we used.
- *Flutter* (flutter.dev) and *geolocator* (pub.dev) — for the mobile player app; the location permissions model and PKCE auth flow are documented in their respective guides.
...
**Autonomy.** The system acts autonomously in well-defined situations: validates answers against explicit rules, unlocks hints when thresholds are met, creates player accounts on first login, rejects invalid tokens, and rate-limits rapid answer attempts when Redis is available. AI is also on the action path: photo verification can pass or reject a photo clue, semantic matching can accept a text answer that string matching would reject, and adaptive hints can be generated after static hints are exhausted.

The lesson: the boundary of autonomy should be drawn at cases where incorrect automatic action would be harmful or hard to reverse. Automatic account creation is low-stakes — a wrong inference can be corrected later. Automatically marking a photo clue complete, or rejecting a valid photo, directly changes a player's run and is harder to unwind.

**Trust.** Creators trust the AI suggestions to the extent they read them carefully before approving. Players trust the hint engine to release hints when they are actually stuck, not just when a timer expires. Observers trust the event stream to be complete, even though dropped events are currently invisible to them — the broadcast channel reports lag (`RecvError::Lagged`), but the handler ends the stream on that error instead of surfacing it.
