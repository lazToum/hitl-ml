...
The Summer Edition asked you to *think* about human-in-the-loop systems. The Winter Edition asked you to *analyze* them rigorously. This one asks you to *build* one — from a blank directory to a running system that puts real humans in a real loop.
...
- A **creator** designs clues — labeling the world, deciding what counts as a correct answer, choosing how much help to offer.
...
- A **running local stack** provisioned with a single `docker compose up`, plus a one-time identity bootstrap (Chapter 5)
...
- Basic familiarity with Rust (you don't need to be an expert — the code is explained as it's written)
- Python 3.11+
- Docker + Docker Compose
- A terminal and a text editor

Optionally, for the Flutter chapters: Flutter SDK 3.19+ (Dart 3.3+).
...
ZITADEL_SA_PAT=<token> bash zitadel/setup.sh  # one-time identity bootstrap (Chapter 5)
cd backend-rust && cargo build
```

If `cargo build` succeeds in your environment and `docker compose ps` shows the services running, you are ready. The `zitadel/setup.sh` step runs once: it creates the OIDC clients, project roles, and dev users from Chapter 5 — without it, nobody can log in.

---

## Further reading

This edition is self-contained, but these works shaped its thinking:

- Robert Monarch, *Human-in-the-Loop Machine Learning* (Manning, 2021) — a practitioner's guide to active learning, annotation, and HITL system design.
- Lisanne Bainbridge, "Ironies of Automation" (*Automatica*, 1983) — the classic paper on why more automation can demand *more* human skill, not less.
- Saleema Amershi et al., "Power to the People: The Role of Humans in Interactive Machine Learning" (*AI Magazine*, 2014) — on making ML systems that humans can teach, correct, and steer.

A fuller list with domain-specific references is in Chapter 15.
