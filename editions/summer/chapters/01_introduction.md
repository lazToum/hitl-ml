...
Human-in-the-loop machine learning (HITL ML) is the practice of designing AI systems whose inclusion of human judgment is deliberate, structured, and ongoing — not a fallback when the machine fails, but a designed feature of how the system works.
...
*Core references: [@monarch2021human; @bainbridge1983ironies; @mosqueira2023hitl].*


<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/01_introduction.html` from the accompanying zip archive.
```
