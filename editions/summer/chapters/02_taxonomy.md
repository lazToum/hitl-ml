...
- The model was trained on human writing, but the humans who wrote those emails never asked to become training data. Were they in the loop?
...
<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/02_taxonomy.html` from the accompanying zip archive.
```
