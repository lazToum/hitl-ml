...
In a growing number of jurisdictions, oversight of algorithmic hiring decisions is becoming a legal requirement — New York City's Local Law 144 (enacted 2021, enforced since 2023), which mandates independent bias audits and candidate notice for automated employment decision tools, and the EU AI Act's human-oversight provisions are two prominent examples, and in the US, "disparate impact" under Title VII of the Civil Rights Act provides a legal framework for challenging biased outcomes even without intent. But it is more than compliance: it's the mechanism by which someone can ask: is the pattern the model learned a pattern worth keeping?

*Core references: [@barocas2019fairness; @bender2021stochastic; @gebru2021datasheets].*

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/15_ethics.html` from the accompanying zip archive.
```
