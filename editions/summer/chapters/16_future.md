...
Permissions in agentic AI systems need to distinguish between "allowed to draft" and "allowed to send," between "can access my calendar" and "can commit on my behalf." The harder question is whether that distinction can even be made in advance. The answer, so far, is: not reliably. Which is why the human needs to stay in the loop — not as a fallback, but as the person who decides what "permission" means in each new context. (GDPR Article 22 provides a right not to be subject to solely automated decisions producing legal effects; the EU AI Act extends similar oversight requirements to high-risk AI systems.)

*Core references: [@bommasani2021opportunities; @bowman2022measuring; @russell2019human].*

:::{admonition} Further reading
:class: tip
**Human Oversight as Adaptive Reserve** (companion position paper) develops a formal model of why systems optimized exclusively for automation efficiency may eliminate the capacity they need to recover from distributional shift — and proposes a framework for resilience-aware HITL design. Available in the [companion materials](https://what-if.io/companion/).
:::

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/16_future.html` from the accompanying zip archive.
```
