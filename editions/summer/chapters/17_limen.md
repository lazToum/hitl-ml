...
1. Think about the last time a piece of technology did something helpful without asking. Did its acting on its own make it more helpful, or would asking have made it better? Does the difference matter to you?
...
**Deliberate**: The morning routine model was built without the user ever intending to provide training data. Three weeks of behavioral traces — coffee time, news opens, calendar checks — were extracted, not offered. The feedback loop was never named as a feedback loop. (Even "local processing" can involve telemetry, crash reporting, and usage analytics that may transfer personal data off-device.)
...
*Core references: [@russell2019human; @bommasani2021opportunities].*

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/17_limen.html` from the accompanying zip archive.
```
