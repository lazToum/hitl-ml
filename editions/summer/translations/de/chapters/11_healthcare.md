...
```{seealso}
Galaxy Zoo Crowdsourcing: {cite}`lintott2008galaxy`. CheXNet Radiologieleistung: {cite}`rajpurkar2017chexnet`. Künstliche Intelligenz im Gesundheitswesen (Übersichtsartikel): {cite}`yu2018artificial`. Klinische NLP-Annotationsmethodik: {cite}`pustejovsky2012natural`. Für FDA KI/ML-Aktionsplan-Leitlinien, siehe die veröffentlichte FDA-Dokumentation (2021).
```
