...
+------------------------+
     |                ^
     | predictions    | feedback (labels,
     | and queries    | corrections, ...)
     v                |
+------------------------+
|         Human          |
+------------------------+
...
Human feedback is valuable but costly. A medical imaging annotation can cost anywhere from tens to hundreds of dollars per image when performed by a specialist, depending on the subspecialty and task complexity {cite}`monarch2021human`. Crowdworker labels on platforms like Amazon Mechanical Turk can cost as little as $0.01–$0.10 per item — median effective worker earnings on the platform are only about $2 per hour {cite}`hara2018data` — but quality is correspondingly variable. The fundamental challenge of HITL ML is **maximizing the value of each unit of human feedback**.
...
```{seealso}
For a practitioner-focused overview of the field, see {cite}`monarch2021human`. For the standard survey of active learning, see {cite}`settles2009active`; the formal foundations are {cite}`cohn1994improving` and {cite}`lewis1994sequential`.
```
