...
Entropy sampling is the most principled of the three — it considers all classes — and often performs comparably to margin sampling; it is a reasonable default on multiclass problems, though empirical differences are often small {cite}`lowell2019practical`.
...
Disagreement can be measured as **vote entropy** (entropy of the distribution of committee votes across classes) or **KL divergence** from the consensus distribution.
...
def mc_uncertainty(model, x, T=30):
    ...
    Returns mean prediction, epistemic, and aleatoric uncertainty.
    """
    ...
    # Total predictive uncertainty: entropy of the mean prediction
    total = -(mean_pred * (mean_pred + 1e-9).log()).sum(-1)
    # Aleatoric uncertainty: mean of per-pass entropies (expected data noise)
    per_pass_entropy = -(preds * (preds + 1e-9).log()).sum(-1)  # shape: (T, N)
    aleatoric = per_pass_entropy.mean(0)
    # Epistemic uncertainty: total minus aleatoric (model uncertainty)
...
In the untrained network above, both values are small in absolute terms, with the out-of-distribution example already showing somewhat higher epistemic uncertainty. After training, the gap typically widens — the model has not learned a reliable mapping for inputs far from the training distribution.
...
labels_used = np.arange(24) * 5 + 30  # label counts at each step
...
match = next((j for j, a in enumerate(accs_active) if a >= accs_random[-1]), None)
if match is not None:
    print(f"Active learning matches random's final accuracy with ~{int(labels_used[-1] - labels_used[match])} fewer labels")
else:
    print("Active learning does not reach random's final accuracy in this run")
```
...
- **Prediction stabilization:** Stop when the model's predictions on the unlabeled pool stabilize between consecutive rounds {cite}`bloodgood2009method`.
