...
No technique has done more to bring HITL ML into the mainstream than Reinforcement Learning from Human Feedback (RLHF). It is the mechanism behind InstructGPT {cite}`ouyang2022training` and a core component of instruction-following pipelines in many modern large language models. Understanding RLHF — not just as a recipe to follow, but as a principled approach to alignment — is essential for anyone working in modern AI.
...
For a set of prompts $\{x_i\}$, we generate $K$ responses per prompt using $\pi_\text{SFT}$ and present them to human annotators as pairwise comparisons: "Which response is better, A or B?"
...
**Proximal Policy Optimization (PPO)** {cite}`schulman2017proximal` was long the standard algorithm for this stage, chosen for its stability relative to raw policy gradient methods; group-relative variants (e.g., GRPO) and offline preference methods are now also widely used.
...
# "Quality" is known analytically (sum of components + 0.5 * mean of squared components)
...
A key failure mode: the policy finds ways to get high reward from the reward model that do not correspond to genuinely good behavior. For example, an LLM might learn to produce responses that are flattering or confidently worded (which annotators tend to rate highly) rather than accurate.
...
**Mitigation strategies:** the KL penalty (above); periodically retraining the reward model on fresh policy outputs; and evaluating on held-out and adversarially chosen prompts to detect overoptimization early.

### Evaluator Bias

Human annotators have systematic biases. They tend to prefer longer responses (verbosity bias), more confident-sounding text (confidence bias), and responses that agree with their prior beliefs. These biases propagate into the reward model.
...
For complex tasks, humans cannot reliably judge which AI response is correct. An annotator comparing two long mathematical proofs or two code implementations may simply pick the more readable one, regardless of correctness. **Scalable oversight** is the open research problem of designing evaluation procedures that remain reliable as task complexity grows {cite}`bowman2022measuring`.

---

## Constitutional AI and RLAIF

**Constitutional AI** {cite}`bai2022constitutional`, developed at Anthropic, reduces reliance on human annotators by using the AI itself to generate preference labels guided by a set of principles (a "constitution"). The process has two phases:
...
3. Revise responses guided by the AI feedback, and fine-tune the model on the revised responses (supervised learning)

**Phase 2 — RL from AI Feedback (RLAIF):**

4. Have the AI compare pairs of responses against the constitution, generating AI preference labels
5. Train a reward model on these AI-generated preferences and fine-tune the model with RL against it
