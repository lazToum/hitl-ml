...
When a task is difficult to specify but easy to demonstrate, it can be more efficient to teach by example than to define by rule. A human expert shows a robotic arm how to grasp an object; a programmer's interaction with their IDE provides a sequence of correct edits; a chess grandmaster's recorded games provide move-by-move examples of expert play. **Learning from demonstrations** extracts a policy from such behavioral data, avoiding the need for hand-crafted reward functions or explicit task specifications.
...
Crucially, errors **compound**: a small deviation leads to an unfamiliar state, where a slightly wrong action leads to an even more unfamiliar state, and so on. Performance degrades as $O(T^2 \epsilon)$ where $T$ is the episode length and $\epsilon$ is the error rate at each step — much worse than the $O(T\epsilon)$ cost of a learner trained on its own induced state distribution {cite}`ross2010efficient`.
...
**DAgger (Dataset Aggregation)** {cite}`ross2011reduction` addresses covariate shift directly: rather than training once on the expert's demonstrations, the learner iteratively runs its own policy in the environment, asks the expert to label the states it visits, and aggregates those new labels into the training set.

```text
Algorithm DAgger:
  Initialize: D <- {M expert demonstrations}
  Train initial policy pi_1 on D
...
DAgger achieves $O(T\epsilon)$ regret — matching the supervised-on-own-distribution bound rather than BC's $O(T^2\epsilon)$ — because the training distribution converges to match the deployment distribution.
...
GAIL achieves expert-level performance on continuous control benchmarks with far fewer demonstrations than BC, while avoiding the RL-in-the-loop cost of classical IRL and scaling to high-dimensional, continuous problems.
...
# Split before training: 900 sequences for training, 100 held out
train_dataset = TensorDataset(src_seqs[:900], tgt_in[:900], tgt_seqs[:900])
...
**Autonomous driving.** Early self-driving systems such as ALVINN {cite}`pomerleau1989alvinn` and NVIDIA's DAVE-2 relied heavily on behavioral cloning from human driving data.
