...
Each pairwise comparison provides information about the *relative* positions of two items on the quality scale. With $K$ items, $\Theta(K \log K)$ comparisons suffice to rank all items, and only $K-1$ comparisons are needed to find the top item. Absolute ratings typically require more judgments to achieve the same precision.
...
where $\sigma$ is the logistic sigmoid. This is equivalent to assuming that the perceived quality of item $i$ is $\alpha_i + \epsilon$ where $\epsilon$ is Gumbel noise; the difference of two Gumbel variables is logistic, which yields the sigmoid form above.
...
# Fix alpha[0] = 0 to resolve the translation ambiguity
result = minimize(
...
- **MergeRUCB:** Combines merge-sort-style tournament comparison with relative UCB

Dueling bandits are used in online ranker evaluation via interleaving (comparing two ranking systems by mixing their results) and in interactive preference elicitation for RLHF data collection.
