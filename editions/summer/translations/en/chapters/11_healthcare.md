...
Reader variability is well-documented in radiology. In the CheXNet study, four radiologists labeled the same pneumonia detection test set, and their individual F1 scores ranged from 0.36 to 0.44 {cite}`rajpurkar2017chexnet` — evidence of how much readings vary at the case level. For nodule detection on lung CT, intra-reader variability (same reader, same case, different day) can be as large as inter-reader variability.
...
- **Session design matters.** Medical annotation is cognitively demanding. Evidence from radiology and pathology links fatigue to increased error over long reading sessions and full workdays, though no specific time threshold is established and the evidence on countermeasures such as scheduled breaks remains thin {cite}`taylorphillips2019fatigue`. Annotation interfaces that enforce break prompts (without making them dismissable) are a simple HITL design decision with real quality impact.
...
label_counts = np.arange(len(aucs_al)) * 30 + 60
...
if pos_rate_rnd > 0:
    print(f"  AL queries {pos_rate_al/pos_rate_rnd:.1f}x more positives than random")
else:
    print("  Random baseline selected no positives in its 50 queries")
```

```{seealso}
Galaxy Zoo crowdsourcing: {cite}`lintott2008galaxy`. CheXNet radiologist performance: {cite}`rajpurkar2017chexnet`. Artificial intelligence in healthcare (review): {cite}`yu2018artificial`. Clinical NLP annotation methodology: {cite}`pustejovsky2012natural`. For FDA AI/ML action plan guidance, see the FDA's published documentation (2021).
```
