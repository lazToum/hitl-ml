...
**Amazon Mechanical Turk (MTurk)** is the original crowdsourcing marketplace, launched in 2005. Workers ("Turkers") complete micro-tasks (HITs) posted by requesters, often at very low effective hourly pay — an ethical concern taken up in Chapter 15. MTurk is best for simple tasks with clear, verifiable criteria.

**Prolific** is an academic crowdsourcing platform that enforces a minimum pay standard (£6/$8 per hour) and recommends at least £9/$12 per hour, per Prolific's published guidelines (as of 2026); it screens participants by demographics, and maintains a panel of workers who have opted in to research participation. Preferred for social science research and tasks requiring representativeness.
...
**Specialized communities.** For domain-specific tasks, communities of domain enthusiasts can provide high-quality annotations: Galaxy Zoo and other Zooniverse projects for astronomy, ecology, and the humanities; iNaturalist for community-verified species identification.
...
Embed **gold standard questions** — tasks with known correct answers — throughout the task batch. Workers whose accuracy on gold questions falls below a threshold are removed from the project.
...
For $p = 0.70$, adding a third annotator increases majority vote accuracy from 70% to 78%; for $p = 0.90$, the same addition yields a smaller absolute gain (90% to 97%) — still a roughly 70% relative reduction in error rate, so whether it is worth the cost depends on the price of an annotation.
