...
Knowing whether a HITL system is working requires more than measuring model accuracy. Evaluation has to establish whether the annotation budget is delivering value, whether the model is actually better aligned with human intent, and whether additional human feedback will continue to improve model performance. This chapter covers the full landscape of evaluation in HITL settings.
...
**AUROC** measures the model's ability to discriminate between classes regardless of threshold — useful when comparing models before an operating threshold is chosen. Note that AUROC is insensitive to calibration (see below): a rank-perfect but miscalibrated model still scores 1.0.
...
- **Human evaluation:** The gold standard. See *Human Evaluation* below.
...
CheckList makes human evaluation targeted and actionable: instead of a single accuracy number, it provides a capability profile. Tools such as Polyjuice {cite}`wu2021polyjuice` automate the generation of counterfactual test cases for this kind of probing.
...
**Reward model evaluation:** The reward model's accuracy on a held-out preference test set. Ouyang et al. {cite}`ouyang2022training` report roughly 72% pairwise accuracy for InstructGPT's reward model on preferences from labelers in the training distribution (about 70% for held-out labelers), though results vary widely by task and data quality.
...
**GPT-4 as evaluator:** Using a capable LLM to evaluate responses has become common for rapid iteration. Zheng et al. {cite}`zheng2023judging` find LLM evaluator agreement with human judgment ranging roughly from 0.7 to 0.9 depending on task — useful for rapid A/B comparison, but less reliable for detecting sycophancy, cultural nuance, or safety issues. (A related but distinct line of work — Gilardi et al. {cite}`gilardi2023chatgpt` — shows LLMs can outperform crowd workers on *annotation* tasks; that is a labeling result, not an evaluator-agreement one.)
