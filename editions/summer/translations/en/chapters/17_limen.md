...
At the foundation is **WID** (the Waldiez ID system, an envisioned component adapted for Limen's local-first architecture) — a causality-aware event tracking system that records not just what happened, but what caused it, and what it in turn caused.
...
**From the human's side of the loop, voice is the most natural output channel for most people.** It requires no training, no physical dexterity beyond ordinary speech, and no knowledge of the system's internal organization. A user who cannot locate a file in a directory hierarchy can describe what they're looking for.
...
1. **Local small model** (always first): fast, private, handles routine tasks — "open the file I was editing yesterday," "set a timer," "what's on my calendar today"
...
Limen addresses this through periodic consistency checks — the operating-system equivalent of the session consistency checks described in Chapter 5, where earlier examples are re-presented to the annotator to detect drift. The system surfaces historical decisions to the user: "A few weeks ago, you asked me to do X. Is that still what you'd want?" These checks serve two functions: they catch drift, and they remind the user of preferences they may have forgotten specifying.
...
**Part VI (Ethics and Horizons):** The local-first, privacy-preserving design is an ethical choice, not just a technical one. The human's data does not leave their device without their explicit consent. The model that learns from a user's behavior belongs to that user.
...
Not necessarily Limen specifically. The specific technology stack (Tauri, Rust, Whisper ONNX) is one choice among many. But the architecture — causal event tracking, local-first processing, graceful degradation, continuous preference learning, transparency as a first-class feature — follows from the principles.
