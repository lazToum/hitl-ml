...
*Ambiguïté des étiquettes.* De nombreux documents appartiennent à plusieurs catégories ou se situent à la frontière. Une critique à 60 % positive et 40 % négative est véritablement ambiguë, non mal étiquetée. Forcer une seule étiquette écarte de véritables informations (voir le Chapitre 13 sur les étiquettes souples et le désaccord des annotateurs).

*Granularité des étiquettes.* Un modèle de sentiment à 2 classes peut suffire pour la surveillance des médias sociaux ; une échelle à 7 points peut être nécessaire pour l'analyse des retours produits. La granularité appropriée dépend de la tâche en aval, mais est généralement fixée avant l'annotation — une décision de conception lourde de conséquences prise avec des données insuffisantes.
...
**HTER (taux d'édition de traduction à référence humaine) :** Une métrique qui mesure la distance d'édition entre la sortie de la TA et la traduction post-éditée, normalisée par la longueur de la phrase {cite}`graham2015accurate`. En règle générale praticienne approximative, un HTER inférieur à environ 0,3 est souvent considéré comme une bonne sortie de TA ; au-dessus de 0,5, la traduction à partir de zéro peut être plus rapide — bien que ces seuils varient selon le domaine et la paire de langues.
...
Il n'y a pas de solution propre. La meilleure pratique actuelle consiste à surveiller la dérive à l'aide de jugements de préférence de réserve (held-out) collectés à intervalles réguliers, et à traiter la version des consignes d'annotation comme une variable expérimentale.
