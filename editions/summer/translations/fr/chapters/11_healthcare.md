...
```{seealso}
Production participative Galaxy Zoo : {cite}`lintott2008galaxy`. Performances des radiologues CheXNet : {cite}`rajpurkar2017chexnet`. Intelligence artificielle en santé (article de synthèse) : {cite}`yu2018artificial`. Méthodologie d'annotation de TAL clinique : {cite}`pustejovsky2012natural`. Pour le plan d'action IA/ML de la FDA, consulter la documentation publiée par la FDA (2021).
```
