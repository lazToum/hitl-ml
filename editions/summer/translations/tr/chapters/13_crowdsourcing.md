...
Etiketleme görevleri uzman olmayan kişiler tarafından yapılabilecek kadar basit olduğunda, kitle kaynaklı platformlar düşük öğe başına maliyette büyük, talep üzerine etiketleme işgücüne erişim sağlar. Kalabalıktan yüksek kaliteli etiketlenmiş veri kümeleri oluşturmak, dikkatli görev tasarımı, stratejik yedeklilik ve titiz kalite kontrolü gerektirir.
...
**Appen** (ve benzerleri: Telus International, iMerit), daha karmaşık görevler ve kurumsal projeler için kullanılan kalite yönetimiyle yönetilen etiketleme işgücü sağlar.

**Özelleşmiş topluluklar.** Alana özgü görevler için alan meraklılarından oluşan topluluklar yüksek kaliteli etiketlemeler sağlayabilir: astronomi için Galaxy Zoo, kuş türleri için eBird, satranç pozisyonu etiketlemesi için Chess Tempo.
...
Karmaşık görevler basit, iyi tanımlanmış mikro görevlere ayrıştırılmalıdır. Çalışanlardan bir belgeyi kapsamlı biçimde etiketlemesini istemek yerine, her seferinde odaklı bir soru sorun: "Bu cümle bir kişinin adını içeriyor mu?" veya "Bu çevirinin anlaşılırlığını 1–5 ölçeğinde değerlendirin."
...
Çoğunluk oyu, doğal bir taban çizgisidir ancak etiketleyici doğruluğundaki farklılıkları görmezden gelir. İstatistiksel modeller daha iyi sonuç verebilir.

### Dawid-Skene Modeli

**Dawid-Skene (DS) modeli** {cite}`dawid1979maximum` şunları birlikte tahmin eder:
- Her $i$ öğesi için **gerçek etiket** $z_i$
- Her $j$ etiketleyicisi için **karışıklık matrisi** $\pi_j^{(k,l)}$: $j$ etiketleyicisinin gerçek sınıfı $k$ olan bir öğeyi $l$ sınıfı olarak etiketleme olasılığı

EM algoritması şunlar arasında yinelenir:
- **E adımı:** Etiketleyici karışıklık matrisleri göz önüne alındığında, her gerçek etiketin arka plan olasılığını hesapla
- **M adımı:** Öğe etiket tahminleri göz önüne alındığında, etiketleyici karışıklık matrislerini güncelle
...
**MACE (Çok Etiketleyicili Yeterlik Tahmini)** {cite}`hovy2013learning`, etiketleyici spam göndermeyi (rastgele etiketleme) ile yetkin etiketlemeyi açıkça temsil eden alternatif bir olasılıksal modeldir. Bir etiketleyici ya anlamlı bir etiket sağlar ($1 - \text{spam}_j$ olasılığıyla) ya da rastgele bir etiket sağlar ($\text{spam}_j$ olasılığıyla). Bu iki bileşenli model, bazı etiketleyicilerin tamamen spam gönderdiği kitle kaynaklı çalışma senaryoları için çoğunlukla Dawid-Skene'den daha iyi kalibre edilmiştir.

---

## Yedeklilik ve Birleştirme Stratejisi

Öğe başına en uygun etiketleyici sayısı, görev güçlüğüne ve etiketleyici kalitesine bağlıdır:

- **Yetenekli etiketleyicilerle kolay görevler:** Öğe başına 1–2 etiketleyici çoğunlukla yeterlidir
- **Eğitimli etiketleyicilerle orta görevler:** 3 etiketleyici + çoğunluk oyu
- **Kitle çalışanlarıyla güç/öznel görevler:** 5–7 etiketleyici + Dawid-Skene

Temel kavrayış: yedeklilik, etiketleyici doğruluğu düşük olduğunda en değerlidir. $p$ doğruluklu etiketleyicilerle $n$ etiketleyiciyle çoğunluk oyu doğruluğu şöyledir:

$$
P(\text{ÇO doğru}) = \sum_{k > n/2}^{n} \binom{n}{k} p^k (1-p)^{n-k}
$$

$p = 0{,}70$ için üçüncü bir etiketleyici eklemek çoğunluk oyu doğruluğunu %70'ten %78'e çıkarır; $p = 0{,}90$ için üçüncü bir açırlayıcıdan elde edilen kazanım ihmal edilebilir düzeydedir (%90'dan %97'ye).
