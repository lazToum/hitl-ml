...
Döngüde insan makine öğrenmesi alanı hızla değişmektedir. Temel modeller, etiketleyici olarak büyük dil modelleri ve insan–yapay zeka işbirliği için yeni paradigmalar, HITL'nin ekonomisini ve pratiğini beş yıl önce hayal dahi edilemeyecek biçimlerde dönüştürmektedir. Bu son bölüm, ortaya çıkan yönlerin ve açık sorunların haritasını çıkarmaktadır.

---

## Temel Modeller ve Etiketlemenin Değişen Rolü
...
**Çıkarım:** Etiketleme çabasının yatırım getirisi değişiyor. Etiketleme çabası artık şunlara yönlendirilmek için daha iyi değerlendirilmektedir:
...
Taban model zaten dili, kodu ve sağduyuyu anladığında, kullanıcılar etiketlenmiş örnekler yerine doğal dille iletişim kurabilir. Bir metin sınıflandırıcı isteyen kullanıcı artık 500 örnek etiketlemek zorunda değildir — sınıflandırma görevinin bir etiketlemesini yazabilir ve sıfır atışlı performansı değerlendirebilir.

Bu durum, HITL güçlüğünü *örnek toplama*dan *görev belirleme*ye kaydırır: kullanıcıların modelin üzerinde eylemde bulunabileceği bir biçimde tam olarak ne istediklerini dile getirmelerine yardımcı olmak. Bu kulağa geldiğinden zordur — görevlerin doğal dil etiketlemeleri, etiketlenmiş örneklerin sahip olmadığı biçimlerde çoğunlukla belirsizdir.

---

## Etiketleyici Olarak Büyük Dil Modelleri

2023–2025'in en önemli gelişmelerinden biri, **BDM'lerin etiketleme kahinleri olarak kullanılmasıdır**. "Bu yorum olumlu mu olumsuz mu?" sorusunu bir insana değil GPT-4 veya Claude'a soruyoruz. İyi tanımlanmış sınıflandırma görevleri için BDM etiketlemeleri çoğunlukla kitle çalışanı doğruluğuyla eşleşiyor veya ona yaklaşıyor {cite}`gilardi2023chatgpt` ve API çağrılarının etiketleme başına maliyeti tipik olarak insan işgücü ücretlerinden kat kat düşük.

### BDM Etiketlemesinin İyi Çalıştığı Yerler
...
- Bağlamlar arasında tutarlı etiketlemenin sağlandığı görevler (yüksek öznellik yoktur)
- İnsan incelemesi için ilk aşama etiketlemeler oluşturma

### BDM Etiketlemesinin Başarısız Olduğu Yerler
...
**Sınır yönelimler:**
- **BDM artırımlı etiketleme işlevleri:** Doğal dil etiketlemelerinden etiketleme işlevleri oluşturmak için BDM'leri kullanma
- **Dilim tabanlı öğrenme:** Etiketleme işlevlerinin anlaşmazlığa düştüğü kritik veri alt kümelerini (dilimleri) belirleyin ve insan etiketlemesini oraya yönlendirin
...
Yapay zeka sistemleri çok kipli hâle geldikçe — metin, görüntü, ses ve videoyu aynı anda işleyip ürettikçe — etiketleme daha karmaşık hâle geliyor. Tek bir içerik parçası, aralarında bağımlılıklar bulunan kipler genelinde etiketleme gerektirebilir.

**Ortaya çıkan görev türleri:**
- Video + transkript etiketlemesi (ne oluyor, kim konuşuyor, metin görsel olarak neyi tanımlıyor?)
- Tıbbi görüntü + klinik rapor etiketlemesi
- Robotik yörünge etiketlemesi (sensör verilerini eylem dizileriyle ilişkilendirme)

Çok kipli temel modeller (GPT-4V, Gemini, Claude) buradaki etiketleme manzarasını da değiştiriyor — modeller artık görüntüleri yorumlayıp aday etiketlemeler üretebiliyor; insanlar ise bunları inceliyor.
...
Protein yapı tahmini, hukuki analiz veya matematiksel kanıt doğrulama gibi görevler için uzman etiketleyiciler bile iki yapay zeka çıktısından hangisinin doğru olduğuna güvenilir biçimde karar veremeyebilir. Değerlendirilen modelden insan yargısı daha az güvenilir olduğunda mevcut HITL yöntemleri işe yaramaz.
...
**İzlenecek eğilimler:**
- **Yapay zeka destekli etiketleme:** Yapay zeka önerir; insanlar karar verir. Yapay zeka daha iyi seçenekler önerdikçe kalite artar.
...
- **Ölçekte tercih öğrenmesi:** Kullanıcıların yapay zeka çıktılarıyla nasıl etkileşime girdiğine ilişkin örtük sinyaller, açık etiketleme oturumları gerektirmeden sürekli tercih öğrenmesini besler.
...
1. **Aktif öğrenmede optimum durdurma:** Bir sonraki etiketlemenin marjinal değeri maliyetin altında kaldığında? Prensipli durdurma kuralları hâlâ zordur.

2. **Görevler arasında etiketleme bütçesi tahsisi:** Çok görevli ortamlarda, sabit bir etiketleme bütçesi görevler arasında nasıl bölünmelidir?
...
6. **Etiketleyici modelleme:** Yalnızca yetkinliği değil sistematik önyargıları, konu uzmanlığını ve yorgunluğu da yakalayan daha iyi istatistiksel etiketleyici davranış modelleri.

7. **Hizalamanın değerlendirilmesi:** Bir modelin insan değerleriyle hizalandığına ilişkin temel gerçek testlerimiz yok. Sağlamlık için karşıt örneklere benzer bu tür testler geliştirmek açık bir sorundur.

8. **Adil veri emeği:** Etiketleme çalışanlarının adil biçimde ücretlendirilmesini ve korunmasını sağlarken büyük ölçekli etiketlemenin maliyet etkinliğini koruyan ekonomik ve kurumsal yapılar.
...
Bu el kitabındaki araçlar ve teknikler — etiketleme, aktif öğrenme, RLHF, tercih öğrenmesi ve geri kalanı — bu amaca ulaşmanın araçlarıdır. Alan geliştikçe belirli teknikler değişecek. Temel ilham — hem yetenekli hem de gerçek anlamda insan niyetiyle hizalanmış sistemler inşa etmek — kalıcı olacaktır.

```{seealso}
Ölçeklenebilir denetim ve tartışma: {cite}`bowman2022measuring`. Snorkel zayıf denetimi: {cite}`ratner2017snorkel`. İnsan–yapay zeka işbirliğinin geniş geleceği için bkz. {cite}`amershi2014power`.
```
