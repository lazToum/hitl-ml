...
\begin{itemize}
    ...
    \item Mathematically: $\sigma^2_{\text{aleatoric}} = \mathbb{E}_{y \sim p(y|x)}\bigl[(y - \mathbb{E}[y \mid x])^2\bigr] = \mathrm{Var}(y \mid x)$
\end{itemize}
...
\begin{itemize}
    ...
    \item Mathematically: $\sigma^2_{\text{epistemic}} = \mathbb{E}_{\theta}\bigl[(f_\theta(x) - \mathbb{E}_{\theta}[f_\theta(x)])^2\bigr]$
\end{itemize}

\subsection{Decision-Theoretic Framework for Human Intervention}
\label{subsec:decision-theoretic}
...
\[
p^* = \frac{C_{\text{stream}}}{C_{\text{interrupt}} + C_{\text{stream}}}
\]
...
Result: Too few interventions; silent errors accumulate
...
def predict_with_uncertainty(model, x, n_samples=100):
    ...
    for _ in range(n_samples):
        model.train()  # Enable dropout for MC sampling (also re-enables BatchNorm updates — for production, toggle only dropout layers)
        pred = model(x)
...
The ``asking for help'' examples from Chapter~1 relate to the active learning literature \autocite{settles2009active}. Throughout, $\mathcal{U}$ denotes the pool of unlabeled candidate instances.

\paragraph{Uncertainty Sampling.}
\[
x^* = \operatorname{argmax}_{x \in \mathcal{U}}\, H\bigl(p(y \mid x)\bigr) \quad \text{(maximum entropy)}
\]

\paragraph{Query by Committee.}
\[
x^* = \operatorname{argmax}_{x \in \mathcal{U}}\, \text{Disagreement}(\text{Committee},\, x)
\]

\paragraph{Expected Error Reduction.}
\[
x^* = \operatorname{argmin}_{x \in \mathcal{U}} \sum_{y} p(y \mid x)\, \text{Loss}_{\text{expected}}(\text{model}_{\text{new}},\, y)
\]
...
\[
\text{Query}^* = \operatorname{argmax}_{Q}\, I\bigl(Y;\, H_{\text{response}} \mid \text{Query},\, X\bigr)
\]
...
Each candidate action has its own expected cost:
\begin{align*}
\text{Cost}(\text{intervene at } t) &= \beta \cdot P(\text{user\_present} \mid t) \cdot \text{InterruptionCost} \\
\text{Cost}(\text{continue streaming}) &= \alpha \cdot P(\text{user\_absent} \mid t) \cdot \text{StreamingCost}(t)
\end{align*}

The optimal intervention time $T^*$ is the first $t$ at which intervening becomes cheaper than continuing to stream---the same indifference condition behind the $p^*$ threshold derived above.
...
\subsection{Illustrative Performance Estimates}

\begin{technote}[Hypothetical Metrics Based on Streaming-Absence Modeling]
\begin{itemize}
...
The system chooses a mode $D \in \{\text{Home},\, \text{Away}\}$ (the decision) while the true occupancy $O$ (the uncertain state of the world) is known only through the sensor evidence above. Each candidate mode carries an expected cost:
\[
\begin{aligned}
\text{Cost}(D{=}\text{Away}) = {} & \alpha \cdot P(O{=}\text{Home} \mid m, g, t, s, w) \cdot \text{ComfortCost} \\
\text{Cost}(D{=}\text{Home}) = {} & \beta \cdot P(O{=}\text{Away} \mid m, g, t, s, w) \cdot \text{EnergyCost}
\end{aligned}
\]
Declaring Away when the house is occupied pays the comfort cost (a cold house); holding Home when the house is empty pays the energy cost (heating an empty house). The system picks the mode with the lower expected cost.
...
class ImprovedNestLearner:
    def __init__(self):
        ...
        self.user_patterns = {}
        self.ask_threshold = 0.6  # initial "ask" tier (see improved_nest_decide)
        self.act_threshold = 0.95  # initial "act" tier
    ...
    def update_from_correction(self, predicted, actual, context):
        ...
        if self.false_positive_count > 5:
            # False Away-acts: require more confidence before acting
            self.act_threshold = min(0.99, self.act_threshold + 0.01)
\end{lstlisting}
...
\begin{itemize}
    \item \textcite{mosqueira2023hitl} --- Human-in-the-loop machine learning: a state of the art. \emph{Artificial Intelligence Review}.
...
\begin{itemize}
    ...
    \item Autonomous Systems: \textcite{michelmore2020uncertainty} --- Uncertainty quantification with statistical guarantees in end-to-end autonomous driving control. \emph{ICRA}.
\end{itemize}

\subsection{Smart Home \& IoT}
\begin{itemize}
    \item \textcite{yang2013learning} --- Learning from a learning thermostat: lessons for intelligent systems for the home. \emph{UbiComp}.
    \item \textcite{brush2011home} --- Home automation in the wild: challenges and opportunities. \emph{CHI}.
\end{itemize}
...
Using the decision-theoretic framework from Section~\ref{subsec:decision-theoretic}, design an improved Home/Away Assist system that:
\begin{itemize}
