...
\[
\sigma^2_{\text{aleatoric}} = \mathbb{E}_{y \sim p(y|x)}\bigl[(y - \mathbb{E}[y \mid x])^2\bigr] = \mathrm{Var}(y \mid x)
\]

This represents the expected squared deviation between the true output and the conditional mean $\mathbb{E}[y \mid x]$, averaged over the true output distribution. It cannot be reduced by collecting more data, because it reflects genuine ambiguity in the relationship between $x$ and $y$.
...
\begin{enumerate}
    \item Partition predictions into $M$ confidence bins $B_m = [(m-1)/M,\, m/M)$ for $m = 1,\ldots,M$
...
A high ECE means the model's confidence scores cannot be trusted to route cases correctly. For example, a model whose 90\%-confidence bin has only 70\% accuracy will dramatically under-request human oversight on the cases it's least reliable about.
\end{cautionbox}
...
Let $\tau$ be the confidence threshold. The system auto-diagnoses if confidence $\geq \tau$; otherwise routes to human review. Define:
\begin{itemize}
    \item $\alpha(\tau)$: error rate on auto-routed cases (decreasing in $\tau$)
    \item $\beta(\tau) = P(\text{confidence} < \tau)$: fraction routed to human review (increasing in $\tau$)
...
\[
\tau^* = \arg\min_\tau \Bigl[ C_E \cdot \alpha(\tau) \cdot (1 - \beta(\tau)) + C_R \cdot \beta(\tau) \Bigr]
\]

In medical imaging, $C_E \gg C_R$, so $\tau^*$ is high. The AI at 51\% correctly placed the case in the review queue, since any medically reasonable $\tau^* > 0.51$ would route it to the radiologist.
...
\begin{enumerate}
    ...
    \item Sweep threshold $\tau \in [0.5, 0.99]$ and compute expected total cost.
    \item Find $\tau^*$.
    \item Compare accuracy at $\tau^*$ vs.\ full automation vs.\ full human review.
\end{enumerate}
\end{Exercise}
