...
\[
\mathrm{gap}(\theta) = R(\theta) - \hat{R}(\theta)
\]

For parameters $\theta$ fitted by ERM, this gap is non-negative \emph{in expectation}---the empirical risk is an optimistically biased estimate of the true risk, since the same data are used to fit the model and to score it---and its expected size tends to grow with model complexity relative to dataset size. (For a single \emph{fixed} $\theta$ chosen independently of the data, the empirical risk is unbiased and the gap can fall on either side of zero.)
...
For a hypothesis class $\mathcal{H}$ with VC dimension $d_{\mathrm{VC}}$, with probability $1 - \delta$ \autocite{vapnikchervonenkis1971uniform}:

\[
R(\theta) \leq \hat{R}(\theta) + \sqrt{\frac{8}{n}\!\left(d_{\mathrm{VC}} \cdot \left(\ln\frac{2n}{d_{\mathrm{VC}}} + 1\right) + \ln\frac{4}{\delta}\right)}
\]
...
A tighter bound for stochastic classifiers \autocite{mcallester1999pacbayes}:
...
A kernel-based two-sample test for whether two samples are drawn from the same distribution \autocite{gretton2012kernel}:
...
Probably Approximately Correct (PAC) learning, introduced by Valiant, formalizes when learning is feasible \autocite{valiant1984theory}.

A concept class $\mathcal{C}$ is PAC learnable if there exists an algorithm $\mathcal{A}$ such that for any $\varepsilon, \delta > 0$ and any distribution $P$ over $\mathcal{X}$, given $m \geq \mathrm{poly}(1/\varepsilon,\, 1/\delta,\, d,\, \mathrm{size}(c))$ examples (with $d$ the dimension of the instance space), with probability $\geq 1 - \delta$:

\[
R\!\left(\mathcal{A}(\mathcal{D}_m)\right) \leq \varepsilon
\]

provided the labels are generated by some concept $c \in \mathcal{C}$ (the \emph{realizable} case). In the \emph{agnostic} case, where $P$ is an arbitrary distribution over $\mathcal{X} \times \mathcal{Y}$, no algorithm can guarantee error below $\varepsilon$ in general; the guarantee weakens to $R\!\left(\mathcal{A}(\mathcal{D}_m)\right) \leq \min_{c \in \mathcal{C}} R(c) + \varepsilon$, with sample complexity scaling as $1/\varepsilon^2$ rather than $1/\varepsilon$.

\begin{keyconcept}[PAC Theory and Rare-Category HITL]
Learning requires a finite number of labeled examples proportional to $1/\varepsilon$ in the realizable case ($1/\varepsilon^2$ in the agnostic case). For very rare categories (low prior probability), the sample required to learn them grows rapidly. Rare-category detection is a natural HITL application: human review of uncertain cases on rare categories is often the only practical way to achieve acceptable error rates.
\end{keyconcept}
