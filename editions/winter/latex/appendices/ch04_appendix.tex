...
For a binary classifier with scores $s(x) \in [0,1]$, sweeping the threshold $\tau \in [0,1]$ traces the Receiver Operating Characteristic (ROC) curve \autocite{fawcett2006roc}:
...
\label{sec:roc-optimal-threshold}
...
For a calibrated score $s(x) = P(y=1 \mid x)$, setting $d\,\mathbb{E}[\mathrm{Cost}(\tau)]/d\tau = 0$ gives the optimal threshold:

\[
\tau^* = \frac{C_{\mathrm{FP}}}{C_{\mathrm{FP}} + C_{\mathrm{FN}}}
\]

The class priors $P(y=0)$ and $P(y=1)$ do not appear: a calibrated posterior already incorporates them, so the threshold depends only on the cost ratio. (This is why \texttt{optimal\_decision} below thresholds on $c_{\mathrm{fp}}/(c_{\mathrm{fp}}+c_{\mathrm{fn}})$ directly.)

\begin{itemize}
    \item For equal costs ($C_{\mathrm{FN}} = C_{\mathrm{FP}}$): $\tau^* = 0.5$
...
AUC is dominated by performance on the majority class. For imbalanced problems (rare positive class), precision-recall analysis is more informative \autocite{davis2006relationship}; \cref{fig:roc-pr} contrasts the two views of the same classifier.
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}
\begin{axis}[hitlaxis, name=roc,
    xlabel={false positive rate}, ylabel={true positive rate},
    xmin=0, xmax=1, ymin=0, ymax=1, xtick={0,0.5,1}, ytick={0,0.5,1}, title={ROC}]
\addplot[dashed, hitlgray, line width=0.8pt, forget plot] coordinates {(0,0)(1,1)};
\addplot[hitlblue, line width=1.4pt, smooth] coordinates
  {(0,0)(0.03,0.30)(0.08,0.52)(0.11,0.60)(0.15,0.68)(0.28,0.82)(0.45,0.91)(0.7,0.97)(1,1)};
\addplot[hitlorange, only marks, mark=*, mark size=2.3pt] coordinates {(0.11,0.60)};
\node[hitlorange!80!black, font=\scriptsize, anchor=west] at (axis cs:0.15,0.55)
  {operating point};
\end{axis}
\begin{axis}[hitlaxis, at={($(roc.east)+(1.6cm,0)$)}, anchor=west,
    xlabel={recall}, ylabel={precision},
    xmin=0, xmax=1, ymin=0, ymax=1, xtick={0,0.5,1}, ytick={0,0.5,1},
    title={Precision--Recall}]
\addplot[hitlblue, line width=1.4pt, smooth] coordinates
  {(0,0.77)(0.3,0.77)(0.52,0.68)(0.6,0.645)(0.68,0.60)(0.82,0.49)(0.91,0.40)(0.97,0.32)(1,0.25)};
\addplot[hitlorange, only marks, mark=*, mark size=2.3pt] coordinates {(0.6,0.645)};
\end{axis}
\end{tikzpicture}}
\caption{ROC and precision--recall views of the same classifier on a test set with 25\% positive prevalence. The diagonal on the ROC plot is chance performance; the marked operating point is the threshold chosen for deployment. On an imbalanced problem the precision--recall curve exposes the precision cost of raising recall that the ROC view can mask.}
\label{fig:roc-pr}
\end{figure}
...
This system achieves high precision in the automated band (only scores above $\tau_{\mathrm{high}}$ are automatically positive) at the cost of sending the uncertain band to humans. Optimizing the band width trades reviewer workload against precision/recall tradeoffs.
...
Most ML frameworks allow cost-sensitive training via class weights \autocite{elkan2001cost}. For a binary classifier with $C_{\mathrm{FN}} \neq C_{\mathrm{FP}}$:
...
def optimal_decision(score, c_fp, c_fn, margin=0.1):
    ...
    # Note: margin is a design parameter, not derived from the cost ratio —
    # it trades reviewer workload against the precision of automated decisions.
...
\label{sec:three-way-classification}
...
A fraud detection system achieves AUC~$= 0.94$. The base rate of fraud is 0.2\%, $C_{\mathrm{FN}} = \$500$ (average fraud amount), $C_{\mathrm{FP}} = \$2$ (customer service cost of false decline). Compute the optimal threshold $\tau^*$ using the formula from \cref{sec:roc-optimal-threshold}. Then explain why the false positive rate at $\tau^*$ cannot be determined from AUC alone, and what additional information you would need to estimate it. What does this imply for the operational feasibility of deploying at $\tau^*$?
\end{Exercise}
...
Implement the three-way classification system from \cref{sec:three-way-classification} in Python. Create a simulation where you vary the margin parameter and observe how the review rate, false negative rate, and false positive rate change. Find the margin that minimizes total cost for a given cost structure.
\end{Exercise}

\begin{Exercise}[ROC vs.\ Precision-Recall on Imbalanced Data]
Draw the ROC curve and precision-recall curve for the same model on an imbalanced dataset (1\% positive class). Identify the operating point on each curve that corresponds to the optimal threshold from \cref{sec:roc-optimal-threshold}. Why does the ROC curve look more favorable than the PR curve for this scenario?
\end{Exercise}
