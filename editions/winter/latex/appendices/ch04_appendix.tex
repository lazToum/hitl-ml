...
\addplot[hitlblue, line width=1.4pt, smooth] coordinates
  {(0,0.98)(0.2,0.96)(0.4,0.92)(0.6,0.85)(0.75,0.74)(0.88,0.58)(0.96,0.40)(1,0.25)};
\addplot[hitlorange, only marks, mark=*, mark size=2.3pt] coordinates {(0.6,0.85)};
\end{axis}
\end{tikzpicture}}
\caption{ROC and precision--recall views of the same classifier. The diagonal on the ROC plot is chance performance; the marked operating point is the threshold chosen for deployment. On an imbalanced problem the precision--recall curve exposes the precision cost of raising recall that the ROC view can mask.}
\label{fig:roc-pr}
\end{figure}
...
A fraud detection system achieves AUC~$= 0.94$. The base rate of fraud is 0.2\%, $C_{\mathrm{FN}} = \$500$ (average fraud amount), $C_{\mathrm{FP}} = \$2$ (customer service cost of false decline). Compute the optimal threshold $\tau^*$ using the formula from \cref{sec:roc-optimal-threshold}. At this threshold, what is the expected false positive rate? Is this operationally feasible?
\end{Exercise}
