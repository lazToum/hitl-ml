...
\paragraph{Optimal ask criterion.} Ask a human when the expected value of the information gained exceeds the cost of asking:

\[
V_\text{decision} \cdot \text{IG}(H;\, Y \mid X) > C_\text{ask}
\]

where $C_\text{ask}$ is the cost (time, attention, trust) and $V_\text{decision}$ is the value of resolving uncertainty about the decision, per bit. Since IG is measured in bits, both sides are in the same units; equivalently, ask when $\text{IG}(H;\, Y \mid X) > C_\text{ask}/V_\text{decision}$.

\subsection{Query Optimisation}

The \term{optimal query} $Q^*$ maximizes information gain subject to cognitive load constraints:
...
A simpler query (lower cognitive load) is only preferable if its information loss is proportionally small. The 911~example from Chapter~5 illustrates the flip side: a one-word change (``breathing'' to ``breathing \emph{normally}'') added almost nothing to the caller's effort, yet sharply increased the information the question extracted---a negligible cost for a large gain.
\end{examplebox}
...
Following Sweller's cognitive load theory \autocite{sweller1988cognitive}, response accuracy degrades beyond working memory capacity $W$:
...
For most practical settings $k^* \in [2,5]$, depending on the complexity exponent $\beta$. At the $\beta \approx 0.6$--$0.8$ typical of simple perceptual tasks, 4--5 choices represent the practical optimum. At higher $\beta$ (e.g., $\beta=1.5$ as in the exercise below), $k^* = 2$ — binary questions dominate.
...
Each human response is a labeled data point. Select the next query to maximize expected model improvement:
...
Optimal policy $\pi^*$ maximizes $\mathbb{E}[\sum_t \gamma^t r_t]$, capturing the tension between short-run accuracy and long-run trust.
