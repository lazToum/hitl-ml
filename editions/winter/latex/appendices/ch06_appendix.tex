...
Following \textcite{sweller1988cognitive} and \textcite{paas1994variability}, we identify three components of cognitive load relevant to \hitl interface design:
...
\begin{table}[htbp]
\caption{Annotation quality vs.\ interface design (illustrative values)}
...
\[
\alpha(f, c) = \alpha_0 \cdot e^{-\lambda_f f} \cdot e^{-\lambda_c c}
\]
...
As an illustrative range (in the spirit of the tables above), values of $\alpha \in [0.3, 0.7]$ are plausible under standard conditions; the decay constants $\lambda_f, \lambda_c$ must be estimated for the deployment setting.
...
Studies of cockpit automation report the same direction of effect: operators prompted to commit to an independent, accountable judgment before consulting the automation verify it more often and commit fewer automation-bias errors than those shown the recommendation first \autocite{mosier1998automation}.
...
\paragraph{Stability.}
\[
\text{Stability}(E, f, x) = \exp\Bigl(-\mathbb{E}_{\epsilon}\bigl[\|E(f, x + \epsilon) - E(f, x)\|_2\bigr]\Bigr)
\]
The exponential kernel keeps the metric in $(0, 1]$: identical explanations score 1, and increasingly unstable ones approach 0.

Illustrative magnitudes: LIME stability scores around 0.4--0.6; SHAP stability scores around 0.7--0.85 for the same models.
...
\begin{table}[htbp]
\caption{Human utility of explanation types (illustrative values)}
...
As an illustrative magnitude, $\Delta_{\text{order}} \in [0.05, 0.15]$ in favor of human-first sequencing is a plausible range, particularly when the AI recommendation is wrong; the actual effect size must be measured in the deployment setting.
...
\[
q(n) = (q_0 - q_{\text{floor}}) \cdot e^{-\lambda_q n} + q_{\text{floor}}
\]
where $q_0$ is the fresh-start quality (so $q(0) = q_0$), $q_{\text{floor}}$ is the asymptotic floor reached under fatigue, and $\lambda_q$ is the decay rate.
...
Take any annotation or AI-assisted review interface. Count the number of clicks required to: (a) confirm the AI recommendation, (b) override the AI recommendation, (c) escalate to senior review. Calculate $C_{\text{disagree}} / C_{\text{agree}}$ (the ratio $k$ from the disagree-facilitation model above). What does this ratio imply about the interface's design assumptions regarding override frequency?
\end{Exercise}
