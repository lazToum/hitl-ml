...
As an absolute minimum, require $N \geq 30$ items per annotator pair for the estimate to be computable and roughly unbiased. Confidence-interval quality at that sample size is poor (at $P_o = 0.8$, $P_e = 0.5$, the 95\% half-width is about $\pm 0.29$); for tighter margins use the sample-size formula in \S\ref{sec:ch07-power} below.
...
\[
P(z_i = k \mid \mathbf{y}_i) \propto p_k \prod_{j \in \mathcal{J}_i} \prod_l \pi_{jkl}^{\mathbf{1}[y_{ij}=l]}
\]
where $\mathcal{J}_i$ is the set of annotators who labeled item $i$.

\paragraph{M-step.} Update annotator error rates and class priors:
\[
\hat{\pi}_{jkl} = \frac{\sum_{i \in \mathcal{I}_j} P(z_i=k) \cdot \mathbf{1}[y_{ij}=l]}{\sum_{i \in \mathcal{I}_j} P(z_i=k)}
\qquad
\hat{p}_k = \frac{1}{N} \sum_i P(z_i = k)
\]
where $\mathcal{I}_j$ is the set of items labeled by annotator $j$, matching the restricted sums in the code below.
...
def dawid_skene_em(annotations, n_classes, n_iters=50):
    ...
    # Initialize T with majority vote (ties → uniform; EM resolves them)
    T = np.zeros((N, K))
    for i, item in enumerate(items):
        for v in annotations[item].values():
            T[i, v] += 1
        T[i] /= T[i].sum()  # one-hot for single-annotation items; a row with no votes would be NaN
...
where $\kappa_1$ is single-annotator kappa and $\lambda \approx 0.3$. Increasing redundancy beyond $n_r = 5$ yields diminishing returns; under this model the marginal quality gain from a 6th annotator is $(1-\kappa_1)(e^{-1.2} - e^{-1.5}) \approx 0.08\,(1-\kappa_1)$, i.e., roughly 0.02--0.04 kappa for typical $\kappa_1 \in [0.5, 0.75]$ (dropping below 0.01 only when $\kappa_1 \gtrsim 0.87$).
