...
A \term{HITL Loop} is a directed graph $G = (V, E)$ where: $V$ includes at least one human review node $H$; and a path exists from $H$ back to some component that influences future model behavior.
...
class ThreeZoneRouter:
    ...
    def route(self, score: float,
        ...
        if stakes is not None and stakes > 0.8:
            widening = 0.15  # high stakes: send more cases to humans
            eff_high = min(self.tau_high + widening, 1.0)
            eff_low  = max(self.tau_low  - widening, 0.0)
        if score >= eff_high:
    ...
    def adjust_for_capacity(self):
        ...
        if capacity < 0.3:   # Overloaded: narrow the review band
            min_band = 0.05  # never collapse the band entirely
            self.tau_low  = min(self.tau_low  + self.config.overflow_narrowing,
                                self.tau_high - min_band)
            self.tau_high = max(self.tau_high - self.config.overflow_narrowing,
                                self.tau_low + min_band)
...
The \texttt{stakes} parameter implements a high-stakes override: when a case is flagged as high-stakes (e.g., a medical decision, a financial transaction above a threshold), the review band widens, so that even moderately confident scores fall into the human-review zone. This pushes high-consequence cases toward human review regardless of reviewer capacity.
\end{examplebox}
...
Cases routed for human review should be prioritized not by arrival order but by \emph{learning value}: the expected improvement to the model from labeling this particular case. The implementation below combines uncertainty (proximity to the decision boundary) with diversity (dissimilarity from already-queued cases).
