    ...
    def compute_project_kappa(self) -> Tuple[float, List[str]]:
        ...
        for i in range(len(all_ann)):
            for j in range(i + 1, len(all_ann)):
                ...
                if len(la) >= 30:  # Appendix 7A: require N >= 30 items per pair
                    pair_kappas[(a, b)] = cohen_kappa_score(la, lb)

        if not pair_kappas:
            return float('nan'), []

        # Aggregate as the mean of per-pair kappas: pooling labels
        # across different pairs would mix rater marginals, and the
        # result would be no pair's kappa (nor Fleiss' kappa).
        aggregate = float(np.mean(list(pair_kappas.values())))
...
Using the \texttt{cost\_estimate} function, calculate the total cost for a project of 10{,}000 items at \$0.10/annotation with the default 10\% overlap fraction. Then extend the function with \texttt{expert\_fraction} and \texttt{expert\_cost\_usd} parameters modeling expert re-annotation at \$1.00/annotation, and compute the total cost when 20\% of items receive expert re-annotation.
\end{Exercise}
