...
\chapter*{Technical Appendix 10A: The Mathematics of Language Models}
\addcontentsline{toc}{chapter}{Technical Appendix 10A: The Mathematics of Language Models}
\label{app:ch10}
...
where $W_Q, W_K \in \mathbb{R}^{d_{\text{model}} \times d_k}$ and $W_V \in \mathbb{R}^{d_{\text{model}} \times d_v}$ are learned projection matrices. The output $\text{softmax}(QK^\top/\sqrt{d_k})$ is the \textbf{attention weight matrix} $A \in \mathbb{R}^{n \times m}$, where $A_{ij}$ represents how strongly position $i$ attends to position $j$.
...
\begin{equation}
\max_\theta \;\mathbb{E}_{x \sim \mathcal{D}} \!\left[\mathbb{E}_{y \sim \pi_\theta(\cdot \mid x)} \!\left[r_\phi(x, y)\right] - \beta \cdot \operatorname{KL}\!\left[\pi_\theta(\cdot \mid x) \;\|\; \pi_{\text{SFT}}(\cdot \mid x)\right]\right]
...
% ============================================================================
\section{Constitutional AI: Two-Phase Process}
...
\subsection{Phase 1: Supervised Learning for Constitutional AI (SL-CAI)}
...
\begin{enumerate}
    ...
    \label{eq:cai-revise}
    \end{equation}
    \item \textbf{Fine-tune:} Train $\pi_0$ on $(x, y')$ pairs to get $\pi_{\text{SL-CAI}}$.
\end{enumerate}

\subsection{Phase 2: Reinforcement Learning from AI Feedback (RLAIF)}

\begin{enumerate}
    \item For each prompt, generate two outputs from $\pi_{\text{SL-CAI}}$.
    ...
    P_{\text{AI}}(y_w \succ y_l \mid x)
      = \pi_{\text{SL-CAI}}\!\bigl(\text{``A is better''} \mid x, y_1, y_2, c_i\bigr)
    \label{eq:rlaif}
    \end{equation}
    \item Train reward model $r_\phi$ on AI preference labels using the Bradley-Terry objective \eqref{eq:rm-loss}.
    \item Fine-tune $\pi_{\text{SL-CAI}}$ via PPO against $r_\phi$ using the objective \eqref{eq:rlhf-objective}.
\end{enumerate}
...
% ============================================================================
\section{Calibration Metrics for Language Models}
