...
For color images with $C$ channels and $F$ filters, the full operation produces an output volume $Z \in \mathbb{R}^{H' \times W' \times F}$:
...
Object detection requires localizing objects via bounding boxes. The standard metric for localization quality is \term{Intersection over Union} (IoU):
...
\qquad
\hat{\sigma}^2 = \frac{1}{M}\sum_{m=1}^{M}\bigl(p_m(y\mid x) - \bar{p}(y\mid x)\bigr)^2
\]

Deep ensembles produce better-calibrated uncertainties than MC Dropout but require $M$ times the training compute and storage.\footnote{The MC-dropout estimator above uses the unbiased $1/(T-1)$ sample variance because the $T$ passes are stochastic draws from an approximate posterior; the ensemble uses $1/M$, treating the $M$ trained models as the full population rather than a sample.}
...
\begin{itemize}
    ...
    \item $\text{Conf}(i \to j)$: objects of class $i$ detected as class $j$ (localization correct, label wrong)
    \item $\text{FN}_c$: objects of class $c$ present but not detected
    \item $\text{FP}_c$: detections labeled as class $c$ with no corresponding ground truth
\end{itemize}

In medical imaging, FN (missed findings) carry far higher consequences than FP (unnecessary follow-up). This asymmetry justifies operating the system at a threshold that minimizes FN rate, even at the cost of a higher FP rate.
...
Given a pool of unlabeled images $\mathcal{U}$, select the image with highest predictive entropy:
...
where $d(x,s)$ is cosine distance in penultimate-layer feature space. This ensures the annotation budget covers the diversity of the unlabeled pool rather than clustering around already-well-represented examples.
...
The \texttt{human\_annotator.annotate()} call represents the HITL intervention. The quality of the final model depends on both the query strategy (selecting informative images) and the quality of the human annotation (correctly labeling them).
...
\begin{enumerate}
    \item Start with 500 randomly labeled examples. Train a baseline CNN.
    \item Compare random sampling, entropy sampling, and BALD as query strategies.
    \item Plot model accuracy as a function of the number of labeled examples.
    \item How many labeled examples does the best active learning strategy need to match the accuracy of 5{,}000 random labels?
\end{enumerate}
\end{Exercise}
