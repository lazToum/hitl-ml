...
By convention, $\mu_N = 0$, $\sigma = 1$, and $\mu_S = d' > 0$. We write $\Phi$ for the standard normal CDF.

The reviewer compares $X$ to a criterion location $k$:
\[
\text{Response} = \begin{cases} \text{``positive''} & \text{if } X > k \\ \text{``negative''} & \text{if } X \leq k \end{cases}
\]
...
\midrule
\textbf{Signal present} & Hit (TP): $P(H) = \Phi(d' - k)$ & Miss (FN): $1 - P(H)$ \\
\textbf{Signal absent} & False Alarm (FP): $P(FA) = \Phi(-k)$ & Correct Rejection: $1 - P(FA)$ \\
\bottomrule
\end{tabularx}
\end{table}
...
The bias $c$ and the criterion location $k$ are related by $c = k - d'/2$, so $c = 0$ places the criterion exactly midway between the noise and signal means ($k = d'/2$, unbiased), whatever the value of $d'$.
...
Let $p^*$ be the prevalence of positive cases in the human review queue. The cost-optimal criterion location:
\[
k^* = \frac{d'}{2} - \frac{1}{d'} \ln\left(\frac{p^* \cdot C_{FN}}{(1-p^*) \cdot C_{FP}}\right)
\]
...
The ROC curve traces (false alarm rate, hit rate) pairs as the criterion $k$ varies. For the Gaussian equal-variance model:
\[
...
where $P_A$ = automation accuracy, $P_H$ = human accuracy, $P_C$ = complementarity (probability exactly one is correct), and $f(\tau) = \tau(1-\tau)$ captures the team synergy term---highest when both parties contribute to the decision, vanishing under full deference ($\tau = 1$) or full dismissal ($\tau = 0$).

\subsection{Optimal Trust Level}

Setting the derivative of $P(\text{correct})$ to zero, $P_A - P_H + P_C(1 - 2\tau) = 0$, gives the optimal trust level:
\[
\tau^* = \frac{P_A - P_H + P_C}{2\,P_C}
\]
(clipped to $[0,1]$ if the parameters fall outside the interior-solution range).
...
\begin{enumerate}[(a)]
    \item Estimate $P(\text{complementary})$, taking it here to be the probability the human is correct conditional on disagreeing with the automation.
    \item Compute $\tau^*$.
