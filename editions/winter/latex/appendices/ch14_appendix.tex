...
\paragraph{Predictive Parity} (predictive rate parity---equal positive predictive value across groups; not to be confused with \emph{calibration} in the score sense, which the impossibility theorem below concerns).
\[
...
\]
where $\text{PPV}_A$ is the positive predictive value and $\text{PR}_A$ is the prediction rate for group $A$. If $b_0 \neq b_1$ and PPV is equalized (predictive parity holds), then FPR equality requires $\text{PR}_A$ to adjust in a way that inevitably violates FNR equality. All three cannot hold simultaneously.
...
def demonstrate_impossibility(base_rate_0=0.2, base_rate_1=0.4, n=2000):
    ...
    # Calibrated, informative scores: P(Y = 1 | s, group) = s.
    # Beta(2, 8) has mean 0.2 = base_rate_0; Beta(4, 6) mean 0.4.
    scores = np.where(
        groups == 0,
        np.random.beta(2, 8, n),
        np.random.beta(4, 6, n)
    )
    y = np.random.binomial(1, scores)

    y_pred  = (scores > 0.3).astype(int)
    metrics = compute_fairness_metrics(y, y_pred, groups)

    for g, gv in [('group_0', 0), ('group_1', 1)]:
        m = metrics[g]
        mask = groups == gv
        print(f"{g}: TPR={m['TPR']:.3f}, FPR={m['FPR']:.3f}, "
              f"PPV={m['PPV']:.3f}, PredRate={m['pred_rate']:.3f}, "
              f"MeanScore={scores[mask].mean():.3f}, "
              f"BaseRate={m['base_rate']:.3f}")
...
def counterfactual_fairness_test(model, X_test, sensitive_col,
    ...
    # The model must see the protected attribute for the flip to register.
    # (A full counterfactual would also update the attribute's causal
    # descendants; this approximate test holds the other features fixed.)
    preds_original = model.predict(X_a)
    preds_counter  = model.predict(X_a_counter)
...
A structured audit checklist derived from NIST AI RMF (2023), US disparate-impact doctrine (the EEOC four-fifths rule), and common practice for EU AI Act conformity preparation:
...
\begin{Exercise}[The Impossibility in Practice]
Run \texttt{demonstrate\_impossibility(base\_rate\_0=0.15, base\_rate\_1=0.35, n=2000)}. Report the false-positive-rate gap and the PPV (calibration) gap between the two groups. Using the impossibility theorem, explain why minimizing the calibration gap cannot also eliminate the FPR gap when the base rates differ.
\end{Exercise}

\begin{Exercise}[Disparate Impact Audit]
Using the Adult/Census Income dataset with \texttt{sex} as the protected attribute, compute the disparate impact ratio and the mean-difference statistic with AIF360. Does the classifier satisfy the four-fifths (0.80) rule? Propose one mitigation strategy and predict its effect on overall accuracy.
\end{Exercise}

\begin{Exercise}[Feedback Loop Simulation]
Simulate a predictive-policing feedback loop over several cycles: two neighborhoods with different base crime rates, where each cycle's arrests become the next cycle's training data. (a)~Build the model and run the loop. (b)~Measure whether the between-group disparity amplifies across cycles. (c)~Propose an intervention that breaks the loop, and demonstrate its effect.
\end{Exercise}

\begin{Exercise}[Counterfactual Fairness Check]
For a hiring model trained on historical data, apply the counterfactual fairness test. (a)~Flip the protected (gender) attribute and measure the prediction flip rate. (b)~Interpret the result. (c)~Explain why a flip rate of zero does not by itself guarantee fairness, considering proxy features.
\end{Exercise}
