...
Simulate a predictive-policing feedback loop over several cycles: two neighbourhoods with different base crime rates, where each cycle's arrests become the next cycle's training data. (a)~Build the model and run the loop. (b)~Measure whether the between-group disparity amplifies across cycles. (c)~Propose an intervention that breaks the loop, and demonstrate its effect.
\end{Exercise}
