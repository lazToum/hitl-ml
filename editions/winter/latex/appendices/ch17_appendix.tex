...
Passive supervised learning in the agnostic (worst-case noise) setting requires $m = \Theta(d / \varepsilon^2)$ labeled examples to achieve error $\varepsilon$ for a hypothesis class of VC dimension $d$; in the realizable case, $O(d / \varepsilon)$ labels suffice. Active learning's gains are regime-dependent: under realizable or low-noise assumptions, label complexity can fall to $O(\theta d \log(1 / \varepsilon))$, where $\theta$ is the disagreement coefficient---an exponential improvement in $1 / \varepsilon$---while in the fully agnostic worst case, no general improvement over $d / \varepsilon^2$ is possible.

Where it exists, this speedup arises because active learning concentrates queries near the decision boundary, where labels provide the most information about the target function $f^*$.
...
\begin{enumerate}
    \item For each unlabeled point $x_i$, compute the gradient of the loss, evaluated at the predicted label, with respect to the model parameters: $g_i = \nabla_\theta L(f_\theta(x_i), \hat{y}_i)$
...
def badge_select(model, unlabeled_X, batch_size: int) -> np.ndarray:
    ...
    for i in range(n_samples):
        ...
        # softmax-level factor p - e_{hat{y}} of the full last-layer
        # gradient (p - e_{hat{y}}) x h(x_i); the hidden-activation
        # factor h(x_i) is omitted in this simplified implementation
        gradient_embeddings[i, label] = confidence - 1
        for j in range(n_classes):
            if j != label:
                gradient_embeddings[i, j] = probs[i, j]
...
Given preference pairs $\mathcal{D} = \{(x, y_w, y_l)\}$, the reward model $r_\phi$ is trained by minimizing the loss:
\[
...
With a trained reward model, the policy $\pi_\theta$ is fine-tuned to maximize the objective:
\[
...
DPO \autocite{rafailov2023direct} eliminates the explicit reward model and is trained by minimizing the loss:
\[
...
Standard ECE is computed on a test set drawn from the same distribution as training. Under distribution shift, ECE typically degrades. A meta-calibration measure, defined over a specified family $\mathcal{P}$ of shifted distributions (e.g., a held-out set of deployment domains):
\[
\mathrm{ECE\text{-}shift} = \mathbb{E}_{P' \sim \mathcal{P}}\bigl[\mathrm{ECE}(f, P')\bigr]
\]
