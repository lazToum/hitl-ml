...
\label{chap:ch01}
\begin{chapterquote}{on asking for help}
...
According to Netflix's own Help Center, the streaming giant asks this question ``so you don't lose your place or use internet data when you're not actually watching.'' The prompt appears after watching a few episodes in a row without interaction, or after roughly 90 minutes of continuous viewing---the precise thresholds vary by device. Netflix's algorithm is constantly monitoring whether you've paused, skipped ahead, or otherwise interacted with playback---any sign that a human is still actively engaged.
...
Google's own community forums are filled with complaints from frustrated homeowners. Users report the thermostat switching to Away mode while they sit in their own living rooms---the house suddenly starts cooling down because the thermostat decided nobody was home. Others describe the Nest flipping into Away mode several times a day while they work from home, until they give up and disable the ``smart'' features entirely, using the device as a dumb thermostat.
...
The key failure is that the Nest acts on its conclusions even when they're uncertain. When it decides you're ``away,'' it doesn't ask for confirmation---it just starts changing the temperature. By default, it never pauses to ask ``I haven't detected any motion for a while. Should I switch to energy-saving mode?'' No opportunity for you to say ``No, I'm actually here, I'm just being quiet.''
...
Many Nest owners have shared a rueful common endpoint: they disabled Home/Away Assist entirely and went back to manual scheduling---effectively turning their \$250 smart thermostat into a \$30 programmable one. Again and again, the story ends the same way: they loved the Nest, but they had to turn off its brain.
...
Similar incidents have occurred repeatedly around the world. In spring 2023, tourists in Hawaii followed their GPS right into Honokohau Harbor while looking for a manta ray tour. The viral video showed bystanders watching in disbelief as the car floated off the boat ramp \autocite{hnn2023honokohau}. ``I didn't hear any brake sound or anything,'' a witness told local news. ``She didn't try to stop at all.''

In 2016, a woman in Ontario, Canada drove her car into Georgian Bay---an inlet of Lake Huron---after following GPS directions onto a boat launch in Tobermory \autocite{cbc2016gps}. She rolled down her window and swam roughly 30 metres to shore.
...
Medical AI systems work on similar principles. In radiology, AI algorithms are increasingly used to analyze X-rays, CT scans, and MRIs. But rather than simply giving a diagnosis, these systems are designed to express their confidence level. A 2019 article in \emph{Nature Machine Intelligence} argued that \term{uncertainty quantification} is essential for machine-assisted medical decisions---essentially, AI that tells the doctor ``I'm 95\% confident this is a fracture'' versus ``I'm only 60\% confident, so you should take a closer look'' \autocite{begoli2019uncertainty}.

AI systems that flag uncertain cases for human review can serve as a safety net: the model handles the routine cases, and the ones it is least sure about get a second look from an expert \autocite{begoli2019uncertainty}. The key insight is that the AI doesn't need to be perfect---it just needs to know when it's not confident, and to route those cases to human experts.
...
The case highlights a critical problem with AI systems: they can be ``confidently wrong.'' Studies across multiple domains have found that AI systems produce incorrect information---sometimes called \term{hallucinations}---at rates that vary widely depending on the task and system, but that remain significant even in purpose-built deployments. A 2024 study found that general-purpose AI models hallucinated on between 58\% and 88\% of specific, verifiable questions about federal court cases \autocite{dahl2024large}---and even purpose-built legal research tools, marketed as more reliable, still hallucinated between roughly 17\% and 33\% of the time \autocite{magesh2024hallucination}.
...
\end{description}
Each subsequent chapter deepens one or more of these dimensions, which together form a loop (\cref{fig:five-dimensions}). Chapter summaries throughout the book include a Five Dimensions Check to make the connections explicit.
\end{keyconcept}

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    dim/.style={draw=hitlblue, line width=0.9pt, fill=hitllight,
        rounded corners=3pt, align=center, text width=2.55cm,
        inner sep=5pt, font=\small},
    num/.style={circle, draw=hitlblue, fill=hitlblue, text=white,
        font=\bfseries\footnotesize, inner sep=0pt, minimum size=15pt},
    flow/.style={-{Stealth[length=6pt]}, hitlgray, line width=1.1pt},
]
\def\R{3.4cm}
\foreach \i/\ang/\name in {1/90/Uncertainty\\Detection, 2/18/Intervention\\Design,
    3/-54/Timing, 4/-126/Stakes\\Calibration, 5/162/Feedback\\Integration}{
    \node[dim] (d\i) at (\ang:\R) {\textbf{\name}};
    \node[num] at ($(\ang:\R)+(0,0.92cm)$) {\i};
}
\foreach \a/\b in {1/2,2/3,3/4,4/5,5/1}{
    \draw[flow] (d\a) to[bend left=18] (d\b);
}
\node[circle, draw=hitlgray, line width=0.8pt, fill=white, text width=1.9cm,
    align=center, font=\footnotesize\itshape, text=hitlgray] at (0,0)
    {the human in the loop};
\end{tikzpicture}
\caption{The Five Dimensions of \hitl system design form a loop: each dimension feeds the next, and the human sits at the center of the cycle.}
\label{fig:five-dimensions}
\end{figure}
...
This is a claim with a boundary, and the boundary is worth stating plainly. Full automation is the right answer in more places than intuition suggests: low-stakes, high-volume tasks---spell-check, search ranking, auto-approval of small transactions---run autonomously at error rates no human workforce could match. And the automation-bias evidence in this book cuts both ways: where humans reliably drag good algorithmic output downward, the loop itself is the error source, and the honest design response is fewer loops, better placed. The collaborative claim is not that every system needs a human. It is that where errors are costly, novelty is constant, and someone must answer for the decision, the human is not a scaffold awaiting removal. They are part of the architecture.
...
\begin{description}
    \item[Netflix ``Are you still watching?''] System monitors engagement and asks after several episodes or roughly 90 minutes without interaction; low-stakes, graceful design
...
\begin{itemize}
    \item \textit{Uncertainty Detection} (Dimension~1): The dollhouse orders, the GPS-into-water incidents, and the Air Canada chatbot were all systems that could not tell when they were out of their depth
    \item \textit{Intervention Design} (Dimension~2): Netflix's low-stakes ``Are you still watching?'' versus the Nest thermostat's silent, unchecked acting---the ask must be designed for the human
    \item \textit{Timing} (Dimension~3): Alert fatigue shows what happens when systems ask too often; the systems that never ask show the opposite failure
    \item \textit{Stakes Calibration} (Dimension~4): The same asking principle runs from a paused binge-watch to medical AI and self-driving cars---the threshold for asking should reflect the cost of being wrong
    \item \textit{Feedback Integration} (Dimension~5): Every fraud alert confirmed and every autocorrect suggestion accepted flows back into the system, closing the loop
\end{itemize}
