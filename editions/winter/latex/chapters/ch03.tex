...
A fair objection hangs over this example, and it deserves a direct answer: this book's evidence is selected on failure. The cases that open its chapters---the chatbot that invented a refund policy, the car that followed GPS directions into a lake---are vivid because something went wrong. But selection on failure can show that loops break, not that they always break, and a skeptic is right to ask where the base rates are. So here is the claim in falsifiable form: if a domain accumulates years of autonomous operation at better-than-human error rates with acceptable tail risk---as spam filtering, search ranking, and route planning largely have---then a human in that loop is cost without benefit, and full automation with monitoring is the right architecture there. The thesis of this book is not that every system needs a human at the edges. It is that in domains where errors are expensive, the distribution keeps moving, and someone must answer for the outcome, the edges regenerate faster than engineering closes them. Where those conditions hold, the loop is not a workaround. Where they do not, it is.
...
\begin{itemize}
    ...
    \item Even a perfect learner has permanent performance boundaries at the edges of its training distribution---not a limitation to be engineered away, but the reason \hitl design is the correct architecture, not a workaround
...
\noindent\textit{In the next chapter, we ask how a system turns uncertainty into a decision. The apparently simple step of choosing an action from a probability distribution turns out to involve deep questions about value and stakes.}
