...
A HITL system that routes only the near-boundary cases to human review~--- the cases where the classifier's confidence is between, say, 0.4 and 0.6~--- is efficiently targeting human attention where it has the highest marginal value (\cref{fig:threshold-band}).
\end{keyconcept}
...
\paragraph{Over time.} The base rate of positive cases changes. If fraud increases from 0.1\% to 0.5\% of transactions, the same model's false positive rate balloons. The threshold needs adjustment.
...
\noindent\textit{Next chapter: once we know we need to ask, how do we ask well? The design of HITL interventions turns out to be a surprisingly deep question~--- one that draws on cognitive psychology, communication design, and the mechanics of how human attention works.}
