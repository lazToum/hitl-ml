...
% ============================================================================
% Source: /markdown/ch5/Ch5_final.md
% ============================================================================

\label{chap:ch05}
\begin{chapterquote}{on the art of asking}
The art of asking for help. Not whether to ask. How.
\end{chapterquote}

% ============================================================================
\section{A Better Question}
\label{sec:ch05-one-word}
% ============================================================================

At the start of a life-or-death call, a 911 dispatcher faces a deceptively simple task: find out whether the patient is breathing. It sounds binary---either someone is breathing or they are not. But cardiac arrest does not always look the way people expect. In the first minutes after the heart stops, many patients produce \term{agonal breathing}: slow, gasping, reflexive breaths that are not real breathing at all, yet look and sound enough like it to fool a frightened bystander.

For years this created a deadly ambiguity. When a dispatcher asked, ``Is he breathing?''---the natural, obvious question---a caller watching a loved one gasp would answer ``yes.'' The dispatcher, hearing ``yes,'' would not recognize a cardiac arrest, would not begin telephone CPR instructions, and would not get a bystander pushing on the chest in the seconds that matter most. The single most important intervention a dispatcher can trigger was being lost to one word in a question.

Researchers studying out-of-hospital cardiac arrest in Dallas introduced a revised dispatch protocol built around better asks. Instead of letting a caller's ``yes'' settle the question, dispatchers were given concrete assessment steps: have the caller count the patient's breaths, ask them to describe what the breathing sounds like---in effect, to judge whether it sounds \emph{normal}---and have them hold the phone next to the patient so the dispatcher can hear for themselves \autocite{roppolo2009agonal}.

Each of these steps does the same work. It forces the caller to judge the \emph{quality} of the breathing rather than answer a yes/no question they had already half-answered in their head. A gasping, agonal breath is many things, but it is not normal breathing---and once a caller is asked to count it, describe it, or hold the phone close enough for the dispatcher to hear it, they hesitate, they report what is actually happening, and the dispatcher can recognize the arrest.

After the protocol changed, the share of cardiac-arrest patients mistakenly thought to be breathing normally fell, and dispatchers who had detected almost no agonal breathing in the months before the change were identifying it case after case afterward---each instance a chance to start CPR minutes earlier \autocite{roppolo2009agonal}.

No new technology. A handful of small changes to the questions dispatchers ask hundreds of times a day changed how callers understood what they were being asked---and changed what information the dispatchers got back.
...
The premise behind asking questions with six items to review, or requiring users to navigate three menus to respond to a notification, or sending a fraud alert in a format that requires calling a phone number---all of these assume that the user is lazy and that the friction is acceptable because dedicated users will push through it.
...
Second, the assumption fails even on its own terms. A willing user given a four-step response format is not four times as likely to respond as one given a single step. The friction does not select for motivated users. It just loses them.
...
\begin{itemize}
  \item Push-approval prompts where a single tap approves the login---so-called \emph{MFA fatigue attacks} exploit users who approve reflexively just to make the prompts stop
...
\paragraph{Throughput pressure wears reviewers down.} The literature on content moderators links heavy workloads and sustained exposure to disturbing material to degraded well-being---fatigue and strain that accumulate across a shift and that consistent, careful judgment can ill afford \autocite{steiger2021psychological}. The ask format cannot compensate for structural workload problems.

\paragraph{Explanation requests improve downstream model quality.} When moderators are asked not just ``is this violating?'' but ``what makes this violating?'' (even briefly), the resulting labels are more useful for model training \autocite{zaidan2007rationales}. The ask format shapes what information is captured, not just what decision is made.
...
Return to the dispatcher's questions. A handful of small changes to the ask---count the breaths, describe the breathing, hold the phone close---changed how often a cardiac arrest was recognized in time.

Why did these changes matter so much?

The original question (``Is he breathing?'') asked the caller for a yes/no fact they thought they already knew. A gasping patient \emph{is} moving air, after all; to a panicked bystander, that reads as breathing. The framing let them confirm what they wanted to believe.

The revised asks demanded a judgment rather than a fact. Describing the breathing, counting it, holding the phone close enough to hear it---each made the caller compare what was in front of them against what breathing is supposed to look and sound like, and agonal gasping fails that comparison. The new prompts turned a reflexive ``yes'' into an act of observation.
...
\begin{description}
  \item[911 dispatch (cardiac arrest)] Asking ``Is he breathing \emph{normally}?'' instead of ``Is he breathing?'' helped dispatchers catch agonal breathing and recognize cardiac arrest in time
...
\begin{itemize}
  \item \textit{Intervention Design} (Dimension~2): The central subject of this chapter---the design of the ask itself, not merely the decision to ask
  \item \textit{Timing} (Dimension~3): One of the three primary levers; the ask must fall within the human's attention window
  \item \textit{Feedback Integration} (Dimension~5): The ask and the response are one system---the ask is wasted unless the answer improves the model
\end{itemize}
