...
In 2015, Stanford researcher Fei-Fei Li gave a TED talk in Vancouver describing what she and her collaborators had been building since 2007: ImageNet \autocite{li2015ted}. The project, assembled with the help of Amazon Mechanical Turk, had grown to 15 million photographs organized into 22,000 categories. It was the largest labeled image dataset ever assembled, and it had already become the training ground for the deep learning revolution \autocite{deng2009imagenet}.

By 2019, a series of studies had identified something troubling in the data. The category ``cheerleader'' contained photographs that were overwhelmingly of women. The category ``guard'' was overwhelmingly of men. The annotations were accurate in a narrow sense---the photographs did show what they were labeled as---but the dataset had encoded cultural stereotypes so thoroughly that models trained on it absorbed and reproduced those gendered associations \autocite{yang2020fairer}.
...
This is what annotation researchers call the \term{instruction problem}, and it is pervasive. Röttger and colleagues showed that even the broad \emph{paradigm} a task adopts changes its labels: a \emph{descriptive} approach, which invites annotators to apply their own subjective judgment, and a \emph{prescriptive} one, which asks them to apply a single specified standard regardless of their personal view, produce systematically different label distributions on the same data---and neither is simply ``correct,'' because they are answering different questions \autocite{rottger2022paradigms}. The same gap opens with a single reworded question. ``Is this comment hateful?'' is not the same task as ``Would a reasonable person find this comment toxic?'' The second framing invokes a hypothetical external standard that moderates individual variation---it asks the annotator to model a composite person rather than report their own reaction.
...
When two annotators label the same set of items, their agreement is measurable. The simplest measure is raw agreement---the proportion of items they label the same way. But raw agreement is inflated by chance agreement: if 90\% of items are NOT TOXIC and each annotator independently labeled 90\% of items NOT TOXIC at random, they would agree 82\% of the time by chance alone---the sum of both-TOXIC and both-NOT-TOXIC chance cells---while making no genuine classification decisions.

The standard correction is \term{Cohen's kappa}, developed by psychologist Jacob Cohen in 1960~\autocite{cohen1960kappa}:
...
\end{itemize}
These thresholds, proposed by Landis and Koch in 1977~\autocite{landis1977kappa}, are conventional, not theoretically grounded. Always interpret kappa in the context of the specific task.
\end{keyconcept}

When the toxicity detection field began systematically measuring annotator agreement, the results were sobering. A landmark study of the OLID benchmark by Zampieri and colleagues in 2019 found a Fleiss' kappa of 0.83 in a small trial with five expert annotators labeling 21 tweets \autocite{zampieri2019predicting}---Fleiss' kappa being the multi-rater counterpart to the two-rater Cohen's kappa derived above, and 0.83 being ``almost perfect'' on the Landis-Koch scale. Yet subsequent annotation projects in the toxicity domain with larger, less-controlled annotator pools have reported substantially lower values, in the 0.45--0.60 range. That gap---between what experts achieve under controlled conditions and what larger pools achieve in practice---is the sobering result, illustrating how sensitive inter-annotator agreement is to annotator composition and task framing. Kappa is chance-corrected, so it does not map directly onto a disagreement rate: with an even split of toxic and benign comments, $\kappa = 0.45$ means two annotators would disagree on roughly a quarter of items; with a skewed label distribution, the same kappa can sit atop much higher raw agreement. A model trained on such data is not learning to detect toxicity; it is learning to predict the label distribution of a specific group of annotators working under specific conditions.
...
\paragraph{Sequence effects.} An annotator who has just reviewed 20 highly toxic comments will have a different calibration threshold for the next borderline comment than an annotator who has just reviewed 20 benign ones. Studies of machine translation quality annotation found that annotators systematically rated the same translations differently depending on the quality of the surrounding translations in their queue \autocite{mathur2017sequence}. The translation had not changed. The annotator's reference point had.

\paragraph{Demographic effects.} Research led by Sap and colleagues at the University of Washington found that tweets containing African American English (AAE) were up to twice as likely to be labeled offensive compared to similar tweets in Mainstream American English, even when the content was neutral or positive. When annotators were primed with information about AAE dialect features before labeling, the racial bias in labeling decreased \autocite{sap2019risk}. Subsequent work by Sap et al.\ (``Annotators with Attitudes,'' 2022) found the same pattern at the annotator level: Black and white annotators differed substantially in their toxicity judgments \autocite{sap2022annotators}. The practical consequence is that a model trained on annotations from a predominantly white annotator pool will have a systematically higher false toxicity rate for AAE.
...
The ImageNet dataset was completed with the help of Amazon Mechanical Turk, a crowdsourcing platform that allows requesters to post small tasks and have them completed by a globally distributed workforce for small payments. A typical toxicity labeling task on Mechanical Turk pays on the order of \$0.02 to \$0.05 per item. In a 2018 study of all work on the platform, Hara and colleagues found a median effective hourly wage of approximately \$2/hr once unpaid time---searching for tasks, waiting on requesters, switching between jobs---was counted \autocite{hara2018data}.
...
Framing effects describe the phenomenon that how a question is posed shapes the answer, independently of what the answer ``truly'' is. A classic study found that a medical treatment described as having a ``90\% survival rate'' was viewed more favorably than the same treatment described as having a ``10\% mortality rate.'' Same statistic. Very different responses \autocite{mcneil1982elicitation}.
...
\begin{description}
    \item[ImageNet (2007--2019)] Cultural stereotypes baked into labels by an annotation process that faithfully transcribed annotator worldview as ground truth
    \item[AAE toxicity labeling] Demographically homogeneous annotator pools produce systematically biased labels for minority language registers
...
\begin{itemize}
    \item \textit{Feedback Integration} (Dimension~5): Labels are the training feedback that drives the loop; biased or noisy annotation corrupts everything downstream
    \item \textit{Intervention Design} (Dimension~2): Guideline wording and question framing are interface choices that shape what the annotator produces
    \item \textit{Uncertainty Detection} (Dimension~1): Low inter-annotator agreement is itself a signal---of an ambiguous task, unclear guidelines, or both
\end{itemize}
