...
% ============================================================================
% Source: /markdown/ch8/Ch8_final.md
% ============================================================================

\label{chap:ch08}
\begin{chapterquote}{on architecture}
Most systems that claim to be ``human-in-the-loop'' are actually pipelines.
\end{chapterquote}
...
In 2018, an Uber self-driving test vehicle struck and killed Elaine Herzberg in Tempe, Arizona. She was walking her bicycle across a multi-lane road at night. The onboard systems detected her 5.6~seconds before impact. But the software never settled on what it was seeing: it classified her first as a vehicle, then as an unknown object, then as a bicyclist---and each time the classification flipped, the system discarded its tracking history and started over. It never classified her as a pedestrian, and it never predicted her path \autocite{ntsb2019uber}.

About 1.2~seconds before impact, the system finally recognized an imminent collision. It did not brake. Emergency braking had been deliberately disabled while the computer was driving, on the theory that a human safety driver was watching the road and would intervene. The system's design response was to sound an alert and hand the moment to the human. By then, it was too late.
...
\paragraph{The Interface Layer.} The system through which human reviewers receive and respond to cases. This is more than a display---it is the context provision, the question framing, the response format, and the ergonomics that determine whether human feedback is useful or garbage. Chapter~6 covers this in depth. Architecturally, the key requirement is that it can present information appropriately for different case types and capture structured responses that can be fed back into training.

\paragraph{The Feedback Integration Layer.} The mechanism by which human responses improve the system. This may be a retraining pipeline, an active learning module, a rule update system, or a calibration-adjustment component. Without this layer, the ``loop'' in \hitl is not a loop---it is an open-ended pipeline.
...
\begin{itemize}
  \item \textit{Feedback Integration} (Dimension~5): The core subject of this chapter
  \item \textit{Uncertainty Detection} (Dimension~1): Required for the routing layer to function
  \item \textit{Stakes Calibration} (Dimension~4): Informs the routing thresholds
\end{itemize}
