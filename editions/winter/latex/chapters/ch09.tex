...
% ============================================================================
% Source: /markdown/ch9/Ch9_final.md
% ============================================================================

\label{chap:ch09}
\begin{chapterquote}{on tools}
The tools are not magic. But the landscape is rich enough now that the question of what tool to use has real answers.
\end{chapterquote}
...
Open-source annotation tools like Label Studio did something their creators did not fully anticipate: they put real labeling capacity in the hands of organizations that could never afford a commercial vendor. Built by a small startup and released as open source, Label Studio was aimed at machine learning teams at well-funded labs---but it was taken up by a very different audience: non-profits labeling satellite images of deforestation, hospital systems annotating medical records, small newsrooms classifying articles for studies of media coverage.
...
% ============================================================================
\section{Self-Hosted and Open-Source Options}
\label{sec:ch09-open-source}
% ============================================================================

\paragraph{Label Studio} is among the most widely used open-source annotation platforms. It supports all major data types: text, images, audio, video, time series, and HTML. The labeling interface is configured with a simple XML-style tag language, which keeps it relatively accessible. It has a Python SDK and REST API, active community development, and a commercial cloud offering for teams that want hosted infrastructure.
...
\paragraph{Docanno and Brat} are simple open-source text annotation tools requiring minimal technical setup. They are effective for single annotators or small teams on text classification tasks, but do not scale well.

\paragraph{Prodigy} is a commercially licensed but developer-friendly tool from the creators of spaCy. It is designed for fast, scriptable annotation workflows with a particular emphasis on active learning integration. Prodigy's model-in-the-loop approach, where the model pre-annotates and humans correct, can dramatically reduce annotation time on \nlp tasks.
...
\paragraph{Scale AI} and Surge AI, along with similar enterprise platforms, offer managed annotation workforces, quality controls, domain-specific pipelines (medical, legal, autonomous vehicle), and APIs for integrating annotation into \ml pipelines. These managed workforces handle the recruitment, training, and quality management of annotators---a significant operational burden for organizations that do not want to manage their own annotation team. One caution on Scale in particular: after Meta's June 2025 purchase of a 49\% stake in the company, several of Scale's largest customers pulled back, and buyers now weigh its independence as a vendor more carefully than they once did.
...
\paragraph{Amazon SageMaker Ground Truth} integrates annotation directly into AWS's \ml toolchain. It offers both Amazon Mechanical Turk (crowdsourced) and private workforce (your own team) options. Its automated data labeling feature provides a built-in form of active learning, automatically sending uncertain cases to human reviewers and using confirmed labels to improve the model. One significant caveat: AWS has announced that it will close Ground Truth to new customers in July 2026 and does not plan to introduce new features, so organizations choosing a platform for a new project should weigh the alternatives in this chapter.

Best for: existing AWS customers with Ground Truth deployments; new projects should look elsewhere.
...
The honest assessment of no-code options: they work well for small, well-defined tasks. They hit ceilings quickly when you need active learning integration or custom label schemas, or when you have privacy requirements or annotation volumes that rule out the managed services. They are a useful starting point, not a long-term architecture.
...
\begin{description}
  ...
  \item[Active learning in SageMaker Ground Truth] Architectural integration of uncertainty routing in a managed cloud service---and a lesson in vendor churn, with the service closing to new customers in July 2026
\end{description}

\subsection*{Five Dimensions Check}

\begin{itemize}
  \item \textit{Feedback Integration} (Dimension~5): Tools are the infrastructure for closing the loop
  \item \textit{Intervention Design} (Dimension~2): The annotation interface is the intervention design implementation
\end{itemize}
