...
On June~8, 2023, a federal judge in New York held an unusual sanctions hearing. Two attorneys---Steven Schwartz and Peter LoDuca of the firm Levidow, Levidow \& Oberman---sat before Judge P.~Kevin Castel in the Southern District of New York. They had filed a legal brief in a personal injury case against Avianca airlines. The brief cited half a dozen case precedents. The judge had noticed something strange: he couldn't find them.
...
Judge Castel imposed sanctions on both attorneys in a written decision issued two weeks later. In the order that had first flagged the fabrications, he had already written something that should be printed on the login screen of every AI assistant: ``The Court is presented with an unprecedented circumstance. A submission filed by plaintiff's counsel in opposition to a motion to dismiss is replete with citations to non-existent cases.''~\autocite{mata2023avianca}
...
Or consider irony: ``Oh, great. Another Monday.'' A computer reading that sentence, unaware that Monday often brings dread, might classify it as a positive statement. Sentiment analysis tools still stumble on sarcasm, because understanding irony requires a model of what the speaker wanted you to \emph{think they meant} as opposed to what they \emph{literally said}---a layered theory of mind that took humans hundreds of thousands of years to evolve.
...
This is how we arrived at \term{large language models}: GPT-4, Claude, Gemini, LLaMA, Mistral. Vast statistical engines with hundreds of billions of learned parameters.
...
Research into hallucination rates is sobering. A 2024 Stanford study found that general-purpose language models hallucinated on 58\% to 88\% of specific, verifiable questions about federal court cases \autocite{dahl2024large}. A companion study of \emph{purpose-built} legal research tools---systems explicitly marketed as nearly hallucination-free---found that even these still fabricated information between roughly 17\% and 33\% of the time \autocite{magesh2024hallucination}. And in medical AI, the uncertainty behind diagnostic outputs is rarely communicated in ways that clinicians can interpret or act on \autocite{kompa2021second}. Even retrieval-augmented systems---models that can look things up---hallucinate at measurable rates. These figures were measured on early 2024 systems; subsequent model generations have reduced, though not eliminated, these errors.
...
\paragraph{Uncertainty Detection.} The calibration picture for language models is nuanced: base models can be reasonably well calibrated on multiple-choice formats, but the free-form verbalized confidence users actually encounter often is not \autocite{kadavath2022language}. They express uncertainty through hedging language (``I think,'' ``I believe,'' ``you may want to verify''), but this hedging is learned stylistic behavior, not a reflection of actual probability estimates. The challenge: the model generates output token by token with no natural point where genuine uncertainty is computed and communicated.

\paragraph{Intervention Design.} The best implementations use a tiered approach: answer directly when confidence is high and stakes are low; hedge and suggest verification when confidence is low or ambiguous; refuse and escalate when stakes are high and confidence is insufficient. The chatbot that says ``For specific legal advice, please consult a licensed attorney'' is practicing good intervention design.
...
The attorney's problem was not that ChatGPT was a flawed prototype that future versions would fix. It was that ChatGPT is, at its core, a very sophisticated text prediction machine---and text prediction machines, however sophisticated, do not have a relationship to truth. They have a relationship to plausibility. Newer models have narrowed that gap, but closing it requires humans in the loop.
...
\begin{description}
    \item[\emph{Mata v.\ Avianca} (2023)] Attorneys sanctioned for submitting AI-fabricated legal citations; demonstrates the hallucination problem's real-world consequences
...
\begin{itemize}
    \item \textit{Uncertainty Detection} (Dimension~1): Hallucination is confident wrongness---a language model that cannot signal what it does not know fails this dimension by construction
    \item \textit{Stakes Calibration} (Dimension~4): The Avianca brief shows what happens when a high-stakes domain inherits a system calibrated for low-stakes fluency
    \item \textit{Feedback Integration} (Dimension~5): RLHF and its successors are feedback integration at industrial scale---human preferences, not just labels, shaping what the model says
\end{itemize}
