...
Ian Goodfellow and colleagues at Google published a landmark 2015 paper demonstrating that you could take any image a neural network classifies correctly, add a carefully calculated pattern of noise---often invisible to the human eye---and cause the network to classify the image as anything you want, with near-absolute confidence \autocite{goodfellow2015explaining}.
...
This fundamental difference in how machines see creates different challenges in different high-stakes applications. Let's examine three in detail.
...
Modern radiology AI systems can detect early-stage lung cancer in chest CTs and flag pneumonia in chest X-rays with accuracy competitive with experienced radiologists on the specific tasks they've been trained for \autocite{ardila2019lung, rajpurkar2017chexnet}.
...
When the AI confidently flags an abnormality, radiologists are more likely to agree with the AI's characterization. And when the advice itself is wrong, it drags the human down with it: in a controlled study, radiologists given deliberately inaccurate advice made significantly worse diagnoses than those given accurate advice---regardless of whether the advice was labeled as coming from an AI or from a human expert \autocite{gaube2021ai}. The AI's confident output acts as a cognitive anchor.
...
The FDA has developed a specific regulatory pathway for AI/ML-based medical devices \autocite{fda2021samed}. Over 1,000 AI-enabled medical devices had received FDA authorization as of 2025---the large majority in radiology---with the number growing rapidly each year. This regulatory architecture is itself a form of institutionalized \hitl: before any medical vision AI can be used on actual patients, it must demonstrate performance to a human oversight body with the authority to require changes.
...
\paragraph{Cruise's approach}---before its service was suspended following a 2023 incident in San Francisco, and GM ultimately wound the robotaxi business down entirely in late 2024---was a kind of middle path: vehicles with safety drivers in some conditions, without in others, with remote human monitoring in all conditions.
...
This is why adversarial examples are possible. The vulnerability exists precisely because the network's classification is based on statistical pattern matching, not conceptual understanding \autocite{goodfellow2015explaining}.

The practical implication is the same either way: machine vision and human vision are complementary, with different strengths, different failure modes, and different robustness profiles. The radiologist's eyes catch the unexpected finding the AI wasn't trained to detect. The AI catches the early-stage nodule the radiologist would have dismissed as artifact. The human in the loop is not compensating for a deficiency---they're contributing a genuinely different capability.
...
The next four chapters of this book turn from the modalities---language and vision---to the system around them: measurement, human psychology, fairness, and failure. In each domain, the five dimensions take on specific texture. The timing constraint of a fraud detection queue looks nothing like the timing constraint of a medical imaging review. The stakes of getting a hiring recommendation wrong are not the stakes of getting a moderation decision wrong.
...
\begin{description}
    ...
    \item[Adversarial examples (Goodfellow et al., 2015)] Invisible pixel perturbations that fool neural networks with near-absolute confidence
    ...
    \item[FDA AI device authorization pathway] Institutional \hitl for medical vision AI---over 1,000 authorized devices as of 2025
    \item[Waymo vs.\ Tesla vs.\ Cruise] Three fundamentally different architectures for human oversight in autonomous vehicles---two survivors and a middle path GM shut down in late 2024
...
\begin{itemize}
    \item \textit{Uncertainty Detection} (Dimension~1): Adversarial examples and distribution shift mean a vision model's confidence can be highest exactly where it is most wrong---external checks, not self-reports, must carry the signal
    \item \textit{Timing} (Dimension~3): The autonomous-vehicle handoff shows timing as physics---a human alerted 1.5~seconds before impact is not meaningfully in the loop
    \item \textit{Stakes Calibration} (Dimension~4): The FDA authorization pathway is stakes calibration institutionalized---medical vision AI must prove itself to a human oversight body before touching a patient
\end{itemize}
