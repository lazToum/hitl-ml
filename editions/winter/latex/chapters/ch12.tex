...
\label{chap:ch12}
\begin{chapterquote}{on the measurement trap}
...
Picture a mid-sized e-commerce platform---the case that follows is a composite, but every element of it is drawn from documented moderation failures. The platform deployed a content moderation system to screen product listings for policy violations. The algorithm handled roughly 85\% of listings automatically. The rest---borderline cases involving possibly counterfeit goods, prohibited items, or ambiguous descriptions---got routed to a team of human reviewers.
...
In human-AI collaboration, the consequences are particularly severe because feedback loops are tight, mistakes are invisible, and the model gets smarter---in exactly the wrong direction.
\end{cautionbox}
...
A system where auto-processed cases achieve 98\% accuracy and human-reviewed cases achieve 94\% accuracy might look fine in aggregate. But it is telling you something important: the cases humans review are harder for humans too. The question is whether human review is adding value, or whether the system is routing the wrong cases, or whether reviewers are fatigued and not giving hard cases the attention they deserve.
...
\paragraph{Model accuracy and calibration.} Accuracy captures raw performance. \term{Calibration} captures something subtler: does the model's expressed confidence correspond to its actual reliability? A well-calibrated model that says it is 80\% confident should be right about 80\% of the time. A poorly calibrated model misroutes cases in both directions: confident-but-wrong cases sail through unreviewed, while cases it could have decided correctly clog the human queue.
...
\paragraph{Human workload.} How many cases per hour is each reviewer handling? How does this change over a shift? Research on human performance consistently shows a fatigue curve: in vigilance studies, detection performance degrades measurably within the first 20--30 minutes of a monitoring task \autocite{warm2008vigilance}. A HITL system routing 500 cases per hour to a team of five implicitly assumes each reviewer can sustain 100 per hour with consistent quality. That assumption is almost certainly wrong for anything requiring genuine judgment.
...
\paragraph{Cost per decision.} Human review is expensive. This needs to be measured explicitly, because cost shapes every design tradeoff. A system routing 40\% of cases to human review when 5\% would achieve similar quality wastes resources that could be spent on better interventions elsewhere.
...
There is a failure mode in HITL measurement that deserves its own name: \term{the divergence trap}---optimizing short-term accuracy at the cost of long-term learning and coverage.
...
The British economist Charles Goodhart made a related observation in 1975; the crisp formulation usually quoted---``when a measure becomes a target, it ceases to be a good measure''---is actually the anthropologist Marilyn Strathern's later restatement of his idea \autocite{strathern1997improving}. The principle has been rediscovered repeatedly across healthcare, education, software development, and now AI---because it describes something fundamental about how people respond to measurement.
...
\begin{itemize}
    ...
    \item A complete HITL measurement framework covers seven dimensions: model accuracy and calibration, human workload, time-to-decision, cost per decision, error rates by pathway, reviewer agreement, and feedback loop latency
...
\begin{itemize}
    \item \textit{Feedback Integration} (Dimension~5): Measurement is how the loop proves it is learning---feedback-loop latency and pathway error rates are the core signals
    \item \textit{Stakes Calibration} (Dimension~4): Tracking error rates by pathway and auditing autonomous cases keeps the system honest about where mistakes carry weight
    \item \textit{Intervention Design} (Dimension~2): What you choose to measure is itself an intervention---it shapes what reviewers and organizations optimize for
\end{itemize}
...
\noindent\textit{The next chapter turns from measuring the system to the human inside it: the cognitive biases, expertise effects, and fatigue that shape how people actually behave in the loop.}
