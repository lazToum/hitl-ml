...
\label{chap:ch13}
\begin{chapterquote}{on the irony of reliable automation}
...
At 2:10 in the morning on June~1, 2009, Air France Flight~447's autopilot disconnected over the South Atlantic Ocean. Four minutes later the aircraft hit the ocean. All 228 people aboard were killed. It remains the deadliest aviation accident in Air France's history \autocite{bea2012af447}.
...
The crew had, for the previous several hours, been managing a comfortable, mostly automated flight. Then, in the space of a few seconds, they were thrust into a rapidly deteriorating emergency with faulty and conflicting sensor data---a situation requiring immediate, accurate action on the aircraft's manual controls.

What the flight data recorders revealed was devastating: the pilots were confused. One pilot pulled back on the stick, increasing the plane's angle of attack even as the aircraft stalled. The confusion lasted four and a half minutes, from autopilot disengagement to impact with the ocean.
...
The paradox runs deeper than practice alone. When a system is highly reliable, there is a rational case for trusting it. If an AI flagging system has 98\% precision, a reasonable reviewer should doubt their own judgment when it conflicts with the AI's recommendation---that's sensible Bayesian updating. The problem is that this rational deference, taken too far, becomes \term{automation bias}.
...
Undertrust manifests as systematic distrust of algorithmic recommendations even when the algorithms are accurate: clinicians who treat AI flags as ``suggestions'' and override them at high rates even when the flags were correct; judges reluctant to ``let an algorithm decide'' who override risk predictions at rates statistical accuracy would not justify.
...
Experts are more susceptible to \term{expectation confirmation}---the \term{consistency trap}: strong prior beliefs about what they will find, and strong expectations about what a decision ``should'' look like, lead them to process new evidence in light of those priors and to dismiss AI outputs that violate those expectations---even when the outputs are correct. An experienced radiologist who ``knows'' that a certain pattern correlates with benign disease may be less willing to register a finding that contradicts that expectation, even when an AI correctly flags it.
...
A 2011 study by Danziger and colleagues examined 1,112 judicial hearings. The probability of a favorable parole decision dropped from roughly 65\% at the start of each session to nearly zero toward the end, before recovering after breaks \autocite{danziger2011extraneous}. To the extent the effect is real, it operated below the judges' awareness.
...
The finding points to something well-established in human performance research: decision quality declines under sustained cognitive load. Annotation quality, medical diagnosis accuracy, content moderation decisions, and financial review accuracy all follow this pattern. All are better early in a work session and worse late. The most rigorous documentation comes from medicine: in one study, physicians' antibiotic prescribing decisions degraded measurably over the course of a clinic day \autocite{linder2014time}.
...
\begin{itemize}
    ...
    \item The expertise effect: domain experts catch more errors but resist disconfirming AI recommendations more strongly
...
\begin{itemize}
    \item \textit{Intervention Design} (Dimension~2): When the recommendation is revealed, how confidence is displayed, and whether explanations are shown all shape automation bias
    \item \textit{Feedback Integration} (Dimension~5): Calibrated trust is built through feedback loops over time, not through instruction alone
    \item \textit{Timing} (Dimension~3): Fatigue and time-of-day effects mean the same reviewer performs differently depending on when the case arrives
\end{itemize}
...
\noindent\textit{The next chapter turns from individual psychology to collective consequences: how \hitl systems become fair or unfair, and why human oversight alone does not guarantee ethical outcomes.}
