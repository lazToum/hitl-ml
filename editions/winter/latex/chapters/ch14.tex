...
% ============================================================================
% Source: /markdown/ch14/Ch14_final.md
% ============================================================================

\label{chap:ch14}
\begin{chapterquote}{on the myth of neutral oversight}
Human oversight does not automatically produce human fairness.
\end{chapterquote}

% ============================================================================
\section{The Proxy Problem}
\label{sec:ch14-proxy}
% ============================================================================

In 2018, Reuters reported that Amazon had disbanded an experimental recruiting tool it had been developing since 2014. The \ai was designed to screen r\'{e}sum\'{e}s and surface top candidates for software engineering positions. Amazon's machine learning team had trained it on ten years of hiring data---the r\'{e}sum\'{e}s of everyone the company had actually hired.

The problem: Amazon's engineering workforce had historically been predominantly male. The model learned this pattern. It taught itself that women's r\'{e}sum\'{e}s were less suitable, even though gender was not an explicit input variable. It found proxies. The model learned to penalize r\'{e}sum\'{e}s that included the word ``women's''---as in ``women's chess club'' or ``women's college.'' It downgraded graduates of two all-women's colleges.

Here is the part of the story that matters most. Amazon's engineers discovered the gender bias themselves, roughly a year into development. They tried to fix it. They edited the model to neutralize the gender-correlated terms---but that was no guarantee the model would not devise other ways of sorting candidates that could prove discriminatory. And the tool's results were underwhelming in their own right, its recommendations little better than random, sometimes surfacing unqualified candidates. Executives lost hope, and the project was disbanded before it was ever deployed as a decision system. Amazon stated that its recruiters looked at the recommendations the tool generated when searching for new hires, but never relied solely on its rankings to make hiring decisions \autocite{dastin2018amazon}. No applicant was ever rejected by this tool; the harm it could have done stayed hypothetical, caught in development rather than discovered in production---which is exactly what makes the case worth studying.

The Amazon recruiting case is instructive not as a story of an oversight failure that went undetected for years, but as a demonstration of a deeper structural problem: bias is not a bug you can patch. It is a signal the model correctly learned from biased data. The model was doing exactly what it was trained to do. The data reflected a biased world. The model faithfully encoded it. And the fix---removing proxies one by one---was unwinnable, because the entire training distribution was saturated with structural gender patterns.
...
\paragraph{Collection bias} is the most fundamental: whose data is collected? Early facial recognition datasets, used to train and evaluate models for decades, were overwhelmingly composed of light-skinned male faces. Models trained on these datasets were well-calibrated for some users and dramatically worse for others. A 2018 study by Joy Buolamwini and Timnit Gebru---the \term{Gender Shades} project---found commercial gender-classification systems from major technology companies had error rates up to 34~percentage points higher for darker-skinned women than for lighter-skinned men \autocite{buolamwini2018gender}. The systems had not been designed to fail for these groups. They had simply been trained primarily on data that did not include them.

The same pattern holds geographically. In 2017, researchers at Google showed that image classifiers trained on the field's standard datasets---collected overwhelmingly from North America and Europe---performed far worse on images from the rest of the world: wedding photographs from Ethiopia and Pakistan drew the lowest recognition confidence, bridegrooms mislabeled with guesses like ``chain mail'' and ``academic gown'' \autocite{shankar2017geodiversity}. A \textit{Nature} commentary the following year supplied the pattern's most famous illustration: a bride in a white Western dress labeled \emph{bride} and \emph{wedding}; a bride in Indian wedding clothing labeled \emph{costume} and \emph{performance art} \autocite{zou2018ai}. The classifiers had not learned what a wedding is; they had learned what weddings look like in the places the data came from.
...
\textcite{chouldechova2017fair} proved that when the base rate of the predicted outcome differs across groups---whatever the reason for the difference---it is mathematically impossible for a model to simultaneously satisfy calibration and equal false positive rates.
...
First, humans reviewing \ai outputs can absorb and launder the \ai's biases rather than catching them. The Amazon case is the exception that shows what catching bias looks like: the bias surfaced because a dedicated engineering team went looking for it during development, not because routine review flagged it in production. When reviewers trust the system, when the system appears to be working, when the organizational culture treats the algorithm as a tool rather than as an autonomous agent, human review provides accountability theater rather than genuine oversight.
...
\paragraph{Explainability.} Understanding why a system made a specific prediction. If a loan application is rejected and the applicant asks why, they should receive an explanation that is genuinely informative. ``The algorithm decided'' is not an explanation; it is an evasion. The EU's General Data Protection Regulation (Articles 13--15) \autocite{eu2016gdpr} grants individuals a right to ``meaningful information about the logic involved'' in automated decisions, and the EU Court of Justice---in Case C-203/22, \emph{Dun \& Bradstreet Austria} (2025) \autocite{cjeu2025dunbradstreet}---has read this as requiring a concise, intelligible explanation of the procedure and principles actually applied to the individual---not a disclosure of the algorithm itself, but not an evasion either.
...
It requires diverse teams---not diversity as a compliance gesture, but because the failure modes of \ai systems are often visible to people with relevant lived experience and invisible to those without it. The facial recognition systems that failed for darker-skinned faces were built and tested primarily by teams that did not include many people with darker skin. Homogeneous teams have systematic blind spots.
...
\begin{description}
  \item[Amazon recruiting \ai (2018)] Model learned gender proxies; engineers caught the bias roughly a year into development, but each fix produced new proxies; project killed before deployment as unfixable
...
\begin{itemize}
  \item \textit{Stakes Calibration} (Dimension~4): Fairness is a stakes question---who bears the cost when the system is wrong, and across which groups
  \item \textit{Intervention Design} (Dimension~2): Human oversight is a design choice with real limits; without fairness infrastructure it becomes accountability theater
  \item \textit{Feedback Integration} (Dimension~5): Feedback loops can amplify bias---predictive-policing arrests feed back into the model and intensify the original disparity
\end{itemize}
...
\noindent\textit{The next chapter turns from fairness to failure: what goes wrong when \hitl systems break down in practice, how to trace the breakdown to its source, and what good remediation looks like.}
