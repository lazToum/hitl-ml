...
% ============================================================================
% Source: /markdown/ch17/Ch17_final.md
% ============================================================================

\label{chap:ch17}
\begin{chapterquote}{on asking better questions}
The most valuable capacity an \ai system can develop is to locate its uncertainty precisely and ask about it efficiently.
\end{chapterquote}
...
Medical educators have a name for one of the hardest things to teach: \term{calibration}---the fit between how much a trainee knows and how much they believe they know. Track students through clinical-reasoning tasks and the two come apart easily; early students are frequently miscalibrated, confident where they should hesitate and hesitant where they are in fact right \autocite{cleary2019calibration}.

The students who improve fastest are the ones who learn to close that gap---not by knowing more, exactly, but by locating their uncertainty precisely. Most learners arrive with questions they already know the answer to, designed to elicit the attending physician's ``yes, you're right''---a way of seeking reassurance rather than knowledge. The more useful question sounds different: ``I think it is likely A or B. I have ruled out C for this reason. What I do not know is whether this presentation is consistent with D---is that worth pursuing?'' Not ``I do not know''---that is too vague to be actionable. Not ``I know''---that forecloses learning. But ``I know A and B and C, and the specific thing I do not know is X''---that is a question a teacher can answer.

This capacity---to locate uncertainty precisely and ask about it efficiently---is one of the most valuable things an \ai system can develop. And a suite of techniques---some decades old, some very new---has emerged that begins to give \ai systems something like this capacity.
...
\term{Active learning} starts from an observation: not all data points teach a model equally. Some examples confirm what the model already knows. Others teach it something new. Active learning is a framework for directing human labeling effort toward the examples that will improve the model most efficiently.
...
\paragraph{Expected model change.} Route the cases that, if labeled, would change the model's parameters most significantly. Computationally intensive, and not always aligned with error reduction.

In practice, hybrid approaches combining uncertainty and diversity sampling tend to work well \autocite{settles2009active}. The practical benefit: on tasks where it works well, active learning can reach the same model performance with substantially fewer labeled examples than random sampling---but the savings are task-dependent, ranging from dramatic to modest, and occasionally negative \autocite{settles2009active}. For organizations where annotation is expensive, the gains, when they come, are worth pursuing.
...
Active learning creates a virtuous dynamic: the model gets better at knowing what it does not know, which makes its queries to humans more efficient, which makes the human labels more targeted, which makes the model improve faster.
...
Most \hitl systems are reactive: the \ai processes a case, registers its uncertainty, and then asks for help. The human responds when asked.
...
For language models, proactive \hitl means the system recognizes when a user's request is in a category where its outputs are less reliable, and flags this proactively: ``I should mention that my information on this topic may be outdated---you may want to verify with a current source.'' This is not the system being asked to express uncertainty. It is the system recognizing its limitations \autocite{kadavath2022language} and surfacing them without being asked.
...
This matters most as \ai systems take on longer and more open-ended work. An agent that runs for hours---writing and revising code, gathering and synthesizing research, carrying a single task across many steps---faces a problem that a one-shot prediction does not: small errors compound. Context drifts, an early wrong assumption propagates, and by the end the output can be subtly off in ways that are hard to trace back to their source. The defense is not to watch every step, which would defeat the purpose, but to build in \emph{checkpoints}: moments where a human re-orients, verifies what has accumulated, and corrects course before the divergence grows. For long-running systems, the human in the loop is less a reviewer of each action than a navigator who keeps the work pointed in the right direction---which is, not coincidentally, how this very book was written.

\begin{keyconcept}[The Almost-Autonomous Ideal]
This is the capacity those well-calibrated students had. They were not fully autonomous---they still had things to learn. They were not dependent on supervision for everything---they handled most cases on their own. They were almost-autonomous: capable and independent in their areas of competence, precise and honest about their areas of uncertainty.
...
\begin{itemize}
  \item Active learning directs human labeling effort toward the examples that improve the model most efficiently---on favorable tasks reaching equivalent performance with substantially fewer labels, though the savings vary
...
\begin{description}
  \item[Active learning sample efficiency] Substantial, task-dependent reductions in annotation needs through intelligent case selection
...
\noindent\textit{In the next chapter, we turn from how the loop works to a question this book has so far left standing: who owns it---the data, the models, the labor, and the access.}
