...
\begin{itemize}
    ...
    \item \textbf{Timing:} After a few episodes or roughly 90 minutes (thresholds vary by device)---rules-based, predictable
...
Timing & A few episodes or $\sim$90 min (varies by device) & Acts silently on its own schedule \\
...
training_data = {
    ...
    'cost_parameters': {
        'false_positive_cost': unnecessary_biopsy_cost,  # ~$200
        'false_negative_cost': missed_diagnosis_cost,    # delayed treatment, ~$50,000+
...
\subsection{Activity 2: Framework Application Workshop---Solution Guide}

\textbf{Note:} This is a facilitated discussion activity; accept any analysis that
engages all five dimensions with specific evidence. The sample below models the
expected depth for one system.

\paragraph{Sample Analysis: Banking Fraud Detection.}
\begin{description}
    \item[Uncertainty Detection] Strong (4--5): anomaly scores on amount,
        location, and merchant; low scores trigger step-up verification.
    \item[Intervention Design] Moderate (3): push notification with
        ``Was this you?'' is clear, but a blocked card with no explanation
        strands travelers.
    \item[Timing] Strong (4): asks at the point of sale, synchronous with
        the decision.
    \item[Stakes Calibration] Moderate (3): thresholds rarely adapt to
        amount or to the cost of a false decline.
    \item[Feedback Integration] Strong (4): confirm/deny responses retrain
        the customer's spending model.
\end{description}

\paragraph{Recommended Improvement (example).} Explain declines in the
notification (``unusual location + new merchant'') so users can confirm
efficiently instead of calling the bank---an Intervention Design fix.

\paragraph{Facilitation Notes.}
\begin{itemize}
    \item Google Photos face tagging typically surfaces weak Stakes
        Calibration (same ask for a mis-tagged friend and a mis-tagged
        stranger in a shared album).
    \item Spotify/Amazon recommendations usually show weak Intervention
        Design---they act silently and offer no easy way to correct them.
    \item Push groups past ratings (``it scores a 4'') to evidence
        (``it scores a 4 \emph{because}\ldots'').
\end{itemize}

\subsection{Activity 3: Nest Thermostat Redesign---Solution Guide}
...
\textbf{Note:} Exercise~4 below implements the same redesign with slightly different constants (35/95 instead of 32/90). Treat the specific cutoffs as illustrative design choices, not canonical values.
...
\subsection{Activity 4: Cost-Benefit Analysis---Solution Guide}
...
# Result: With this linear cost model, reviewing all email (r=1.0) minimizes cost.
# To get an interior optimum (~30% review), the model would need a
# confidence-ordered marginal-benefit curve rather than flat per-email costs.
\end{lstlisting}
...
class DropoutClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = self.fc3(x)
        return x  # raw logits; nn.CrossEntropyLoss applies softmax itself
...
\subsection{Exercise 4: Nest Thermostat Redesign---Complete Solution}
