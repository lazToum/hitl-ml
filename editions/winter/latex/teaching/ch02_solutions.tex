...
Framework-aligned answers enabling instructors to facilitate discussions on uncertainty with confidence and consistency. Discussion questions without a solution here are open-ended seminar prompts; target insights for every question appear in the teacher manual.
\end{chapterquote}
...
\textbf{Prompt:} ``The radiologist's AI didn't give the right diagnosis but still saved the patient. How? What exactly did the AI do correctly?''
...
\begin{enumerate}
    \item Triggered human review (the radiologist looked more carefully)
...
\begin{description}
    \item[A-grade response] ``The AI correctly implemented the Uncertainty Detection dimension of the HITL framework. By outputting 51\% confidence rather than a confident diagnosis, it signaled to the radiologist that the case was at the edge of its training. This is the system working as designed --- the AI's value wasn't in diagnosing the fungal infection but in flagging that something unusual was happening. The 51\% made the uncertainty visible and actionable.''
...
\begin{itemize}
    \item ``The AI found the infection'' --- No, the CT scan found it. The AI triggered the CT.
...
Outcome & Rare infection caught & Customer given wrong policy, financial loss \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Prompt:} ``A voice recognition system has 40\% word error rate on a dialect underrepresented in its training data but 5\% on the dialect that dominated training. What type of uncertainty? Should it flag its output differently?''

\paragraph{Model Answer.}

\paragraph{Type:} Epistemic uncertainty --- the model was trained primarily on the dominant training dialect and lacks sufficient examples of the underrepresented one. The accent is clear to any human listener; the model simply hasn't learned to process it.
...
\paragraph{Broader point.} Presenting a transcript in an underrepresented dialect with the same visual confidence as one in the dominant dialect is a design choice to hide epistemic uncertainty---the Nest thermostat problem applied to speech recognition.

% ============================================================================
\subsection{Intermediate Level Solutions}
% ============================================================================

\subsubsection{Question 1: Framework Application --- Radiology AI, Five Dimensions}
...
Feedback Integration & ?/5 & Unknown --- does the radiologist's correct diagnosis improve the model? \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Question 2: Design Thinking --- Legal Document Review AI}

\textbf{Prompt:} ``Design an uncertainty signaling strategy for a legal document review AI. The AI reads contracts and flags potential issues. When should it express uncertainty? How?''

\paragraph{Model Answer.}

\textbf{When to express uncertainty:}
\begin{itemize}
    \item When clause language matches no well-represented pattern in training data (epistemic --- unusual jurisdiction, novel clause structures)
    \item When two risk categories are near-equally scored (aleatoric --- genuinely ambiguous drafting)
    \item \emph{Not} on routine clauses where the model is accurate --- flags there create alert fatigue and erode attorney trust
\end{itemize}

\textbf{How:}
\begin{itemize}
    \item Show the flag \emph{with its basis}: ``flagged because this indemnification clause differs from patterns the model knows,'' not a bare risk score
    \item Distinguish severity from confidence --- a high-risk, high-confidence clause and a low-confidence clause need different attorney actions (substantive review vs.\ uncertainty resolution)
    \item Offer a quick confirm/override control so the attorney's resolution feeds back as training signal
\end{itemize}

\textbf{HITL rationale:} contract review is a high-stakes, low-volume setting --- the cost asymmetry favors routing more cases to humans than a spam filter would, but the same calibration logic applies: route on uncertainty, not on everything.

\subsubsection{Question 3: Calibration Analysis --- Fraud Detection}

\textbf{Prompt:} ``A fraud detection system reports 95\% confidence on 10,000 transactions. It's wrong on 800 of them. Is this system well-calibrated? What does this mean for HITL routing?''

\paragraph{Model Answer.}

No. At 95\% average confidence, a calibrated system should be right about 95\% of the time --- wrong on roughly 500 of 10,000. It is actually wrong on 800 (8\% error = 92\% accuracy). The system is \textbf{overconfident}: confidence exceeds realized accuracy by about 3 percentage points.

\textbf{For HITL routing:}
\begin{itemize}
    \item Routing thresholds set on raw confidence are misplaced --- cases the model calls 95\% certain deserve less trust than the number suggests
    \item Recalibrate first (e.g., temperature scaling on a held-out set from the deployment stream), then set routing thresholds on the calibrated scores
    \item Until recalibrated, widen the human-review band on the high-confidence side --- sample some ``confident'' decisions for audit, because overconfidence is exactly where autonomous channels fail silently
\end{itemize}

\subsubsection{Question 4: Trade-off Analysis --- Content Moderation}

\textbf{Prompt:} ``An image classifier for content moderation is 99\% accurate on 85\% of images, but drops to 65\% accurate on the remaining 15\%. Should the HITL system route all 15\% to human review, or try to distinguish within that group?''

\paragraph{Model Answer.}

Distinguish within the group --- but verify the group is identifiable first.

\textbf{Why not route all 15\%:} at 65\% accuracy the subgroup genuinely needs help, but 15\% of total volume is a large, permanent review load; if reviewers cannot absorb it, queues grow and review quality degrades (the capacity problem from Chapter~4). Worse, the 15\% is likely heterogeneous: some cases are hopeless even for humans, some are easy for humans though hard for the model.

\textbf{The better design:}
\begin{enumerate}
    \item Check whether the 15\% is detectable \emph{before} inference (e.g., a recognizable content category, language, or image type). If yes, that detection --- not the model's confidence --- becomes the routing signal.
    \item Within the subgroup, route by uncertainty: humans first handle cases where their advantage over 65\% is largest; auto-decide (or defer) only where even humans would not do better.
    \item Treat the subgroup as a data-collection priority --- it is an epistemic gap; targeted labeling there reduces the 15\% over time, which routing alone never will.
\end{enumerate}

\textbf{Key insight:} a confidence drop this localized is a training-data gap wearing a routing problem's clothes. Route in the short term; fix the data in the long term.

\subsubsection{Additional Worked Example: College Admissions Essay AI}
...
\subsubsection{Question 2: Calibration Strategy --- Cross-Hospital Deployment}
...
\subsection{Activity 4: Five-Dimension Framework Audit --- Sample Completed Worksheet}

\textbf{Note:} This audit applies the five-dimension framework to the radiology AI from Chapter~2's opening; accept any ratings defended with specific evidence. The sample below models the expected depth (it mirrors Intermediate Question~1's analysis).

\begin{table}[htbp]
\caption{Sample completed worksheet: radiology AI audit}
\label{tab:radiology-audit-solution}
\centering
\begin{tabular}{@{}p{3.5cm}p{2cm}p{3.5cm}p{3.5cm}@{}}
\toprule
\textbf{Dimension} & \textbf{Rating (1--5)} & \textbf{Evidence} & \textbf{Improvement} \\
\midrule
Uncertainty Detection & 5 & Outputs 51\% confidence; system knows it is uncertain & None needed \\
Intervention Design & 4 & Flags ``recommend clinical review'' --- clear action & Explain \emph{why} uncertain \\
Timing & 4 & Flag arrives before the radiologist commits & None needed \\
Stakes Calibration & 5 & Medical setting; high bar for auto-diagnosis & Verify thresholds against error costs \\
Feedback Integration & 1--2 & No evidence the correction improves the model & Log radiologist verdicts as training labels \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Discussion answer.} The dimension most commonly ignored in real deployed systems is \textbf{Feedback Integration}: human corrections are typically logged for compliance but never re-enter training, so the same uncertain cases recur indefinitely.
...
\textbf{Subgroup analysis interpretation:} If a voice recognition system shows ECE $\approx$ 0.03 for the dominant dialect but ECE $\approx$ 0.25 for an underrepresented dialect, flagging inputs in that dialect for lower-confidence output is both technically correct and ethically required for a fair system.
...
\paragraph{Connection to Chapter~2.} The voice recognition system with high error rates on an underrepresented dialect would benefit from Mahalanobis-style OOD detection: if the audio feature representation is far from the centroid of any trained accent cluster, flag for low confidence regardless of what the softmax says.
...
Human review costs time, attention, and money. At scale: a spam filter processing 100 million emails per day cannot route all 100 million to human reviewers. Even at 1 second per email, full review would require over a thousand person-days of review labor every day. Calibrated routing selects the cases where human judgment adds the most value.
