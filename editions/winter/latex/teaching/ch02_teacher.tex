...
Open with the radiologist's story before introducing any vocabulary. Ask students what the AI did right \emph{before} telling them. Most students will initially say the AI failed (it didn't diagnose correctly). The reveal --- that the 51\% score was the valuable information --- creates a memorable cognitive shift.
\end{technote}
...
Tell the radiologist's story without revealing the punchline (the 51\% score) until you've asked the class what happened.
...
\begin{enumerate}
    \item \textbf{Recognition:} ``The radiologist's story shows an AI that didn't give the right diagnosis but still saved the patient. How? What exactly did the AI do correctly?''\\
    ...
    \item \textbf{Application:} ``A voice recognition system has 40\% word error rate on a dialect underrepresented in its training data but only 5\% on the dialect that dominated training. What type of uncertainty is this? Should the system flag its output differently for speakers of the underrepresented dialect? How?''\\
...
\begin{itemize}
    ...
    \item The radiologist's AI was uncertain about a case that turned out to be genuinely difficult
\end{itemize}
