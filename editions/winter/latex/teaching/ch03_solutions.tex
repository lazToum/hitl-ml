...
Framework-aligned answers enabling instructors to facilitate discussions on machine learning foundations and the structural origins of the generalization gap. Discussion questions without a solution here are open-ended seminar prompts; target insights for every question appear in the teacher manual.
\end{chapterquote}
...
\subsubsection{Question 4: Vocabulary Check}

\textbf{Prompt:} ``In your own words, explain the difference between overfitting and the generalization gap. Are they the same thing?''

\paragraph{Model Answer.}

They are not the same thing. \textbf{Overfitting} is a property of a model relative to its training set: the model has learned patterns --- including noise and spurious correlations --- that do not generalize beyond the training data. It is diagnosed by a gap between training and held-out performance from the \emph{same} distribution, and it can in principle be reduced with more data, regularization, or a simpler model.

The \textbf{generalization gap} is a structural property of learning from finite data: deployment performance falls short of test performance whenever deployment inputs differ from the training distribution --- even for a perfectly fit, non-overfit model. Overfitting is one contributor to the gap; distribution shift, population coverage gaps, and adversarial adaptation are others.

\textbf{Why the distinction matters for HITL:} overfitting is addressed in the modeling loop (data, regularization); the residual generalization gap is addressed at deployment (uncertainty monitoring, human routing). Confusing them leads teams to keep retraining when the real problem is that the world has moved.

% ============================================================================
\subsection{Intermediate Level Solutions}
% ============================================================================

\subsubsection{Question 1: Medical AI in Kenya}
...
\subsubsection{Question 2: Gap Interpretation}
...
\subsubsection{Question 3: Overfitting and HITL Routing Failure}
...
\subsubsection{Question 4: Covariate Shift Design}

\textbf{Prompt:} ``A content moderation AI trained on 2019--2022 US/UK/AU English social media is deployed globally in 2026. Identify two distribution shifts that would produce high-confidence wrong answers rather than uncertain ones. Why is this the dangerous failure mode?''

\paragraph{Model Answer.}

\textbf{Shift 1: Adversarial concept drift.} Post-2022 attack patterns (new scams, new coded hate speech, AI-generated spam) are designed to superficially resemble safe content. The model maps them onto learned ``safe'' patterns and classifies them confidently --- and wrongly. The input is novel, but it does not \emph{look} novel to the model's features.

\textbf{Shift 2: Cultural distribution shift.} Deployed globally, the model sees cultural contexts where harmful patterns do not appear harmful to a model trained on US/UK/AU norms --- and vice versa (innocuous local idioms flagged as toxic). Again the model is confidently wrong in both directions, because its training distribution gave it no reason to doubt.

\textbf{Why this is the dangerous failure mode:} HITL routing relies on the model's uncertainty signal. Uncertain-but-wrong cases get routed to humans and corrected; high-confidence wrong cases sail through the autonomous channel with no flag and no review --- and at deployment scale they become the dominant source of harm. This is why confidence auditing (sampling \emph{high}-confidence outputs for human verification) matters as much as uncertainty routing.

% ============================================================================
\subsection{Advanced Level Solutions}
% ============================================================================

\subsubsection{Question 1: Bias-Variance and HITL Value}
