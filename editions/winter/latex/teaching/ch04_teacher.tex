...
\begin{technote}[Naming the Child Welfare Example]
The chapter presents this case as a generic ``child protective services AI.'' In class it helps to name the widely discussed real-world instance this example is based on: the Allegheny Family Screening Tool (AFST), referenced throughout this guide.
\end{technote}
...
\begin{enumerate}
    \item \textbf{Optimal Threshold Derivation:} ``Derive the optimal threshold formula $\tau^* = C_{FP} / (C_{FP} + C_{FN})$ from first principles using expected cost minimization. Explain why the class priors do not appear, and what happens to $\tau^*$ as $C_{FN}/C_{FP} \to \infty$.''
...
\begin{itemize}
    \item 0.5 is optimal only when the costs of each error type are equal (for a calibrated model, class balance does not matter --- the posterior already reflects it)
    \item For most real applications, that condition does not hold
    \item 0.5 is a choice with consequences, not a natural starting point
    \item The optimal threshold formula $\tau^* = C_{FP} / (C_{FP} + C_{FN})$, which holds for calibrated scores at any class balance, shows that only equal costs produce $\tau^* = 0.5$
\end{itemize}
