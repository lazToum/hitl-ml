...
Framework-aligned answers enabling instructors to facilitate discussions on HITL intervention design with confidence and consistency. Discussion questions without a solution here are open-ended seminar prompts; target insights for every question appear in the teacher manual.
\end{chapterquote}
...
\subsubsection{Question 1: Alert Fatigue in Clinical Settings}
...
\subsubsection{Question 2: Design Challenge --- Plagiarism Detection}

\textbf{Prompt:} ``Design a HITL intervention for a plagiarism detection system used by university professors. Specify: what gets flagged, when the professor is notified, what format the notification takes, and how the professor's response feeds back to the system.''

\paragraph{Model Answer.}

\textbf{What gets flagged:} Not raw similarity scores --- flag cases where the similarity signal is decision-relevant and the system is uncertain: high overall overlap with a single source, or mid-range scores where context matters (quoted material, common phrases, the student's own prior work). Auto-clear very low scores; auto-escalate only near-certain verbatim copying. The middle band goes to the professor.

\textbf{When:} Batch notifications to natural workflow breaks --- at grading time, when the professor opens the assignment --- never as real-time interruptions. Timing follows the grading session, not the submission event.

\textbf{Format:} Specific over generic, per the information-value principle: not ``similarity score 42\%,'' but side-by-side passages with the matched source, the matched text highlighted, and context (is the match quoted and cited? is it the assignment prompt itself?). The professor's question is answerable at a glance: ``Is this legitimate overlap or unoriginal work?''

\textbf{Feedback integration:} Record the professor's verdict (legitimate / plagiarism / needs conversation with student) per flagged passage, not per document. Use verdicts to recalibrate which match types reach the flag threshold --- matches professors consistently mark legitimate (cited quotations, boilerplate) get down-weighted, reducing future false flags and protecting against alert fatigue.

\subsubsection{Question 3: Information Value of Specific vs.\ Generic Alerts}
...
\subsubsection{Question 1: Content Moderation Platform Design}
...
\subsubsection{Question 2: Experiment Design for Alert Format Information Value}
...
\paragraph{Scenario B: University plagiarism detection.}

\textbf{Trigger:} Overall similarity above 70\% with a single dominant source, or mid-range scores where matched passages are uncited.

\textbf{Notification:} Inline in the grading interface: ``Submission 14: 78\% similarity, mostly with [source]. 4 of 6 matched passages are quoted and cited; the 2 uncited matches are highlighted.''

\textbf{Timing:} Batched into the grading session --- shown when the professor opens the assignment, never as a real-time interruption.

\textbf{Frequency:} One consolidated flag per submission; no repeat notifications for the same match.

\textbf{Feedback integration:} The professor's verdict (legitimate / plagiarism / needs conversation with the student) is recorded per flagged passage; match types consistently marked legitimate (cited quotations, assignment boilerplate) are down-weighted in future flags.

\paragraph{Scenario C: Smart home security.}

\textbf{Trigger:} Door unlocked for more than 15 minutes and no one detected home, with occupancy confidence (here: 82\%) below the auto-lock threshold.

\textbf{Notification:} Push alert: ``Front door unlocked for 20 minutes and I don't detect anyone home (82\% confident). [Lock now] [Someone's home --- leave unlocked] [Remind me in 30 min]''

\textbf{Timing:} After a sustained dwell (15--20 minutes), not instantly; escalate to an immediate alert if the door is still unlocked at night.

\textbf{Frequency:} At most one reminder per 30 minutes; suppressed entirely when a household member's phone is home.

\textbf{Feedback integration:} ``Someone's home'' responses teach the occupancy model the missed-presence pattern; repeated dismissals in the same context raise the confidence required for future alerts.
...
\subsection{Exercise 5A.1: Information Value Calculation}

\textbf{Setup:} Spam filter confidence 0.65 on an email; human reviewer 90\% accurate on boundary cases.

\paragraph{Part (a): Expected information gain.}
Treat the filter's belief as Bernoulli with $p = 0.65$. Prior entropy:
\[
\mathbb{H}(Y \mid X) = -0.65 \log_2 0.65 - 0.35 \log_2 0.35 \approx 0.93 \text{ bits.}
\]
After asking the reviewer, residual uncertainty is the entropy of the reviewer's error rate:
\[
\mathbb{H}(Y \mid H, X) \approx -0.90 \log_2 0.90 - 0.10 \log_2 0.10 \approx 0.47 \text{ bits.}
\]
\[
\text{IG} = 0.93 - 0.47 \approx 0.47 \text{ bits.}
\]

\paragraph{Part (b): Minimum email value.}
Cost of asking: $30~\text{s} \times \$0.01/\text{s} = \$0.30$. Review is worthwhile when $V_\text{decision} \cdot \text{IG} > C_\text{ask}$:
\[
V_\text{decision} > \frac{\$0.30}{0.47} \approx \$0.64.
\]
The email's decision must be worth at least roughly \$0.64 per bit of uncertainty resolved for human review to pay for itself.

\paragraph{Part (c): Filter confidence 0.95.}
\[
\mathbb{H}(Y \mid X) = -0.95 \log_2 0.95 - 0.05 \log_2 0.05 \approx 0.29 \text{ bits,}
\]
so $\text{IG} \approx 0.29 - 0.47 \approx -0.18$ bits --- \emph{negative}. A 90\%-accurate reviewer is less certain than the model itself at 95\% confidence: asking adds noise rather than removing it. This is the quantitative version of the chapter's core lesson --- ask humans where the model is uncertain, not where it is confident.

\subsection{Exercise 5A.2: Alert Fatigue Simulation}
...
\paragraph{Key result:} The additive walk above has no fixed point --- trust is a random walk whose \emph{drift} per alert is $\alpha p - \beta (1-p)$ for a system with precision $p$ (fraction of alerts that are actionable). Trust trends upward on average exactly when the drift is positive:
\[
p > \frac{\beta}{\alpha + \beta}.
\]
To state a steady state, model engagement as a two-state Markov chain: each actionable alert moves the user toward the engaged state with rate $\alpha p$; each false alert moves them toward disengagement with rate $\beta (1-p)$. The stationary distribution of that chain is:
\[
\text{Steady-state trust} = T^* = \frac{\alpha \cdot p}{\alpha \cdot p + \beta \cdot (1-p)}
\]

Note $T^* > 0.5$ (the minimum for active engagement) under the same drift condition: $p > \beta / (\alpha + \beta)$.
...
\subsection{Exercise 5A.3: Optimal Query Granularity}
...
\subsection{Exercise 5A.4: HITL Policy Optimisation}
