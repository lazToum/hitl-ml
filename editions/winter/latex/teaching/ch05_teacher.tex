...
The chapter's core argument: \textbf{the moment of the ask is a design problem, not just an engineering trigger}. Even when detection, routing, and threshold decisions are correct, the ask can fail if its design does not respect human psychology. In the chapter's opening story, a handful of small changes to what Dallas 911 dispatchers asked callers --- count the patient's breaths, describe what the breathing sounds like, hold the phone close enough to hear it --- changed how often cardiac arrest was recognized in time. This is not a marginal efficiency gain; it is the difference between a system that works and one that does not.

\begin{technote}[Teaching Tip]
Open with the 911 dispatcher story before introducing any vocabulary. Ask students: ``A caller reports that a man has collapsed. What would you ask to find out whether he is breathing?'' Then reveal what actually happened: the natural question --- ``Is he breathing?'' --- earned a reflexive ``yes'' from callers watching agonal gasping, and dispatchers missed cardiac arrests they should have caught. The Dallas fix was not a rephrased yes/no question but a different kind of ask: count the breaths, describe what the breathing sounds like, hold the phone next to the patient so the dispatcher can hear it. Then ask: ``Why did these changes matter so much?'' Lead students to: the question's format determines what the caller actually observes, and therefore what information they report.
\end{technote}
...
Tell the Dallas agonal-breathing story slowly. Before revealing the protocol change, ask students: ``What would you change about the question `Is he breathing?''' Then reveal the answer: not a rephrased yes/no question, but a set of observation tasks --- count the breaths, describe the breathing, hold the phone close.

\begin{trythis}
\textbf{Key question:} ``Why did the new asks work when `Is he breathing?' failed?'' Lead students to: the yes/no question let callers confirm what they already half-believed --- a gasping patient \emph{is} moving air, which reads as breathing. Counting, describing, and holding the phone close demand an act of observation rather than a confirmation, and agonal gasping fails the ``does it sound normal?'' comparison.
\end{trythis}
...
\midrule
0:00--0:07 & Opening: Dallas 911 story + discussion & Ch.\ 5 opening narrative \\
...
\begin{enumerate}
    \item \textbf{Recognition:} ``The 911 dispatcher story shows that the format of a question can dramatically change the information it produces. Give two examples from your own experience where how you were asked a question changed how you answered it.''\\
