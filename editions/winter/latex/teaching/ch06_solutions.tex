...
\textbf{Prompt:} ``The Dratsch study found that when an AI gave a deliberately wrong mammography suggestion, radiologists' accuracy collapsed --- even experienced readers fell from about 82\% to roughly 46\%. How is this possible?''
...
\begin{itemize}
    ...
    \item \textbf{OR} same judgment time per post ($33 + 5 = 38$ s/post), complete $\approx 495$ posts in the same time ($18{,}800 / 38 \approx 495$, +24\% throughput)
\end{itemize}

\textbf{Three reasons it matters:}
\begin{enumerate}
    \item \textbf{Throughput:} $\approx 24\%$ more reviews in the same time directly reduces moderation backlog.
...
\textbf{Prompt:} ``Write a technical post-mortem for a hospital where the deployed AI's performance materially degraded (the chapter's multi-site evaluation).''
...
\textbf{Contributing cause: No independent-read workflow.} Unlike the sequencing that worked in the chapter's reader study (radiologists read unassisted first, AI suggestion and explanation second --- Pesapane et al.), this hospital deployed the AI as a primary triage tool. Radiologists were effectively reviewing the AI's work rather than the scan itself.
...
\paragraph{Interface mechanics effect.} Reducing mechanical overhead from 14 to 5 seconds per item means less of each item's time is spent on depleting mechanical work and more on judgment --- qualitatively, fatigue accumulates more slowly and the quality degradation threshold is reached later in the session.

\paragraph{Crossing the $\alpha = 0.5$ threshold.} With the time-based model above, $\alpha(t) = 0.7 \cdot e^{-0.15t}$ falls below $0.5$ at $t = \ln(0.7/0.5)/0.15 \approx 2.24$ hours --- about 172 items at 47 seconds per item. (The appendix's per-item queue model $q(n)$, with $q_{\min} = 0.85$, separately gives $n_{\text{break}} \approx 139$ items; the two models answer different questions and should not be mixed.)
