...
\begin{itemize}
    ...
    \item \textbf{Chapters~2, 4, and 5} (uncertainty and calibration, HITL band, ask design and review queues) --- Chapter~6 addresses how cases routed through the HITL band are presented to human reviewers
...
\begin{description}
    ...
    \item[Medical informatics] CheXNet, COMPAS, and the mammography reader studies (Gaube, Dratsch, Pesapane) directly applicable
...
Chapter~6 works best organized around a central contrast: interfaces that support human judgment vs.\ interfaces that replace it. Use COMPAS (failure) and the Pesapane et al.\ mammography reader study (success) as the anchor pair.

\begin{table}[htbp]
\caption{COMPAS vs.\ Pesapane et al.\ reader study: interface contrast}
\label{tab:contrast-ch06}
\centering
\begin{tabular}{@{}p{3cm}p{4cm}p{4cm}@{}}
\toprule
\textbf{Dimension} & \textbf{COMPAS} & \textbf{Pesapane Reader Study} \\
\midrule
Sequence & Score shown before/during judgment & Human reads unassisted first, AI suggestion second \\
Explanation & None (proprietary) & Saliency heatmap showing where the AI looked \\
Independent judgment preserved? & No & Yes --- unassisted reads established each reader's own view \\
Override mechanism & Informal/cultural & Structured phases make the AI a second opinion \\
Outcome & Racial bias amplified, reasoning outsourced & Explanation cut automation and anchoring bias roughly in half \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Confidence bar:} A confidence signal provided but systematically misread --- taken as a cue for how much attention a case deserves rather than how sure the model is (the chapter's unnamed case)
    \item \textbf{LIME stability problem:} Small changes to the input can produce noticeably different explanations of the same underlying model behavior
\end{enumerate}
...
\subsubsection{Part 1: The Say--Do Gap (12 minutes)}

\textbf{Core claim:} Experts say they distrust AI advice more than a colleague's --- and then rely on both equally.

Present the Gaube et al.\ (2021) experiment:
\begin{enumerate}
    \item Same advice labeled ``AI'' vs.\ ``experienced radiologist'' $\to$ no difference in diagnostic accuracy
    \item Deliberately inaccurate advice $\to$ followed far too often from either source; over a quarter of radiologists accepted it every time
    \item Quality \emph{ratings} $\to$ radiologists scored AI-labeled advice significantly lower --- stated aversion, unchanged behavior
\end{enumerate}

\begin{trythis}
\textbf{Discussion prompt:} ``Which condition would you want your radiologist to use? Why?'' Take a vote. Then introduce the Dratsch mammography finding: a wrong AI suggestion dragged radiologists' accuracy down sharply --- even experienced readers fell from about 82\% to roughly 46\%. Let this land before continuing.
\end{trythis}

\subsubsection{Part 2: The Click Budget (10 minutes)}

Walk through the content moderation example. The chapter (drawing on Roberts's ethnography) gives only ``hundreds of posts per shift'' with average review times measured in seconds; the numbers below are a constructed classroom exercise to make the arithmetic concrete:
\begin{itemize}
...
Show the Stureborg annotation result: interface design alone changed label quality, with the largest gains on the hardest examples. Ask: ``What changed? The annotators were drawn from the same pool, looking at the same content.''
...
\midrule
LIME & Intuitive local explanation & Unstable under small input perturbations \\
SHAP & More stable, consistent values & Requires data literacy to interpret correctly \\
Grad-CAM & Visual; useful in imaging & Shows where the model looks, not whether those features are the right ones \\
...
Return to the multi-site evaluation from the chapter (Yang et al.): the same AI model, deployed across different hospitals, performed strongly where the local data resembled its training data and materially degraded elsewhere. Same model, different sites, different outcomes --- and part of that variation is how the model is put in front of people.
...
0:05--0:17 & Gaube say--do gap experiment + automation bias intro & Interface comparison slides \\
0:17--0:27 & Click budget, annotation-interface result, cognitive load & Cognitive load diagram \\
0:27--0:37 & XAI reality check: LIME/SHAP/Grad-CAM & Human utility table \\
0:37--0:45 & Four judgment-support principles + multi-site study & Design principles slide \\
...
\begin{enumerate}
    ...
    \item \textbf{Automation bias recognition:} ``The Dratsch study found that a wrong AI suggestion collapsed radiologists' accuracy on mammograms. How is this possible? What does it tell you about how the AI's recommendations were displayed?''\\
    ...
    \textit{Target: Time saved = $400 \times 9 = 3,600$ seconds = 60 minutes; or judgment time per post increases from 33 to 42 seconds (+27\%); or throughput rises to $\approx 495$ posts in the same total time (+24\%). Yes, it matters: calibration-adjusted for fatigue model, quality also improves.}
...
\begin{enumerate}
    ...
    \item \textbf{Safety case analysis:} ``The chapter's multi-site evaluation found the same AI model's performance materially degraded at some hospitals after deployment. Write a technical post-mortem. What interface design decisions likely contributed to the decline? What metrics should those hospitals have been monitoring?''
...
\begin{itemize}
    \item The chapter documents a confidence signal that was provided but misread (the confidence bar), explanations that are unstable under small input changes (LIME), and explanations with negative human utility for non-experts (SHAP)
...
\paragraph{Correction strategy.}
\begin{itemize}
    \item The chapter's reader studies show the harm is real: wrong AI suggestions collapsed radiologists' accuracy (Dratsch), and unassisted-then-assisted sequencing with explanation cut those biases roughly in half (Pesapane)
    \item The multi-site evaluation (Yang et al.) showed the same model helping where local data matched its training data and degrading elsewhere
    \item The variable was context and interface design, not AI presence
...
\begin{itemize}
    ...
    \item \textbf{Chapter~2:} Uncertainty detection and calibration --- Chapter~6 addresses how calibrated uncertainty gets displayed to human reviewers
