...
\begin{description}
    ...
    \item[Ethics/fairness courses] The ImageNet story, AAE toxicity findings, and demographic blind spot analysis are central case studies
...
Annotator background & AAE toxicity labeling by non-AAE speakers & Demographically matched annotator pool \\
Sequence effects & Items look milder after seeing extreme cases & Randomized item ordering \\
Fatigue effects & Quality degrades over long sessions & Session length limits; breaks \\
\bottomrule
\end{tabular}
\end{table}
...
Show Li's 2015 TED Talk clip (first 2--3 minutes). Then fast-forward to 2019 and the gender stereotype findings. Ask: ``What went wrong, and when?''
...
\begin{enumerate}
    \item \textbf{Sequence effects} --- the Mathur et al.\ machine translation quality study. Same translation, different rating depending on surrounding items. Design implication: randomized item ordering.

    \item \textbf{Demographics} --- the AAE toxicity study by Sap et al. Ask: ``If you were building a toxicity classifier for a platform used by a large AAE-speaking community, what would you want the annotator pool to look like?''

    \item \textbf{Fatigue} --- quality degrades over long sessions (any degradation curve shown is instructor-supplied; the chapter provides none). Session length limits. Connects to Chapter~6's vigilance and attention-budget discussion.
\end{enumerate}
...
0:29--0:42 & Annotator variation: sequence, demographics, fatigue & AAE study data \\
...
\begin{enumerate}
    ...
    \textit{Target: Over-label: content using AAE or non-standard English dialects that superficially resemble harmful content. Under-label: formally polite but culturally harmful content; harassment patterns specific to non-Western cultural contexts. Solution: demographically matched annotator pools; cultural consultants.}
...
\begin{itemize}
    ...
    \item AAE-language content that appears offensive out of context but is positive/neutral in context
...
\textbf{Debrief:} Compare labels across the room. Identify systematic patterns. Were AAE items disproportionately flagged? Were formally polite harmful items under-flagged? What does this tell us about annotator pool composition requirements?
...
\begin{itemize}
    ...
    \item \textbf{Chapter~5:} HITL intervention design; the annotation session is a HITL interaction; its timing, frequency, and format design levers all apply to annotation design
    \item \textbf{Chapter~6:} Interface design directly affects annotator performance; the Stureborg finding that interface design alone improves label quality is the Chapter~6 argument applied to Chapter~7 measurement
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Linguistics:} The Sap et al.\ AAE study; linguistic relativity; dialect and register variation
