...
The sensors detected her 5.6 seconds before impact. The failure was not detection --- it was what happened next. The classification never settled: it flipped from vehicle to unknown object to bicyclist, discarding its tracking history with each flip, and it never classified her as a pedestrian or predicted her path. No actionable uncertainty signal reached the driver. The failure was in the \textbf{routing component} (Component 3): the only escalation came 1.2 seconds before impact, when the system finally recognized an imminent collision --- and with emergency braking deliberately disabled, the design response was to alert a human who had been kept out of the loop. Too late.
...
class HITLFeedbackPipeline:
    ...
    def process_case(self, case, confidence):
        ...
        if confidence > HIGH_THRESHOLD:
            ...
            if random.random() < self.audit_rate:
                ...
                self.audit_buffer.append((case, action, human_label))
    ...
    def monthly_retrain(self):
        ...
        audit_errors = [(c, human) for c, auto, human in self.audit_buffer
                        if human != auto]  # Human disagrees with auto
