...
The downstream failure: the model appears to train normally, but its uncertainty estimates are unreliable because its training labels are unreliably noisy in exactly the regions of input space where uncertainty matters.
...
\midrule
1 & Prediction Layer & No & You build and serve the model \\
2 & Uncertainty Quantification & No & You build calibrated confidence scores \\
3 & Routing Layer & No & You build threshold logic and routing rules \\
4 & Interface Layer & Yes & The annotation UI is the platform's core \\
5 & Feedback Integration & Partial/No & Stores labels; connecting them back to retraining is your job \\
6 & Monitoring & Partial & Agreement tracking and throughput; not model drift \\
7 & Audit Layer & No & Decision and escalation logging is not included \\
\bottomrule
\end{tabular}
\end{table}
...
The annotation platform is the \emph{interface layer} of the HITL system. It implements Component~4, partially covers Components~5 and~6, but leaves the architecturally hard parts --- routing logic (Component~3) and closing the feedback loop (Component~5) --- for the engineering team to build. This is why the chapter's central observation holds: the annotation UX is not where the architectural moat is. The feedback integration layer is.
...
Quality checks should track correction rates by speaker demographic. If a reviewer is consistently ``correcting'' African American English (AAE) to standard forms, that is an annotation failure requiring retraining and re-annotation of affected labels.
...
\begin{itemize}
    ...
    \item For cases where the two readers disagree (per-item agreement below the majority threshold): convene a joint review session to clarify the edge cases driving disagreement
