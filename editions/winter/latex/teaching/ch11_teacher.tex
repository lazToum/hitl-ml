...
\toprule
\textbf{Feature} & \textbf{Tesla FSD} & \textbf{Waymo} & \textbf{Cruise (suspended 2023; wound down by GM late 2024)} \\
\midrule
...
Cruise is included even though its robotaxi business no longer exists: its service was suspended after a 2023 incident in San Francisco, and GM wound the business down entirely in late 2024. The defunct middle path is still worth teaching precisely because it failed --- it shows that conditional automation with remote monitoring sits in an unstable middle ground between Tesla's habituated-supervisor model and Waymo's no-in-vehicle-human model, and students should be able to argue why that middle ground collapsed.
\end{technote}
...
\begin{enumerate}
    ...
    \item The same sign with adversarial stickers (from the Eykholt et al.\ 2017 study, publicly available)
