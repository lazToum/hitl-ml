...
\subsubsection{Bias or Learning?}

\paragraph{Model Answer Framework.}

A strong answer proposes both explanations and a measurement strategy that can separate them.

\begin{itemize}
    \item \textbf{Automation bias explanation:} the reviewer is deferring more to the AI over time --- agreeing because the recommendation is present, not because their independent judgment has improved. Expected signature: agreement rises across the board, including on cases where the AI is wrong; accuracy on wrong-AI cases falls.
    \item \textbf{Genuine learning explanation:} the reviewer's judgment has improved through feedback and experience. Expected signature: agreement rises specifically on cases where the AI is right; the reviewer still overrides confidently --- and correctly --- when the AI is wrong.
    \item \textbf{Discriminating data:} override accuracy over time (are overrides right?); accuracy on cases where the AI is wrong (blind or seeded audit cases); comparison of the reviewer's independent assessments (recorded before seeing the AI recommendation) against final decisions. If independent assessments are unchanged while final decisions drift toward the AI, it is bias, not learning.
    \item \textbf{Why it matters:} rising agreement looks like a healthy system on the surface. Only the wrong-AI accuracy and independent-assessment comparisons reveal whether the human component is adding value or silently rubber-stamping.
\end{itemize}

\paragraph{Note on the SDT extension question.}
The optional Signal Detection Theory question in the teacher's manual formalizes this same distinction: automation bias as a criterion shift versus genuine improvement as a $d'$ increase. It has no model answer here because the material is outside the chapter; instructors using it should supply their own SDT grading criteria.
