...
\begin{itemize}
    \item Analyze reviewer behavior patterns using the three psychological failure modes (automation bias, complacency, calibration mismatch)
    \item Distinguish between overtrust (automation bias) and undertrust (algorithm aversion) in described reviewer behavior
...
\begin{itemize}
    ...
    \item \textbf{Chapter~12:} Measurement framework --- precision, recall, and reviewer agreement rate provide the vocabulary for describing reviewer performance
...
\paragraph{Optional instructor extension: Signal Detection Theory Mini-Lecture (15 min).}
\emph{This material is not covered in the chapter; use it only if students have been introduced to Signal Detection Theory elsewhere.} Introduce $d'$ (discriminability) and criterion $c$. Automation bias is a criterion shift toward the AI's recommendation. Automation complacency is a decline in $d'$ over time. Calibrated trust is having appropriate $d'$ for the domain.
...
\begin{enumerate}
    ...
    \item \textbf{Bias or Learning?:} ``A reviewer's decisions increasingly agree with the AI's recommendations over time. Propose two explanations --- one reflecting automation bias, one reflecting genuine improvement in the reviewer's judgment --- and describe what data you would collect to distinguish between them. Why does the distinction matter for HITL design?''

    \item \textbf{SDT Application (instructor extension):} ``Using Signal Detection Theory, explain the difference between automation bias (a criterion shift) and genuine reviewer expertise improvement (a $d'$ increase). How would you distinguish between the two empirically, and why does the distinction matter for HITL design?'' \emph{This question requires Signal Detection Theory material not covered in the chapter; assign it only alongside supplementary SDT instruction (see the optional mini-lecture in the 75-minute session).}
\end{enumerate}
