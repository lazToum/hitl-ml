...
The four failure modes are not mutually exclusive. The Apple Card case shows how they layer: a suspected model-level proxy-bias mechanism (suspected, but never established at population level --- the NYDFS analysis of roughly 400,000 New York applicants found no evidence of unlawful discrimination) sat on top of a human servicing layer with no mechanism to explain, challenge, or appeal individual decisions. The diagnostic task is to identify which failures are present and which is primary --- because each requires a different remedy.
\end{keyconcept}
...
The Apple Card case is the most compelling demonstration: the NYDFS inquiry could proceed at all because Goldman Sachs had logged its credit decisions. Those records allowed a statistical analysis of roughly 400,000 New York applicants --- turning viral social-media anecdotes into an answerable regulatory question. Without that audit trail, the complaints would have remained unverifiable.
...
0:31--0:43 & Case studies: Apple Card or content moderation & Case study slides \\
...
\begin{itemize}
    ...
    \item \textbf{Annotation failure:} Dialect bias in content moderation (Sap et al.\ on African American English; Facebook's 2020 civil rights audit). Sneakiest mode because the model is doing what it was trained to do --- the problem is the training.
    \item \textbf{Design failure:} Alert fatigue (clinical decision support; documented dismissal rates of 46--96\% of alerts). Invisible in standard accuracy metrics.
    \item \textbf{Reviewer failure:} Reviewers measured on throughput who defer to the model's scores --- automation bias and perverse incentives eroding the human layer the system was counting on.
\end{itemize}
...
Walk through the Apple Card investigation. Emphasize the twist students will not expect: the regulator found \emph{no} evidence of unlawful discrimination at the population level --- the model did not use gender, directly or through proxies the data could establish. The failure the NYDFS actually faulted was the human layer: customer-service representatives who could neither explain nor override the algorithm's output. Then note the coda: in October 2024 the CFPB ordered Goldman Sachs to pay a \$45 million civil penalty and at least \$19.8 million in consumer redress over Apple Card servicing failures. The underwriting model had passed its fairness audit; the humans around it had not.

Then run the chapter's own Five Whys on the annotation-bias failure:
\begin{enumerate}
    \item \textbf{Why did the model make systematically incorrect predictions?} Because it was trained on biased labels.
    \item \textbf{Why were the labels biased?} Because the annotation team was working under time pressure and used available news framing as implicit context.
    \item \textbf{Why were they working under time pressure?} Because the annotation schedule was set by a product team that didn't understand annotation quality degrades under pressure.
    \item \textbf{Why didn't the product team understand this?} Because there was no standardized process for communicating annotation quality constraints to product planning.
    \item \textbf{Why wasn't that process in place?} Because the organization's HITL processes were designed primarily around speed, not quality --- and no one owned cross-functional accountability for label quality.
\end{enumerate}
...
Connect audit trail to the Apple Card case: the NYDFS could analyze roughly 400,000 New York applicants because Goldman Sachs had logged its credit decisions.
...
\begin{enumerate}
    ...
    \item \textbf{Apple Card Layered Failures:} ``The NYDFS investigation of the Apple Card found no evidence of unlawful discrimination in the underwriting model --- yet the chapter treats the case as a HITL failure. Which failure modes were actually present, and what does the case teach about the limits of a model-level fairness audit?''\\
    \textit{Target: Not model failure at population level (no gender input; no established disparity across roughly 400,000 New York applicants). The failure was at the human layer --- Intervention Design plus Feedback Integration: customer-service representatives were in the loop but could neither explain nor override decisions, and there was no appeal channel, so individual harms surfaced only via social media and, later, a \$45 million CFPB servicing penalty. Lessons: an aggregate clean bill of health is not an answer to an individual asking \emph{why}; absence of a protected attribute in the feature list is not a fairness guarantee; detection happened outside the bank's own monitoring entirely.}
    ...
    \item \textbf{Design Failure Diagnosis:} ``Consider a hypothetical hospital whose clinical decision support system generates 120 alerts per physician per day, of which physicians dismiss 95\% --- near the top of the 46--96\% dismissal range documented in the chapter. Which failure mode is this, and what are the consequences?''\\
...
\begin{enumerate}
    \item \textbf{Remediation Design:} ``Design a complete remediation plan for the Apple Card case. Include: immediate harm reduction, root cause fix, monitoring to detect recurrence, and stakeholder communication strategy.''
