...
\textbf{Prompt:} ``Explain the trade-off in industrial alarm systems. Why can't you alert the human about everything? Why can't you trust the AI to handle everything? What does this tell you about the purpose of HITL in high-volume monitoring?''
...
\subsubsection{Shared Autonomy Application}

\textbf{Prompt:} ``Apply the shared autonomy framework to another domain not mentioned in the chapter. What does `the robot handles low-level, human handles high-level' translate to in your domain?''

\paragraph{Model Answer.}

Accept any domain with well-reasoned analysis. The grading criteria: (1)~the low-level/high-level division is specific, not generic; (2)~the handoff mechanism between levels is identified; (3)~the analysis notes what happens when the levels disagree. Three reference answers:

\begin{description}
    \item[Surgical robotics] The robot handles precision fine motor execution --- tremor filtering, motion scaling, maintaining a steady tool position. The surgeon handles incision strategy, tissue assessment, and the decision to change approach mid-procedure. Handoff: the surgeon's movements are continuously translated; disagreement surfaces when the robot's safety constraints (e.g., virtual fixtures bounding the workspace) override the surgeon's commanded motion.
    \item[Drone delivery] The drone handles flight control --- stabilization, navigation, obstacle avoidance at millisecond timescales. The human handles address verification, delivery confirmation, and edge cases (unclear drop-off location, recipient dispute, weather judgment). Handoff: the drone escalates when sensor confidence drops or the delivery site does not match expectations.
    \item[Automated trading] The algorithm handles execution --- order timing, routing, slippage minimization. The human handles strategy, position limits, and risk management, plus the decision to halt trading under anomalous market conditions. Handoff: pre-committed circuit breakers and kill-switch authority; the 2010 Flash Crash (Chapter~15) illustrates what happens when the high-level human layer cannot intervene at machine speed.
\end{description}

In each case the shared autonomy principle is the same: the machine takes the layer where speed and precision dominate; the human takes the layer where context and judgment dominate; and the design work is in the boundary between them.
...
A standard nurse-to-patient ratio of 1:2 to 1:3 means each nurse manages 2--3 patients per shift. Target: $\leq 15$ actionable alarms per patient per day; using 3 patients per nurse (the high end of the ratio), that is 45 alarms per nurse per day, or $\approx 15$ per nurse per 8-hour shift. This requires a model precision (positive predictive value) of at least 60\% on alarms produced and recall $\geq 99\%$ on critical events.
...
\subsubsection{BCI Ethics}

\textbf{Prompt:} ``In a motor BCI, the human's neural signals are directly in the feedback loop. The human cannot always `opt out'; feedback is implicit rather than explicit; and co-adaptation may change the user's neural patterns over time. What specific ethical obligations does this create for BCI designers?''

\paragraph{Model Answer Framework.}

A strong answer converts each of the three properties named in the question into a concrete design obligation, rather than gesturing at ``ethics'' generally.

\paragraph{No reliable opt-out $\to$ obligation of reversibility and graceful failure.}
If the user cannot simply stop participating, the designer owes: (1)~a safe fallback state the system can reach autonomously (the prosthetic holds position rather than dropping or freezing mid-motion); (2)~failure modes that degrade toward safety, not away from it; (3)~the ability for the user to perform the task, or an acceptable substitute, without the device --- or honest disclosure when that is impossible.

\paragraph{Implicit feedback $\to$ obligation of transparency about what is being learned.}
When adaptation signal is implicit (neural patterns, usage behavior) rather than explicit labels, the user cannot inspect or correct what the system is learning from them. The designer owes: disclosure of what signals are collected and how they drive adaptation; on-request explanation of how the system's behavior has changed; and limits on using neural data for purposes beyond the device's function (secondary use of neural data is a consent problem, not a feature).

\paragraph{Co-adaptation changing the user $\to$ obligation of monitoring and non-exploitation.}
If use of the device changes the user's neural patterns over time, the designer owes: longitudinal monitoring of user-side adaptation, not just device performance; transparency about known co-adaptation effects; reversibility planning where adaptation is found to be harmful; and a commitment that user-side changes must not create dependency the designer can exploit (e.g., adaptation that locks the user into one vendor's system).

\paragraph{The general principle.}
In ordinary HITL systems the feedback loop changes the \emph{model}; the human is assumed to exit the loop unchanged. In BCIs both components of the system are plastic. Every ethical obligation above follows from that single fact: the system designer is partly designing changes in a person, and must accept responsibility for the human side of the loop, not only the machine side.
...
\begin{itemize}
    ...
    \item Connect to the crisis-line story: the words were fine; the voice was not
