...
The crisis-line opening story is specifically designed to surface a concept that is non-obvious: the gap between what a model can be trained on and what a human expert perceives.
...
In the crisis-line story: text model says low-risk, audio model would say high-risk. The disagreement between models is itself a signal of uncertainty even when neither individual model is uncertain. This principle generalizes to any system integrating multiple data streams (sensor fusion, multimodal AI, time-series + contextual data).
...
\midrule
0:00--0:07 & Opening: crisis-line story (cold, no framing) & None \\
...
\subsubsection{Opening: The Crisis-Line Story (7 minutes)}

Tell the story cold, before any framing. A trained crisis counselor was monitoring call quality when something seemed off. The AI had assigned a low-risk score and the transcript looked fine. But the counselor played the audio. The voice was not the voice of someone who was fine.

\begin{trythis}
\textbf{Ask:} ``What did the model fail to detect? What did the counselor detect that the model didn't?''\\[0.5em]
...
\begin{itemize}
    ...
    \item \textbf{Crisis detection design:} The crisis-line solution is a parallel prosody model whose disagreement with the semantic model triggers human review. Model-to-model disagreement as uncertainty signal.
\end{itemize}
