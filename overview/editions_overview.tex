...
\begin{center}
  ...
  {\footnotesize\color{midgrey}\sffamily 2026}
...
These five dimensions analyze a system from the outside. The Spring edition works from a complementary, builder-facing set of five --- Control, Transparency, Feedback, Autonomy, and Trust --- the same design space seen from inside the build; its Chapter~2 maps the two framings onto each other.
...
\begin{enumerate}
  \item \textit{``Human in the Loop = Rubber Stamping.''} — Your job is not to approve what the AI decided. It is to notice when the AI's decision and your judgment diverge and register that divergence.
  \item \textit{``Human in the Loop = Backup Plan for Bad AI.''} — The human is not in the loop because the AI can't handle things yet. The human is there because the system needs to stay accountable to what humans value.
  \item \textit{``Human in the Loop = The Human Is in Charge.''} — Oversight without visibility, diagnostic tools, or the authority to override is not control. It is responsibility without power.
  \item \textit{``I'm Just One Human in Millions of Loops.''} — Your labels are not noise. They are signals that aggregate. One consistent mislabeler is not invisible to a training pipeline.
  \item \textit{``Human in the Loop = Slowing Things Down.''} — Slowing down is sometimes the point. The cases you stop to look at are the ones where speed would have produced harm.
  \item \textit{``Human in the Loop = Keeping AI Honest.''} — Honesty is not a role a human can fill one output at a time. Ethical failures are structural. Your job is to notice the pattern, not just the output.
\end{enumerate}
...
\textbf{Spot the Human.} Scenario-based exercises presenting a concrete situation where a learner must identify where human judgment adds value, where the system is overconfident, and what the design should change. Each chapter closes with a ``One more thing'' punchline that challenges the learner's assumptions.
...
% WINTER EDITION
%--------------------------------------------------------------
\section{Winter Edition: Full Course Textbook}

\begin{tcolorbox}[editionbox=textbookteal, title=Winter Edition \textbullet\ Academic Course Edition]

\subsection*{\color{charcoal} What It Is}

The Winter edition is the scholarly anchor of the series. It is written for semester-long courses at the undergraduate or graduate level. It proceeds from first principles — what is a decision, what is uncertainty, what is accountability — through to applied system design and policy implications. It is long, rigorous, and rewarding in the way that careful books are.

\subsection*{\color{charcoal} Structure: 19 Chapters and Appendices}

\begin{minipage}[t]{0.47\textwidth}
\textbf{Part I: When Machines Ask for Help} (Ch. 1--5)\\
{\small
Ch. 1: The Computers That Ask for Help\\
Ch. 2: When Smart Systems Get Confused\\
Ch. 3: How Computers Learn (The Simple Version)\\
Ch. 4: From Confusion to Decision\\
Ch. 5: The Art of Asking for Help (Without Being Annoying)\\
}

\vspace{8pt}
\textbf{Part II: The Human Side of the Loop} (Ch. 6--9)\\
{\small
Ch. 6: Interfaces That Make Humans Smarter\\
Ch. 7: Getting Good Information from People\\
Ch. 8: The Blueprint for Smart Helper Systems\\
Ch. 9: Tools Anyone Can Use\\
}

\vspace{8pt}
\textbf{Part III: Teaching Machines to Perceive} (Ch. 10--12)\\
{\small
Ch. 10: Teaching Computers to Understand Language\\
Ch. 11: Teaching Computers to See\\
Ch. 12: Measuring Success\\
}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.47\textwidth}
\textbf{Part IV: The Human Factor} (Ch. 13--15)\\
{\small
Ch. 13: The Psychology of Human-AI Teams\\
Ch. 14: Building Fair and Ethical Systems\\
Ch. 15: When Things Go Wrong (And How to Fix Them)\\
}

\vspace{8pt}
\textbf{Part V: Beyond and Ahead} (Ch. 16--19)\\
{\small
Ch. 16: Beyond Text and Images\\
Ch. 17: Almost-Autonomous: When AI Learns to Ask Better Questions\\
Ch. 18: Who Owns the Loop?\\
Ch. 19: Your Role in This Future\\
}
\end{minipage}

\vspace{10pt}

\textbf{Appendices include:} A technical appendix for every chapter (nineteen in all) with the underlying mathematics and reference implementations; a glossary; a full bibliography; and an index.

\subsection*{\color{charcoal} Instructor Materials}

The teaching package includes, for every chapter, a teacher's manual (learning objectives, discussion questions with instructor notes, classroom activities, assessment strategies, common misconceptions, and a sample 50-minute lesson plan) and a full solutions manual covering the discussion questions and the technical-appendix exercises.
...
\subsection*{\color{charcoal} Exam Study Companion}

The Winter edition is accompanied by a separate study companion (\textit{Winter is Coming}) that consolidates frameworks, key cases, vocabulary, and practice questions into a compact exam-reference format. It includes structured case summaries, four tiers of practice questions, and a self-test for identifying weak spots before exams.
...
\begin{tcolorbox}[amberstyle]
The characters appear across editions but are most fully realized in the Autumn edition (\textit{Misunderstood}) and the Summer Workbook. The Winter study companion cites them by name in practice questions.
\end{tcolorbox}
...
\textbf{Optional:} Winter Textbook, Part II (The Human Side of the Loop) and Part IV (The Human Factor)\\[6pt]
...
The main textbook's teaching package provides a per-chapter teacher's manual and solutions manual, including sample lesson plans that pace a chapter into a 50-minute session. The characters from \textit{Misunderstood} can anchor discussion sections and case analysis workshops throughout the course.
\end{tcolorbox}
