...
3. The "human" in RLHF is usually a team of paid contractors doing thousands of comparisons quickly — often at low wages, with minimal training, in jurisdictions with weak labor protections. Does that change how you think about the word "human"? What happens to "human values" when the humans encoding them are under time pressure and economic precarity?
...
One response to this is **Direct Preference Optimization (DPO)** [@rafailov2023direct] — an alternative to the PPO-based training loop in classic RLHF. (The full classic pipeline has four stages: supervised fine-tuning on demonstration data, training a reward model on human preference comparisons, optimizing the policy against that reward model with PPO, and iterating. DPO collapses the last two stages into one by showing that the optimal policy under the Bradley-Terry preference model can be expressed directly, without training a separate reward function.) It doesn't eliminate Goodhart's Law entirely — the same human preferences still shape what gets rewarded — but it removes one surface where the optimization can drift.
...
<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/06_rlhf.html` from the accompanying zip archive.
```
