...
- DAgger tries to fix distribution shift by asking the expert what they would do in the states the model actually visits — then aggregates those new labels with the original data and retrains, repeating the cycle until the policy stops drifting into unfamiliar territory. Why is that different from just recording more drives?
...
*Core references: [@ross2011reduction; @pomerleau1989alvinn; @ho2016generative].*

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/07_demonstrations.html` from the accompanying zip archive.
```
