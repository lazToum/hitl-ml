...
3. Think about how much the task of labeling an image pays on Mechanical Turk — maybe a few cents. MTurk workers are classified as independent contractors: no minimum wage, no benefits, no workers' compensation. Does that affect what kind of answers you'd expect? What kind of workers? And whose responsibility is it when the "human in the loop" can't afford to pay full attention?
...
*Core references: [@dawid1979maximum; @gray2019ghost; @hara2018data].*

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/13_crowdsourcing.html` from the accompanying zip archive.
```
