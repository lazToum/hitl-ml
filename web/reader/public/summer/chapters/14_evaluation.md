...
Accuracy measures the fraction of correct predictions. It does not measure whether the model learned anything. A human evaluator who looked at the outputs would notice immediately — in the scenario above, one finally did. The metric never would have. How many benchmarks have no such reviewer?

*Core references: [@cohen1960coefficient; @krippendorff2011computing; @ribeiro2020beyond].*

<!-- html-link-injected -->
```{only} latex
*Interactive version (word search, games):* open `summer_interactive/chapters/14_evaluation.html` from the accompanying zip archive.
```
