...
Judge Castel imposed sanctions on both attorneys in a written decision issued two weeks later. In the order that had first flagged the fabrications, he had already written something that should be printed on the login screen of every AI assistant: "The Court is presented with an unprecedented circumstance — a submission replete with citations to non-existent cases."
...
Research into hallucination rates is sobering. A 2024 Stanford study found that general-purpose language models hallucinated on 58% to 88% of legal queries — not getting things slightly wrong, but generating completely fictional citations, statutes, or holdings. A companion study of *purpose-built* legal research tools — systems explicitly marketed as nearly hallucination-free — found that even these still fabricated information between roughly 17% and 33% of the time. Across medical domains, studies of AI diagnostic outputs have found clinically significant error rates that vary widely by task and patient population. Even retrieval-augmented systems — models that can look things up — hallucinate at measurable rates.
