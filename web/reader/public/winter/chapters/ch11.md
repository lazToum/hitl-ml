...
When the AI confidently flags an abnormality, radiologists are more likely to agree with the AI's characterization — a phenomenon called **automation bias**. And when the advice itself is wrong, it drags the human down with it: in a controlled study, radiologists given deliberately inaccurate advice made significantly worse diagnoses than those given accurate advice — regardless of whether the advice was labeled as coming from an AI or from a human expert. The AI's confident output acts as a cognitive anchor.
