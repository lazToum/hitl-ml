...
In 2018, Reuters reported that Amazon had disbanded an experimental recruiting tool it had been developing since 2014. The AI was designed to screen résumés and surface top candidates for software engineering positions. Amazon's machine learning team had trained it on ten years of hiring data — the résumés of everyone the company had actually hired.

The problem: Amazon's engineering workforce had historically been predominantly male. The model learned this pattern. It taught itself that women's résumés were less suitable, even though gender was not an explicit input variable. It found proxies. The model learned to penalize résumés that included the word "women's" — as in "women's chess club" or "women's college." It downgraded graduates of two all-women's colleges.

Here is the part of the story that matters most. Amazon's engineers discovered the gender bias themselves, roughly a year into development. They tried to fix it. They removed the gender-correlated terms. The model adapted and found new proxies. When the team realized the model kept finding new signals correlated with gender, they concluded the problem was not fixable by removing specific features. They killed the project before it was ever used to evaluate real candidates. Amazon stated that recruiters never relied on the tool's rankings to make hiring decisions.

The Amazon recruiting case is instructive not as a story of an oversight failure that went undetected for years, but as a demonstration of a deeper structural problem: bias is not a bug you can patch. It is a signal the model correctly learned from biased data. The model was doing exactly what it was trained to do. The data reflected a biased world. The model faithfully encoded it. And the fix — removing proxies one by one — was unwinnable, because the entire training distribution was saturated with structural gender patterns. The engineers who killed the project understood what the story is meant to teach: proxy-hunting is an infinite game with no win condition. The only way to stop the model from finding new proxies for gender was to stop training it on a gendered world.

## The Layers Where Bias Enters

There is no single place where bias enters an AI system. It enters everywhere.

**Collection bias** is the most fundamental: whose data is collected? Early facial recognition datasets, used to train and evaluate models for decades, were overwhelmingly composed of light-skinned male faces. Models trained on these datasets were well-calibrated for some users and dramatically worse for others. A 2018 study by Joy Buolamwini and Timnit Gebru — the Gender Shades project — found commercial facial recognition systems from major technology companies had error rates up to 34 percentage points higher for darker-skinned women than for lighter-skinned men. The systems hadn't been designed to fail for these groups. They had simply been trained primarily on data that didn't include them. The same pattern holds geographically. In 2017, researchers at Google showed that image classifiers trained on the field's standard datasets — collected overwhelmingly from North America and Europe — performed far worse on images from the rest of the world: wedding photographs from Ethiopia and Pakistan drew the lowest recognition confidence, bridegrooms mislabeled with guesses like "chain mail" and "academic gown." A *Nature* commentary the following year supplied the pattern's most famous illustration: a bride in a white Western dress labeled *bride* and *wedding*; a bride in Indian wedding clothing labeled *costume* and *performance art.* The classifiers hadn't learned what a wedding is; they had learned what weddings look like in the places the data came from.
...
**Explainability** — understanding why a system made a specific prediction — is one response. If a loan application is rejected and the applicant asks why, they should receive an explanation that is genuinely informative. "The algorithm decided" is not an explanation; it is an evasion. The EU's General Data Protection Regulation grants individuals a right to "meaningful information about the logic involved" in automated decisions, and the EU Court of Justice has read this as requiring a concise, intelligible explanation of the procedure and principles actually applied to the individual — not a disclosure of the algorithm itself, but not an evasion either.
...
**Key Examples:**
- **Amazon recruiting AI (2014–2017):** Experimental model learned gender proxies; its builders found the bias within a year, could not remove it, and killed the tool before production use
