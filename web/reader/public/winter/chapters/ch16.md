...
Researchers have tried to teach machines to hear these signals too. Models trained on the *acoustic* texture of hotline calls — pitch, rhythm, timing, not just the words — can now flag elevated suicide risk better than chance. But only modestly so, and the people who build them are careful to frame them as an aid to the counselor rather than a substitute. The reason is the gap this chapter is about: a transcript can read as perfectly composed — measured, even grateful — while the voice it was lifted from is nothing of the kind. A call whose words say "I'm fine" can be a call from someone who is not.
...
The crisis-line example at the opening of this chapter illustrates the hardest problem in speech-based HITL for mental health applications: the gap between what a model can process and what a human can perceive.
