...
const BALL_R = 18;
const PIN_R = 11;
// Direct-neighbor inter-pin distance ≈ 37 px; CASCADE_R just above that
// so a knocked pin takes out its immediate ring.
const CASCADE_R = 39;
const START_Y = 378;
const FOUL_Y = 318;
const TARGET_Y = 110;
...
export function BowlingGame() {
  ...
  const drawFrame = useCallback(() => {
    ...
    if (!canvas) return;
    const dpr = window.devicePixelRatio || 1;
    const needW = Math.floor(CANVAS_W * dpr);
    const needH = Math.floor(CANVAS_H * dpr);
  ...
  useEffect(() => {
    const tick = (time: number) => {
      ...
      if (s.phase === "rolling" && s.ball) {
        ...
        // Real-time collision: knock any pin the ball touches
        const hit: number[] = [];
        s.pins.forEach((pin, i) => {
          if (!pin.standing) return;
          const dist = Math.hypot(s.ball!.x - pin.x, s.ball!.y - pin.y);
          if (dist < BALL_R + PIN_R) {
            pin.standing = false;
            hit.push(i);
            // Slight lateral deflection — heavier ball barely moves
            const nx = (s.ball!.x - pin.x) / Math.max(dist, 1);
            s.ball!.vx += nx * 0.45;
          }
        });

        // Single-pass cascade: each just-knocked pin topples its immediate neighbors
        hit.forEach((i) => {
          const src = s.pins[i];
          s.pins.forEach((other, j) => {
            if (j === i || !other.standing) return;
            if (Math.hypot(src.x - other.x, src.y - other.y) < CASCADE_R) {
              other.standing = false;
            }
          });
        });

        // End: ball exits the pin zone or gutters
        if (s.ball.y < 44 || s.ball.x < LANE_L - BALL_R || s.ball.x > LANE_R + BALL_R) {
          s.phase = "settling";
          settleAtRef.current = time + 400;
        }
