...
const DEFAULT_WORDS = [
  "HUMAN", "LABEL", "MODEL", "TRUST", "ANNOTATE", "FEEDBACK",
  "DATASET", "ETHICS", "BIAS", "REVIEW", "LOOP", "AGENT",
];

interface Props { words?: string[] }

export function CrosswordGame({ words = DEFAULT_WORDS }: Props) {
  const effective = words.length >= 3 ? words : DEFAULT_WORDS;
  const { placed, grid: rawGrid } = useMemo(
    () => buildCrossword(effective.map((w) => w.toUpperCase())),
    [effective]
  );
