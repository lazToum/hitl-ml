...
export function PongGame() {
  ...
  // Mouse/touch: control left paddle (or right paddle in 2P if on right half)
  const setPaddleFromClient = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const norm = Math.max(
        PADDLE_HALF,
        Math.min(1 - PADDLE_HALF, (clientY - rect.top) / rect.height),
      );
      const s = stateRef.current;
      if (s.twoPlayer && clientX - rect.left >= rect.width / 2)
        s.right = norm;
      else s.left = norm;
    },
    [],
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) =>
      setPaddleFromClient(e.clientX, e.clientY),
    [setPaddleFromClient],
  );
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent<HTMLCanvasElement>) => {
      e.preventDefault();
      const t = e.touches[0];
      if (t) setPaddleFromClient(t.clientX, t.clientY);
    },
    [setPaddleFromClient],
  );
