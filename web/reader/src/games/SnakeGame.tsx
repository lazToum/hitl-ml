...
function drawScene(canvas: HTMLCanvasElement, w: number, h: number, state: SnakeState) {
  ...
  const cell = Math.min(cw, ch); // for radius/size calculations
  const foodX = state.food.x * cw + cw / 2;
  const foodY = state.food.y * ch + ch / 2;
  const apple = ctx.createRadialGradient(
    foodX - cell * 0.12, foodY - cell * 0.14, cell * 0.08,
    foodX, foodY, cell * 0.42,
  );
  ...
  state.snake.forEach((part, index) => {
    ...
    const x = part.x * cw + inset;
    const y = part.y * ch + inset;
    const sw = cw - inset * 2;
    const sh = ch - inset * 2;
    ...
    if (index === 0) {
      const dx = state.heading.x * cw * 0.08;
      const dy = state.heading.y * ch * 0.08;
      ctx.fillStyle = "#083344";
      ctx.beginPath();
      ctx.arc(x + sw * 0.34 + dx, y + sh * 0.34 + dy, Math.max(2, cell * 0.055), 0, Math.PI * 2);
      ctx.arc(x + sw * 0.66 + dx, y + sh * 0.34 + dy, Math.max(2, cell * 0.055), 0, Math.PI * 2);
      ctx.fill();
    }
  });
...
export function SnakeGame() {
  ...
  const boardH = Math.round(boardW * GRID_ROWS / GRID_COLS);
  ...
  const draw = useCallback(() => {
    ...
    drawScene(canvas, boardW, boardH, stateRef.current);
  }, [boardW, boardH]);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      const w = Math.max(300, Math.floor(host.clientWidth - 8));
      setBoardW(w);
    };
